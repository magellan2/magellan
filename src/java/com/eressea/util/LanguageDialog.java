...
public class LanguageDialog {
	private final static Logger log = Logger.getInstance(LanguageDialog.class);
		...
		try{
			...
			if (files != null && files.length > 0) {
				for(int i = 0; i < files.length; i++) {
					if (files[i].toString().toLowerCase().endsWith(jarSuffix) && !files[i].toString().toLowerCase().endsWith(magSuffix)) try {
