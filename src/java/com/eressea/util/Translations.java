...
/**
 * Helper class for centrally managing translation tables (resource
 * bundles) for components.
 */
public class Translations {
	private final static Logger log = Logger.getInstance(Translations.class);
	...
	/**
	 * Sets the class loader to use for loading resource
	 * bundles.
	 */
	...
	/**
	 * Returns the resource bundle for the specified class using the
	 * GUI locale. The corresponding properties file must be named
	 * like the class c with all '.' (periods) replaced bu '-'
	 * (dashes).
	 * @deprecated This function is deprecated, use getTranslation instead
	 */
	public static ResourceBundle getGUIBundle(Class c) {
		return getBundle(c, Locales.getGUILocale());
	}

	/**
	 * Returns the translated string for the specified class and string using the
	 * GUI locale. The corresponding properties file must be named
	 * like the class c with all '.' (periods) replaced bu '-'
	 * (dashes).
	 * @param Object for which the translation is searched
	 ...
	 * like the class c with all '.' (periods) replaced bu '-'
	 * (dashes).
	 * @param Class for which the translation is searched
	 * @param key   the key to translate
		...
		if(result == null) {
			...
			if(result == null) {
				...
				if(!Locale.ENGLISH.equals(Locales.getGUILocale())) {
					if(!key.endsWith(".tooltip") && !isInstanceOf(c,"com.eressea.demo.actions.MenuAction")) {
						log.error("Translations.getTranslation("+c+","+key+"): no translation found for Locale "+
								  Locales.getGUILocale()+", calling getDefaultTranslationFromClass");
					}
				}
				if(log.isDebugEnabled()) {
					log.debug("Translations.getTranslation("+c+","+key+"): no translation found for Locale "+
							  Locales.getGUILocale()+", calling getDefaultTranslationFromClass");
				}
				// fall back to key
				result = key;
	...
	private static String getTranslationFromClass(Class c, String key) {
		...
		if(t!=null) {
			...
			if(c.getSuperclass() != null) {
				if(log.isDebugEnabled()) {
					log.debug("Translations.getTranslationFromClass("+c+","+key+"): climbing to super class "+c.getSuperclass());
				}
	...
	/**
	 * Returns the translated string for the specified bundle and string using the
	 * GUI locale. The corresponding properties file must be named
	 * like the class c with all '.' (periods) replaced bu '-'
	 * (dashes).
	 * @param Class the name of the bundle
	 * @param key  the key to translate
	 * @return the translation, if found, or null instead
	 */
	private static String getBundleTranslation(Class c,String key) {
		...
		if(rb != null) {
			...
			if(c.getSuperclass() != null) {
				if(log.isDebugEnabled()) {
					log.debug("Translations.getBundleTranslation("+c+","+key+"): climbing to super class "+c.getSuperclass());
				}
	...
	/**
	 * Returns the translations of the orders using the current order
	 * locale.
	 * @deprecated This function is deprecated, use getOrderTranslation instead
	 */
	public static ResourceBundle getOrderBundle() {
		return getBundle(null, Locales.getOrderLocale());
	}


	/**
	 * Returns the translations of the orders using the specified
	 * order locale.
	 * @deprecated This function is deprecated, use getOrderTranslation instead
	 */
	public static ResourceBundle getOrderBundle(Locale locale) {
		return getBundle(null, locale);
	}

	/**
	 * Returns the translated string for the specified class and string using the
	 * order locale. The corresponding properties file must be named
	 * like the class c with all '.' (periods) replaced by '-'
	 * (dashes).
	 * @param key the key to translate
	...
	public static String getOrderTranslation(String key, Locale l) {
			...
			try {
				...
				if(translation != null) {
					if(log.isDebugEnabled() && !loggedOrderTranslations.contains(key)) {
						log.debug("Translations.getOrderTranslation("+key+","+l+"): \""+translation+"\"");
		...
		if(!Locale.ENGLISH.equals(l)) {
			log.warn("Translations.getOrderTranslation("+key+","+l+"): no valid translation found, returning key");
		}
	...
	/**
	 * Loads a resource bundle with the specified name and locale from
	 * a 'lang' subdirectory.
	 *
	 */
	 ...
	 * java.util.ResourceBundle.getBundle() but it ignores the default
	 * locale and it only loads PropertyResourceBundle objects
	 * requiring appropriate .properties files.
	 */
	public static PropertyResourceBundle loadResourceBundle(final String name, Locale l, ClassLoader loader) {
		String fileName = null;
		java.io.InputStream in = null;

		try {
			// baseclass + "_" + language1 + "_" + country1 + "_" + variant1
			fileName = name + "_" + l.getLanguage() + "_" + l.getCountry() + "_" + l.getVariant() + ".properties";
