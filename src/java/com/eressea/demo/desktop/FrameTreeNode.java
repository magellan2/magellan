 ...
 */
	...
	public void write(PrintWriter out) {
		if (isLeaf()) {
			...
			out.println("SPLIT "+getPercentage()+' '+ (getOrientation()==javax.swing.JSplitPane.HORIZONTAL_SPLIT?'H':'V'));
			if (getChild(0)!=null)
	...
	public void refreshPercentage() {
		...
		double fullsize = (double) ((splitPane.getOrientation() == JSplitPane.VERTICAL_SPLIT) ? splitPane.getHeight() : splitPane.getWidth()
		...
		setPercentage(new BigDecimal(location).divide(new BigDecimal(fullsize),2,BigDecimal.ROUND_HALF_UP).doubleValue());
/*if (!isLeaf())
			for(int i=0;i<2;i++)
	 ...
	 * null means no configurable object/no configurations.
