...
public class ClientPreferences extends InternationalizedPanel implements ExtendedPreferencesAdapter {
	private final static Logger log = Logger.getInstance(ClientPreferences.class);
	...
	private void initGUI() {
        ...
        GridBagHelper.setConstraints(
            c, 0, 3, GridBagConstraints.REMAINDER, 1, 1.0, 1.0, /* different weighty!*/
            GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL, c.insets, 0, 0);
		// progress panel
	...
	private Component getTempUnitPanel() {
		...
		GridBagConstraints c2 = new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.VERTICAL, new Insets(4, 4, 4, 4), 0, 0);
		...
		//pavkovic
		Boolean ascending = Boolean.valueOf(settings.getProperty("ClientPreferences.ascendingOrder", "true"));
		ascendingOrder  = new JRadioButton(getString("tempids.ascendingorder.caption"), ascending.booleanValue());
		descendingOrder = new JRadioButton(getString("tempids.descendingorder.caption"), !ascending.booleanValue());
		...
		c2.gridx = 0;
		c2.gridwidth = 3;
		showTempUnitDialog = new JCheckBox(getString("showtempunitdialog"), settings.getProperty("MultiEditorOrderEditorList.ButtonPanel.ShowTempUnitDialog", "true").equalsIgnoreCase("true"));
	...
	/**
	 * Creates a panel for the controls determining startup and exit
	 * behaviour.
	 */
	private Component getStartupExitPanel() {
		GridBagConstraints c = new GridBagConstraints();
		JPanel pnl = new JPanel(new GridBagLayout());
		pnl.setBorder(new javax.swing.border.TitledBorder(BorderFactory.createEtchedBorder(), getString("border.startupexit")));

		chkCheckVersionOnStartup = new JCheckBox(getString("chk.checkversiononstartup.caption"), source.isCheckingVersionOnStartup());
	...
	/**
	 * Creates a panel containing the GUI elements for setting the
	 * locales.
	 */
	private Component getLocalesPanel() {
		...
		pnlLocales.setBorder(new javax.swing.border.TitledBorder(BorderFactory.createEtchedBorder(), getString("border.locales")));
	...
	public void applyPreferences() {
		...
		if (s == null || s.equalsIgnoreCase("")) {
			...
			try {
				...
				log.warn("ClientPreferences.applyPreferences: Error when parsing the initial value of the temp ids: " + tempIDsInitialValue.getText());
			}
			if (tempID > IDBaseConverter.getMaxId()) {
				log.warn("ClientPreferences.applyPreferences: Found tempID out of valid values: " + tempID);
				tempIDsInitialValue.setText("");
				settings.setProperty("ClientPreferences.TempIDsInitialValue", "");
				tempID = 0;
			}
			if (tempID != 0) {
				settings.setProperty("ClientPreferences.TempIDsInitialValue", IDBaseConverter.toString(tempID));
			}
		}
	...
	/**
	 * A wrapper for storing Locale objects in a combo box while
	 * displaying a full name instead of only a language code.
	 */
	...
	protected void changeMetalBackground() {
		...
		if (laf instanceof javax.swing.plaf.metal.MetalLookAndFeel) {
			Color col = JColorChooser.showDialog(this, getString("desktopcolor.title"), MetalLookAndFeel.getWindowBackground());
			if (col != null) {
	...
	protected class LAndF extends JPanel implements PreferencesAdapter, ActionListener {
		...
		protected Container createFontPanel() {
			...
			panel.setBorder(new javax.swing.border.TitledBorder(BorderFactory.createEtchedBorder(), getString("border.fontsize")));

			GridBagConstraints con = new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(3, 3, 3, 3), 0, 0);
		...
		protected Container createLAndFPanel() {
			...
			GridBagConstraints con = new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL, new Insets(3, 3, 3, 3), 0, 0);
			...
			panel2.setBorder(new javax.swing.border.TitledBorder(BorderFactory.createEtchedBorder(), getString("border.lookandfeel")));
			panel2.add(panel);
			return panel2;
		}
		...
		public void applyPreferences() {
			try {
				...
				this,
				getString("msg.fontsizeerror.text") + ex.toString(),
	...
	protected class HistoryPanel extends JPanel implements PreferencesAdapter {
		...
		private void initGUI() {
			...
			help.setBorder(new TitledBorder(new CompoundBorder(BorderFactory.createEtchedBorder(), new EmptyBorder(0, 3, 3, 3)), getString("border.filehistory")));
			help.setLayout(new GridBagLayout());
			GridBagConstraints con = new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL, new Insets(0,0,0,0), 0, 0);
	...
	public synchronized static Map getDefaultTranslations() {
		if(defaultTranslations == null) {
			...
			defaultTranslations.put("txt.restartforfontsize.caption" , "Magellan has to be restarted to have the new font size setting take effect.");
			...
			defaultTranslations.put("lbl.filehistoryentries.caption" , "Number of file-history entries: ");

			defaultTranslations.put("txt.filehistorydescription.text" , "This determines how many recently loaded report files are listed in the File menu.");
			defaultTranslations.put("msg.fontsizeerror.text" , "The following error occurred while setting the new font-size: ");
			...
			defaultTranslations.put("tempidsinitialvalue.tooltip" , "Starting with this value the suggested tempunit ids gets counted. The number can be in- or decreased.");
