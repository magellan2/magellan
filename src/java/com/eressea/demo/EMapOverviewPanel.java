...
public class EMapOverviewPanel extends InternationalizedDataPanel implements TreeSelectionListener, TreeExpansionListener, SelectionListener, OrderConfirmListener, PreferencesFactory, TempUnitListener, ShortcutListener, ChangeListener, TreeUpdate, MenuProvider {
	private final static Logger log = Logger.getInstance(EMapOverviewPanel.class);
	// GUI elements
	...
	public final static int EXPAND_FLAG = 1;
	public final static int EXPAND_IFINSIDE_FLAG = 2;

	private int expandMode = EXPAND_FLAG | (3 << 2);
	private int expandTrustlevel = Faction.TL_PRIVILEGED;

	public final static int COLLAPSE_FLAG = 1;
	public final static int COLLAPSE_ONLY_EXPANDED = 2;
	...
	private final static Comparator idCmp = new IDComparator();
	private final static Comparator nameCmp = new NameComparator(idCmp);
	...
	public EMapOverviewPanel(EventDispatcher d, Properties p) {
		...
		nodeWrapperFactory=new NodeWrapperFactory(settings,"EMapOverviewPanel",getString("wrapperfactory.title"));
		...
		scpTree = new JScrollPane(tree, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		...
		scpHistory=new JScrollPane(lstHistory, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	 ...
	 * Note: It is significant for the valueChanged() method, how
	 * deep the different node types are nested.
	 */
	...
	protected void rebuildTree() {
		...
		boolean createIslandNodes = (new Boolean(settings.getProperty("EMapOverviewPanel.displayIslands", "true"))).booleanValue() == true && (new Boolean(settings.getProperty("EMapOverviewPanel.sortRegions", "true"))).booleanValue() == true && settings.getProperty("EMapOverviewPanel.sortRegionsCriteria", "coordinates").equalsIgnoreCase("islands") == true;
		
		if (treeBuilder == null) {
			createTreeBuilder();
		}
		
		treeBuilder.setMode((treeBuilder.getMode()&16383) | (createIslandNodes?treeBuilder.CREATE_ISLANDS:0));
	...
	/**
	 * Retrieve a Comparator to sort the units according to
	 * the settings.
	 */
	public static Comparator getUnitSorting(Properties settings) {
		...
		if (criteria.equals("skills")) {
			if (settings.getProperty("EMapOverviewPanel.useBestSkill", "true").equalsIgnoreCase("true")) {
				cmp= new UnitSkillComparator(
						new BestSkillComparator(SkillComparator.skillCmp,
						new SkillTypeComparator(
						new SkillTypeRankComparator(new NameComparator(null), settings),
						SkillComparator.skillCmp), null), idCmp);
			} else {
		...
		while (i >= 0) {
			switch (treeStructure[i]) {
				case TreeHelper.FACTION:       help = new UnitFactionComparator(new FactionTrustComparator(nameCmp), cmp); break;
				case TreeHelper.GROUP:         help = new UnitGroupComparator(idCmp, cmp, cmp); break;
				case TreeHelper.COMBAT_STATUS: help = new UnitCombatStatusComparator(cmp); break;
				case TreeHelper.HEALTH:        help = new UnitHealthComparator(cmp); break;
				case TreeHelper.FACTION_DISGUISE_STATUS: help = new UnitFactionDisguisedComparator(cmp); break;
				case TreeHelper.TRUSTLEVEL:    help = new UnitTrustComparator(cmp); break;
				default:            help = cmp;
			}
			cmp = help;
			i--;
	...
	/**
	 * Retrieve the structure of the unit tree
...
//		System.out.println("");
	...
	/**
	 * Change event handler, change the display status of the tree
	 * if an unit orderConfimation Status changed
	 */
	 ...
	 *
	 * First the active object is considered and selected in the tree
	 * if contained.
	 * After that selected objects are considered, but only if != null
	 * and selection type is different to SelectionEvent.ST_REGIONS.
	 * In this case the tree selection is set to the selected objects
	 * (as long as they are contained in the tree anyway).
	 *
	 * Keep in mind, thtat this will produce the active object _NOT_ to be
	 * selected, if selectedObjects != null && !selectedObjects.contains(activeObject) !!!
	 */
	public void selectionChanged(SelectionEvent se) {
				...
				if (o instanceof Unit) {
					...
					if (g == null) {
						...
						if (f.allies == null && activeAlliances.values().size() > 0) {
							...
							activeAlliancesAreDefault = false;
							tree.repaint();
						} else if (f.allies != null && !f.allies.equals(activeAlliances)) {
							activeAlliances.clear();
							activeAlliancesAreDefault = false;
							activeAlliances.putAll(f.allies);
							// add the selected faction to be able to show, on whose alliances
		...
		/**
		 * HANDLE selectedObjects
		 *
		 * Don't change anything, if selectedObjects == null or
		 * selection event type is ST_REGIONS (which means that this is
		 * a selection of regions on the map or of regions to be selected
	...
	public void tempUnitCreated(TempUnitEvent e) {
		...
		if (parentNode != null) {
			...
			List siblings = CollectionFactory.createLinkedList(e.getTempUnit().getParent().tempUnits());
...
/*		parentNode.remove(unitNode);
	...
	public void shortCut(javax.swing.KeyStroke shortcut) {
		...
		switch(index) {
			...
			case 0:
			case 1: DesktopEnvironment.requestFocus("OVERVIEW");
			tree.requestFocus(); //activate the tree, not the scrollpane
			break;
			//case 2: shortCut_N();break;
			//case 2: toggleOrderConfirmation();break;
			case 2:
			case 4: jumpInSelectionHistory(1);break;
			case 3:
			case 5: jumpInSelectionHistory(-1);break;
		}
	}
	...
	protected void shortCut_N(boolean traverseDown) { 
		...
		if(u!=null) {
			if(log.isDebugEnabled()) {
				log.debug("EMapOverviewPanel.shortCut_N(): firing Selection Event with Unit "+u+" ("+u.ordersConfirmed+")");
			}
	...
	/**
	 * Repaints this component. This method also explicitly repaints
	 * it's managed components because the may be separated by the
	 * Magellan Desktop.
	 */
	public void repaint() {
		super.repaint();
	...
	private void toggleOrderConfirmation() {
		...
		if (activeObject instanceof Region) {
			...
			boolean first = true, confirm = false;
			for (Iterator iter = r.units().iterator(); iter.hasNext();) {
				Unit u = (Unit)iter.next();
				if (u.getFaction() != null && u.getFaction().trustLevel >= Faction.TL_PRIVILEGED && u.isSpy() == false) {
					if (first) {
	...
	/**
	 * Selects and entry in the selection history by a relative
	 * offset.
	 */
	...
	/**
	 * This is a helper method to set this.activeAlliances to a usefull
	 * value, if no faction or group is active.
	 * The idea is to take all alliances of all privileged factions
	 * and combine their states by & (or in other words to take the intersection
	 * over all alliances of all privileged factions)
	 */
	...
	protected void loadExpandProperties() {
		...
		}catch(Exception exc) {expandMode = EXPAND_FLAG | (3 << 2);}
		try{
			expandTrustlevel = Integer.parseInt(settings.getProperty("EMapOverviewPanel.ExpandTrustlevel"));
		}catch(Exception exc) {expandTrustlevel = Faction.TL_PRIVILEGED;}
	}
	...
	protected void loadCollapseProperty() {
		...
		}catch(Exception exc) {expandMode = 3 << 2;}
	}
	...
	private class EMapOverviewPreferences extends JPanel implements ExtendedPreferencesAdapter{
		...
		private class SkillPreferences extends JPanel implements PreferencesAdapter {
			...
			public SkillPreferences() {
				...
				this.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("prefs.skillorder")));
				...
				if (data != null) {
					...
					Collections.sort(v, new SkillTypeRankComparator(new NameComparator(new IDComparator()), EMapOverviewPanel.this.settings));
		...
		public EMapOverviewPreferences(Properties settings) {
			chkSortRegions = new JCheckBox(getString("prefs.sortregions"), (new Boolean(settings.getProperty("EMapOverviewPanel.sortRegions", "true"))).booleanValue());

			rdbSortRegionsCoordinates = new JRadioButton(getString("prefs.sortbycoordinates"), settings.getProperty("EMapOverviewPanel.sortRegionsCriteria", "coordinates").equals("coordinates"));
			rdbSortRegionsIslands = new JRadioButton(getString("prefs.sortbyislands"), settings.getProperty("EMapOverviewPanel.sortRegionsCriteria", "coordinates").equals("islands"));
			...
			pnlRegionSortButtons.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("prefs.regionsorting")));
			...
			chkDisplayIslands = new JCheckBox(getString("prefs.showislands"), (new Boolean(settings.getProperty("EMapOverviewPanel.displayIslands", "true"))).booleanValue());
			...
			pnlTreeStructure.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("prefs.treestructure")));

			JPanel elementsPanel = new JPanel();
			elementsPanel.setLayout(new BorderLayout(0, 0));
			elementsPanel.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("prefs.treeStructure.available")));
			...
			usePanel.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("prefs.treeStructure.use")));
			String criteria = settings.getProperty("EMapOverviewPanel.treeStructure", " " + TreeHelper.FACTION + " " + TreeHelper.GROUP);
			StringTokenizer tokenizer = new StringTokenizer(criteria);
			...
			// Unit sorting
			rdbSortUnitsUnsorted = new JRadioButton(getString("prefs.reportorder"), settings.getProperty("EMapOverviewPanel.sortUnitsCriteria", "skills").equals("unsorted"));
			...
			rdbSortUnitsSkills = new JRadioButton(getString("prefs.sortbyskills"), settings.getProperty("EMapOverviewPanel.sortUnitsCriteria", "skills").equals("skills"));
			...
			boolean temp = settings.getProperty("EMapOverviewPanel.useBestSkill", "true").equalsIgnoreCase("true");
			...
			rdbSortUnitsNames = new JRadioButton(getString("prefs.sortbynames"), settings.getProperty("EMapOverviewPanel.sortUnitsCriteria", "skills").equals("names"));
			...
			pnlUnitSort.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("prefs.unitsorting")));
			...
			help.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("prefs.expand.title")));
		...
		public void applyPreferences() {
			settings.setProperty("EMapOverviewPanel.sortRegions", (new Boolean(chkSortRegions.isSelected())).toString());
			...
			settings.setProperty("EMapOverviewPanel.displayIslands", (new Boolean(chkDisplayIslands.isSelected())).toString());
		...
		protected class ExpandPanel extends JPanel implements ActionListener {
			...
			public ExpandPanel() {
				...
				radioButtons[0] = new JRadioButton(getString("prefs.expand.none"), !expanded);
				...
				radioButtons[1] = new JRadioButton(getString("prefs.expand.faction"), (expanded && ((expandMode >> 2) == 0)));
				group.add(radioButtons[1]);
				con.gridy++;
				this.add(radioButtons[1], con);

				radioButtons[2] = new JRadioButton(getString("prefs.expand.full"), (expanded && ((expandMode >> 2) == 3)));
				...
				con.gridy++;
				checkBox = new JCheckBox(getString("prefs.expand.ifinside"), (expandMode & EXPAND_IFINSIDE_FLAG) != 0);
		...
		protected class CollapsePanel extends JPanel implements ActionListener {
			...
			public CollapsePanel() {
				...
				radioButtons[0] = new JRadioButton(getString("prefs.collapse.none"), !collapse);
				group.add(radioButtons[0]);
				this.add(radioButtons[0], con);
				...
				radioButtons[1] = new JRadioButton(getString("prefs.collapse.faction"), (collapse && ((collapseMode >> 2) == 0)));
				group.add(radioButtons[1]);
				con.gridy++;
				this.add(radioButtons[1], con);

				radioButtons[2] = new JRadioButton(getString("prefs.collapse.full"), (collapse && ((collapseMode >> 2) == 3)));
				group.add(radioButtons[2]);
				con.gridy++;
				this.add(radioButtons[2], con);

				con.gridy++;
				checkBox = new JCheckBox(getString("prefs.collapse.onlyautoexpanded"), (collapseMode & COLLAPSE_ONLY_EXPANDED) != 0);
				this.add(checkBox, con);
	...
	private class UnitWrapperUpdater implements UnitOrdersListener {
		...
		Collection buf[] = {CollectionFactory.createLinkedList(), CollectionFactory.createLinkedList()};
	...
	class TreeBuilder {
		...
		private Map regionNodes, unitNodes, buildingNodes, shipNodes, activeAlliances;
		...
		public void buildTree(DefaultMutableTreeNode rootNode, Collection regionCollection, Collection units, Map regionNodes, Map unitNodes, Map buildingNodes, Map shipNodes, Comparator unitSorting, Map activeAlliances, int treeStructure[], GameData data) {
			...
			while (regions.hasNext()) {
				...
				if (!((unitInteresting && !r.units().isEmpty()) || (buildingInteresting && !r.buildings().isEmpty())
					|| (shipInteresting && !r.ships().isEmpty()) || (commentInteresting && !(r.comments == null || r.comments.size() == 0)))) {
					continue;
				}
			
				// add region node to tree an node map
				regionNode = (DefaultMutableTreeNode)TreeHelper.createRegionNode(r, nodeWrapperFactory, activeAlliances, unitNodes, buildingNodes, shipNodes, unitSorting, treeStructure, data);
