...
public class ArmyStatsDialog extends InternationalizedDataDialog implements ActionListener {
	...
	protected List excludeSkills, excludeNames, excludeCombatStates;
	...
	public ArmyStatsDialog(Frame owner, EventDispatcher ed, GameData data, Properties settings, Collection selRegions) {
		super(owner, false, ed, data, settings);
		
		LayoutManager lm = new FlowLayout(FlowLayout.CENTER);
		
		JPanel help = new JPanel(lm);
		help.add(categorize = new JCheckBox(getString("categorized"), settings.getProperty("ArmyStatsDialog.Categorize","true").equals("true")));
		...
		inner.add(panel = new ArmyStatsPanel(ed,data,settings, categorize.isSelected(), selRegions), BorderLayout.CENTER);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == categorize) {
			...
			if (data!=null) {
				ExcludeDialog ed = new ExcludeDialog(this, getString("exclude.title"), data, excludeNames, excludeSkills, excludeCombatStates);
				ed.show();
	...
	protected class ExcludeDialog extends JDialog implements ActionListener {
		...
		public ExcludeDialog(Dialog owner, String title, GameData data, List eNames, List eSkills, List eStates) {
