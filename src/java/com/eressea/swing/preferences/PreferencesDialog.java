 ...
 * <p>This class has a kind of plug-in mechanism, that allows
 * displaying a dynamic set of panels as tab panes without
 * changing the implementation.</p>
 * <p>In order to use this feature, these are the preconditions:
 * <ul><li>Make all preferences of your actual UI component (a
 * subclass of DataPanel or something similar, e.g. the
 * EMapDetailsPanel) publicly accessible (usually via get/set
 * methods).</li>
 * <li>Write a container that makes these preferences editable
 * (nice checkboxes and so on).</li>
 * <li>That container must, upon creation, reflect the current
 * preferences of your UI component.</li>
 * <li>That container also has to provide a means of applying the
 * changes made by the user to your UI component.</li>
 * <li>Create a class implementing the PreferencesAdapter interface,
 * which holds a reference to the container. It is responsible for
 * passing the your container to this PreferencesDialog and making
 * it 'apply' the changes made by the user to your UI component.</li>
 * <li>Finally, plug in your container into this PreferencesDialog
 * object by calling the addTab() method.</li></ul>
 * <p>Note that such a preferences container has to resize with its
 * parent component, i.e. the preferences dialog</p>
 */
public class PreferencesDialog extends InternationalizedDialog {
	...
	/**
	 * Creates a modal or non-modal dialog with the specified
	 * title and the specified owner frame.
	 *
	 * @param owner the frame from which the dialog is displayed.
	 * @param title the String to display in the dialog's title
	 * 		bar.
	 * @param modal true for a modal dialog, false for one that
	 * 		allows others windows to be active at the same
	 * 		time.
	 */
	public PreferencesDialog(Frame owner, boolean modal, Properties settings) {
		...
		int x = Integer.parseInt(settings.getProperty("PreferencesDialog.x", ((screen.width - width) / 2) + ""));
		int y = Integer.parseInt(settings.getProperty("PreferencesDialog.y", ((screen.height - height) / 2) + ""));
		setLocation(x, y);
	}
	...
	/**
	 * Register a new container with this dialog. It will be put
	 * into its own tab and notified via the apply() method, if
	 * the user confirmed the presumably modified preferences.
	 */
	...
	protected class DialogTree extends JPanel implements TreeSelectionListener {
		...
		protected DefaultMutableTreeNode addAdapterImpl(PreferencesAdapter pa) {
			...
			if (pa instanceof ExtendedPreferencesAdapter) {
				for(Iterator iter = ((ExtendedPreferencesAdapter) pa).getChildren().iterator(); iter.hasNext(); ) {
		...
		public void valueChanged(javax.swing.event.TreeSelectionEvent treeSelectionEvent) {
			...
			if (selection.length > 0) {
				settings.setProperty("PreferencesDialog.DialogTree.SelectedRow", String.valueOf(selection[0]));
			}
			PreferencesInfo pref = (PreferencesInfo)((DefaultMutableTreeNode)tree.getSelectionPath().getLastPathComponent()).getUserObject();
			if (pref != null) {
