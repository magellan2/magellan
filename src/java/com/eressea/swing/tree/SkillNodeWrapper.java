...
public class SkillNodeWrapper implements CellObject2, SupportsClipboard {
	...
	public final static int SHOW_NEXTLEVEL = 0;
	public final static int SHOW_NEXTLEVELPOINTS = 1;
	public final static int SHOW_NEXTLEVELTURNS = 2;
	public final static int SHOW_CHANGES = 3;
	public final static int SHOW_CHANGE_STYLED = 4;
	public final static int SHOW_CHANGE_TEXT = 5;
	 ...
	 * @param s the base skill. If s is null, it is assumed that the
	 * unit aquires that skill only through a person transfer. s and
	 * ms may not both be null.
	 * @param ms the modified skill. If ms is null, it is assumed that
	 * the modification of the skill cannot be determined. s and
	 * ms may not both be null.
	 */
	...
	public String toString() {
		if (text == null) {
			...
			if (!skill.noSkillPoints()) {
				if (modSkill != null) {
					if (skill.getPoints() == modSkill.getPoints() && unit.persons == unit.getModifiedPersons()) {
						...
						sb.append(" (").append(modSkill.getLevel()).append(" [").append(modSkill.getPointsPerPerson()).append("])");
					}
				} else {
					sb.append(" [").append(skill.getPointsPerPerson()).append("]");
					sb.append(" (? [?])");
	...
	/**
	 * Controls whether this wrapper shows the skill points required
	 * before the next skill level can be reached.
	 */
	...
	/**
	 * Returns whether this wrapper shows the skill points required
	 * before the next skill level can be reached.
	 */
	...
	/**
	 * Controls whether this wrapper shows the number of turns
	 * to learn before the next skill level can be reached.
	 */
	...
	/**
	 * Returns whether this wrapper shows the number of turns
	 * to learn before the next skill level can be reached.
	 */
	...
	static {
		...
		defaultTranslations.put("prefs.turns.text" , "Anzahl der Runden bis zur nächsten Stufe anzeigen"); // TODO: why german here?
	...
	public List getGraphicsElements() {
		if (GEs == null) {
			...
			if (isDiff && isShowingChanges() && isShowingChangesStyled()) {
				ge.setStyleset(SKILL_CHANGE_STYLE_PREFIX+(skill.getChangeLevel()>=0?">.":"<.")+SKILL_CHANGE_STYLE_PREFIX+String.valueOf(skill.getChangeLevel()));
			}
	...
	private NodeWrapperDrawPolicy createSkillDrawPolicy(Properties settings, String prefix) {
		return new DetailsNodeWrapperDrawPolicy(2, new int[] {2, 2}, settings, prefix, new String[][] {
