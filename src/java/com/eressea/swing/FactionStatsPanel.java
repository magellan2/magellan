...
public class FactionStatsPanel extends InternationalizedDataPanel implements SelectionListener, TreeSelectionListener {
	private final static Logger log = Logger.getInstance(FactionStatsPanel.class);
	...
	public void selectionChanged(SelectionEvent e) {
		...
		if (e.getSelectedObjects() != null && e.getSelectionType() == SelectionEvent.ST_REGIONS) {
			...
			/**
			 * Ulrich Küster:
			 * (!) Special care has to be taken. Generelly it can not be differed, if
			 * SelectionEvents come from the faction list in FactionStatsDialog
			 * or from other components of Magellan (except if a special SelectionType
			 * has been defined in SelectionEvent and is used).
			 * To keep the faction list in FactionStatsDialog consistent to the
			 * displayed data in this FactionStatsPanel object, setFaction()
			 * should be _never_ called by this selectionChanged()-method, but
			 * directly by the valueChanged()-method of FactionStatsDialog.
			 */
			setRegions(regions);
		}
	}
	...
	/**
	 * Ulrich Küster:
	 * The algorithm wasn't correct as it is wrong to count the number of persons
	 * in the temp units to get the number of recruited persons.
	 * a) Temp units always have zero persons
	 * b) 'normal' units can recruit persons
	 * So it is necessary to look at all unit's RecruitmentRelations in the
	 * units's cache-object. I changed that.
	 * (Of course this doesn't consider, that persons can be given to '0'.)
	 */
	private void updateTree() {
		...
		/**
		Used to collect persons of different race than their faction.
		Key: String (racename), Value: List containing the units
		*/
		...
		if (f != null) {
			...
			if (f.email != null) {
				n = new DefaultMutableTreeNode(new SimpleNodeWrapper(getString("node.e-mail") + f.email, null, f.email));
				rootNode.add(n);
			}
			...
			if (f.score > 0) {
				n = new DefaultMutableTreeNode(getString("node.score") + f.score + "/" + f.averageScore + " (" + (int)(100.0 * f.score / f.averageScore) + "%)");
				rootNode.add(n);
			}

			if (f.migrants > 0) {
				n = new DefaultMutableTreeNode(getString("node.migrants") + f.migrants + "/" + f.maxMigrants);
				rootNode.add(n);
			}

			if (f.getRaceNamePrefix() != null) {
				n = new DefaultMutableTreeNode(getString("node.racenameprefix") + f.getRaceNamePrefix());
				rootNode.add(n);
			}
			...
			if (f.groups != null && f.groups.size() > 0) {
				...
				for (Iterator iter = f.groups.values().iterator(); iter.hasNext(); ) {
					...
					if (g.getRaceNamePrefix() != null) {
						m.add(new DefaultMutableTreeNode(getString("node.racenameprefix") + g.getRaceNamePrefix()));
					}
			...
			for (Iterator iter = r.buildings().iterator(); iter.hasNext(); ) {
				...
				if (building.getOwnerUnit() != null && factions.containsKey(building.getOwnerUnit().getFaction().getID())) {
					if (!buildingsCounter.containsKey(building.getType())) {
						buildingsCounter.put(building.getType(), CollectionFactory.createLinkedList());
					}
		...
		for (Iterator iter = buildingsCounter.keySet().iterator(); iter.hasNext(); ) {
			...
			m = new DefaultMutableTreeNode(buildingType.getName() + ": " + ((List)buildingsCounter.get(buildingType)).size());
			n.add(m);
		...
		for (Iterator iter = shipsCounter.keySet().iterator(); iter.hasNext(); ) {
			...
			m = new DefaultMutableTreeNode(shipType.getName() + ": " + ((List)shipsCounter.get(shipType)).size());
			n.add(m);
		...
		if (sortedSkillTypes.size() > 0) {
			...
			for (Iterator iter = sortedSkillTypes.iterator(); iter.hasNext(); ) {
				...
				for (Iterator i = sortedSkills.iterator(); i.hasNext(); ) {
					...
					Comparator unitCmp = new UnitSkillComparator(new SpecifiedSkillTypeSkillComparator(type, new SkillComparator(), null), idCmp);
					...
					for (Iterator iterator = unitList.iterator(); iterator.hasNext(); ) {
						...
						if (!data.noSkillPoints) {
							...
							text += ": " + " [" + currentDays + " -> " + nextLevelDays + " {" + turnsToLearn + "}], " + u.persons;
						}
