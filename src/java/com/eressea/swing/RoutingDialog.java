 ...
 * Out of the knowledge of this destination region magellan
 * can create respective orders that move the ship to that region.
 * It is also possible, to determine, if ship range shall be considered,
 * whether Vorlage-meta-orders shall be created if the destination region
 * can not be reached in one week and if the captain's orders shall be replaced.
 * @return a List which first element is a Coordinate object with the
 * coordinates of the destination region (be careful, no z-coor is specified),
 * second a Boolean, that determines, whether a permanent route shall be constructed,
 * third a Boolean, that determines, whether ship range shall be considered,
 * fourth a Boolean, that determines, whether Vorlage-meta-orders shall be created and
 * fifth a Boolean, that determines whether the captain's orders shall be replaced.
...
public class RoutingDialog extends InternationalizedDialog {
	...
	private JRadioButton createRoute, createSingleTrip;
	private JCheckBox considerShipRange, createVorlageOrders, replaceOrders;
	private List regionList;
	private JComboBox regions;
	private JTextField regionName;
	private JTextField xCor, yCor;
	...
	public RoutingDialog(Frame owner, GameData data, Collection destRegions, boolean excludeUnnamed) {
		...
		GridBagConstraints c = new GridBagConstraints(0, 0, 4, 1, 0, 0.2, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(1, 3, 1, 3), 0, 0);
		...
		if (destRegions != null) {
			...
			regionName.getDocument().addDocumentListener(new DocumentListener() {
				public void insertUpdate(DocumentEvent e) {
					int i = Collections.binarySearch(regionList, regionName.getText(), new RegionNameComparator());
					if (i < 0) {
						i = -i - 1;
					}
	...
	public class RetValue {
		...
		public boolean makeRoute, useRange, useVorlage, replaceOrders;
