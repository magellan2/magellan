...
public class TipOfTheDay extends InternationalizedDialog implements ActionListener {
	...
	public final static String HTML_START ="<html><body style=\"margin-left: 40px;margin-top: 15px\"><font color=#4f3f30><b>";
	public final static String HTML_END ="</b></font></body></html>";
	
	public final static String E_HTML_START ="<html><body style=\"margin-left: 40px;margin-top: 2px\"><font color=#4f3f30><b>";
	public final static String E_HTML_END ="</b></font></body></html>";
	public final static String E_KEY = "ETIP";
	...
	protected void initUI() {
		...
		showTips = new JCheckBox(getString("showTips"), settings.getProperty("TipOfTheDay.showTips", "true").equals("true"));
		...
		GridBagConstraints c = new GridBagConstraints(0,0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(1,1,1,1), 0, 0);
		...
		if (hasTips() || firstTime) {
			...
			try {
				...
				if (tip.text.startsWith(E_KEY)) {
					kit.insertHTML(doc2, 0, E_HTML_START+"<font size=+1>"+getString("tip.eressea")+"</font><br>"+tip.text.substring(E_KEY.length())+E_HTML_END, 0, 0, null);
				} else {
	...
	public synchronized static Map getDefaultTranslations() {
		if(defaultTranslations == null) {
			...
			defaultTranslations.put("tip.10" , "The fastest way to load the last report is the key combo <u><i>CTRL-o ALT-o</i></u>.");
			defaultTranslations.put("tip.9" , "ETIPYou can directly write to other units, even faction disguised ones.  With an E-Mail to <i><u>einheit-NUMBER@eressea.kn-bremen.de</u></i> you send a message to the chieftain of the faction the unit belongs to, i.e.: <u><i>einheit-wegi@eressea.kn-bremen.de</i></u>");
			defaultTranslations.put("tip.8" , "Bugs or Feature-Requests can be told at <i>http://eressea.upb.de/magellan/bugs</i>.");
			defaultTranslations.put("tip.7" , "To always have known spells and potions in your report you need to add the report of the last week to the current report and save them together.");
			defaultTranslations.put("tip.6" , "If you want to have a look on the changes of a week just add the report of an older week with <u><i>Add...</i></u> to your actual report.");
			defaultTranslations.put("tip.5" , "If you choose <i>\"Skip units with unconfirmed orders\"</i> at the order save dialog, you can send orders for only a part of the units. The game server will use only orders of confirmed units, the orders of other units will remain.");
			defaultTranslations.put("tip.4" , "Magellan is able to send your orders per Mail. You can find this ability under \"Files - Save orders\" by choosing \"E-Mail\".");
			defaultTranslations.put("tip.3" , "If you altered the Magellan options but do not want to save them during quitting you can choose \"Abort\" from the Files menu.");
			defaultTranslations.put("tip.2" , "If there are too many icons for skills and items in the region overview it may happen that you cannot see the unit's name. In this case you may turn of the icons by choosing the appropriate icon view state from the 'Tree - Regions' menu. You can turn them on or off there.");
			defaultTranslations.put("tip.1" , "Magellan is able to open your report out of a zip archive. You just have to choose <i>Zip</i> from the file-type box at the open dialog.");
			defaultTranslations.put("tip.0" , "To mark regions on the map simply hold the <i><u>Shift<u><i>-Key and click at the region(s).");
			...
			defaultTranslations.put("warningString","<font size=+2>Important!</font><p>Magellan has some features that are only prognostic. Use them with care for Magellan is not the Eressea Server. The developers are not responsible for any damage on your faction resulting from these features.</p><p>For more information please consult the Magellan help.");
		}
