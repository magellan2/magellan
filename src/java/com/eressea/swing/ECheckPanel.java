...
public class ECheckPanel extends InternationalizedDataPanel implements SelectionListener {
	private final static Logger log = Logger.getInstance(ECheckPanel.class);
	...
	public void runECheck() {
		...
		if (selectedFaction == null) {
			JOptionPane.showMessageDialog(this, getString("msg.nofaction.text"), getString("msg.nofaction.title"), JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		if (!exeFile.exists()) {
			JOptionPane.showMessageDialog(this, getString("msg.echeckmissing.text"), getString("msg.echeckmissing.title"), JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		try {
			if (!JECheck.checkVersion(exeFile)) {
				...
				Object[] msgArgs = {
					JECheck.getRequiredVersion()
				};
				JOptionPane.showMessageDialog(this, java.text.MessageFormat.format(getString("msg.wrongversion.text"), msgArgs), getString("msg.wrongversion.title"), JOptionPane.ERROR_MESSAGE);
				return;
			}
		} catch (IOException e) {
			this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			JOptionPane.showMessageDialog(this, getString("msg.versionretrievalerror.text"), getString("msg.versionretrievalerror.title"), JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		try {
			...
			this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			JOptionPane.showMessageDialog(this, getString("msg.ordersaveerror.text") + e, getString("msg.ordersaveerror.title"), JOptionPane.ERROR_MESSAGE);
			if (orderFile != null) {
				orderFile.delete();
				orderFile = null;
			}
		...
		try {
			List messages = CollectionFactory.createLinkedList(JECheck.getMessages(exeFile, orderFile, options));
			if (messages.size() > 0) {
				JECheck.determineAffectedObjects(data, orderFile, messages);
				lstMessages.setListData(messages.toArray());
			} else {
				JOptionPane.showMessageDialog(this, getString("msg.noecheckmessages.text"), getString("msg.noecheckmessages.title"), JOptionPane.INFORMATION_MESSAGE);
			}
		} catch (IOException e) {
			this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			JOptionPane.showMessageDialog(this, getString("msg.echeckerror.text") + e, getString("msg.echeckerror.title"), JOptionPane.ERROR_MESSAGE);
			if (orderFile != null) {
				orderFile.delete();
				orderFile = null;
			}
			...
			this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			JOptionPane.showMessageDialog(this, getString("msg.parseerror.text") + e, getString("msg.parseerror.title"), JOptionPane.ERROR_MESSAGE);
			if (orderFile != null) {
				orderFile.delete();
				orderFile = null;
			}
	...
	public synchronized static Map getDefaultTranslations() {
		if(defaultTranslations == null) {
			...
			defaultTranslations.put("msg.echeckmissing.text" , "The specified ECheck executable does not exist!");
			defaultTranslations.put("msg.echeckmissing.title" , "Error");
			defaultTranslations.put("msg.nofaction.text" , "Please select a faction whose orders should be checked.");
			defaultTranslations.put("msg.nofaction.title" , "Select faction");
			defaultTranslations.put("msg.wrongversion.title" , "ECheck too old");
			defaultTranslations.put("msg.wrongversion.text" , "The specified version of ECheck is too old. Please use version {0} or higher.");
			defaultTranslations.put("msg.versionretrievalerror.title" , "Version undetermined");
			defaultTranslations.put("msg.versionretrievalerror.text" , "While trying to gather version information about the specified ECheck the following error occurred:\n");
			defaultTranslations.put("msg.ordersaveerror.text" , "The following error occurred while saving the faction's orders: ");
			defaultTranslations.put("msg.ordersaveerror.title" , "Error on save");
			defaultTranslations.put("msg.noecheckmessages.text" , "ECheck did not find any errors in your orders.");
			defaultTranslations.put("msg.noecheckmessages.title" , "Orders checked");
			defaultTranslations.put("msg.echeckerror.text" , "The following error occurred while executing ECheck:  ");
			defaultTranslations.put("msg.echeckerror.title" , "ECheck error");
			defaultTranslations.put("msg.parseerror.text" , "Unable to parse the messages generated by ECheck: ");
			defaultTranslations.put("msg.parseerror.title" , "ECheck error");
			defaultTranslations.put("msg.messagetargetnotfound.text" , "Unable to determine the unit or region the selected message refers to.");
	...
	private Container getMessagesPanel() {
			...
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) {
					...
					if (msg.getAffectedObject() != null) {
						dispatcher.fire(new SelectionEvent(ECheckPanel.this, null, msg.getAffectedObject()));
					} else {
						JOptionPane.showMessageDialog(ECheckPanel.this, getString("msg.messagetargetnotfound.text"), getString("msg.messagetargetnotfound.title"), JOptionPane.INFORMATION_MESSAGE);
					}
				}
			}
		});
		...
		messages.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("border.echeckmessages")));
	...
	private Container getOutputPanel() {
		...
		orders.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("border.orders")));
	...
	/**
	 * Returns a file name by showing the user a file
	 * selection dialog.
	...
	private final static class ECheckMessageRenderer extends JPanel implements ListCellRenderer {
