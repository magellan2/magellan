 ...
 * This class takes care of dynamic loading and proper scaling of
 * the images. All images are loaded from the images/map/
 * sub-directory of the current resource bundle.
 */
public abstract class ImageCellRenderer extends HexCellRenderer {
	private final static Logger log = Logger.getInstance(ImageCellRenderer.class);
	private Map images = new HashMap();
	private static MediaTracker tracker = null;
	
	/**
	 * Creates a new ImageCellRenderer with the specified cell
	 * geometry and a Properties object to read the render
	 * settings from.
	 */
	 ...
	 * 
	 * @param scaleFactor the factor to scale the images with (a
	 * scaleFactor of 1.0 would scale all images to their original
	 * size).
	 */
	...
	/**
	 * Return a scaled version of the supplied using the current
	 * scale factor. If there is no media tracker, this function
	 * enforces synchronous scaling.
	 * 
	...
	/**
	 * Set the cell geometry this renderer is based on and make it
	 * reload all of its cached images.
	 */
	...
	/**
	 * Set a media tracker that is used to track all images that are
	 * scaled. If no media tracker is present scaling is synchronous.
	 */
	...
	/**
	 * Load an image by file name. This procedure tries different
	 * file formats in the following order: .png, if not found then
	 * .gif. If a .gif file is available, an optional -alpha.gif file
	 * is used for alpha-channel information. If no such -alpha.gif
	 * file can be found, the optional alpha information in the .gif
	 * file is used. If no such file seems to exist, null is returned.
	 * All file names are prepended with the path 'images/map/'
	 * enforcing that the files are located in such a sub-directory
	 * of the resources root directory /res.
	 *
	 * @param fileName a file name without extension.
	 * @return the image loaded from fileName, or null if not file
	 * could be found.
	 */
	protected Image loadFile(String fileName) {
		String file = "images/map/" + fileName.toLowerCase();
		Image imgAlpha = null, imgRGB = null, img = null;
		java.net.URL pngURL = null, gifURL = null, alphaURL = null;
	...
	/**
	 * Combine two images of equal size to one, where the resulting
	 * image contains the RGB information of the first image directly
	 * and the RGB information of the second one as alpha channel
	 * information.
	 *
	 * @param rgb the image to take rgb information from.
	 * @param alpha the image to take the alpha channel information
	 * from.
	 * @return the composite image, or null if rgb or alpha were null
	 * or they were not of equal size.
	 */
	protected Image merge(Image rgb, Image alpha) {
		...
		if ((pgRGB.getStatus() & ImageObserver.ABORT) != 0 || (pgAlpha.getStatus() & ImageObserver.ABORT) != 0) {
		...
		result = Toolkit.getDefaultToolkit().createImage(new MemoryImageSource(w, h, pixelsRGB, 0, w));
				
		return result;
	}
	
	/**
	 * Returns an image that is associated with name. If name has
	 * never been supplied to this function before, it attempts to
	 * load an image with a file name of name. If no such file
	 * exists, there will be no further attempts to load the file
	 * when this function is called with the same value for name.
	 * @param name a name identifying the image to get. This name is
	 * also used as a file name without extension to load the image
	 * from a file.
	 * @return the image associated with name or null, if there is no
	 * such image and it cannot be loaded.
	 */
