...
public abstract class AbstractTextCellRenderer extends HexCellRenderer{
	...
	protected void setFont(float scaleFactor) {
		font = unscaledFont.deriveFont(Math.max(unscaledFont.getSize() * scaleFactor, (float)minimumFontSize));
	...
	protected FontMetrics getFontMetrics() {
		if (fontMetrics == null) {
			fontMetrics = Toolkit.getDefaultToolkit().getFontMetrics(new Font("TimesRoman", Font.PLAIN, 10));
	...
	public void render(Object obj, boolean active, boolean selected) {
		if (obj instanceof Region) {
			...
			int middleX=rect.x+(rect.width/2)-offset.x,middleY=rect.y+(rect.height/2)+1-offset.y;
			int upperY=middleY;
			if (display.length == 1) {
				upperY += height/4;
			} else {
			...
			switch(hAlign) {
				// left
				case 0:	for(int i=0;i<display.length;i++)
				...
				break;
				// center
				case 1: for(int i=0;i<display.length;i++) {
					int l=fontMetrics.stringWidth(display[i]);
					graphics.drawString(display[i],middleX-(l/2),upperY+i*height);
				}
				break;
				// right
				case 2:	for(int i=0;i<display.length;i++)
					if (fontMetrics.stringWidth(display[i])>maxWidth)
						maxWidth=fontMetrics.stringWidth(display[i]);
				int rightX=middleX+(maxWidth/2);
				for(int i=0;i<display.length;i++)
					graphics.drawString(display[i],rightX-fontMetrics.stringWidth(display[i]),upperY+i*height);
				break;
			}
		}
	}
