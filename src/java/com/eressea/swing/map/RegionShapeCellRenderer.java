 ...
 * Regiontype-Mode, the Politics-Mode and the All-Factions-Mode. In Regiontype-Mode a region is rendered
 * with a color based on its type while in Politics Mode the color is based on the
 * faction reigning over the region. In All-Factions-Mode the colors of all factions in
 * this region are used.
 */
public class RegionShapeCellRenderer extends AbstractRegionShapeCellRenderer implements Initializable, ContextChangeable, GameDataListener{
	private final static Logger log = Logger.getInstance(RegionShapeCellRenderer.class);

	public final static String MAP_TAG = "mapcolor";
	
	/**
	 * The default value for factionKey.
	 */
	public final static String DEFAULT_FACTION_KEY="GeomRenderer.FactionColors";
	/**
	 * The default value for regionKey.
	 */
	public final static String DEFAULT_REGION_KEY="GeomRenderer.RegionColors";
	/**
	 * The default value for paintKey.
	 */
	public final static String DEFAULT_PAINTMODE_KEY="GeomRenderer.paintMode";
	
	/**
	 * Defines the RegionType paint mode.
	 */
	public final static int PAINT_REGIONTYPE=0;
	/**
	 * Defines the Politics paint mode.
	 */
	public final static int PAINT_POLITICS=1;
	/**
	 * Defines the All Factions paint mode.
	 */
	public final static int PAINT_ALLFACTIONS=2;
	/**
	 * Defines the Trust Level OVERALL paint mode.
	 */
	public final static int PAINT_TRUST_OVERALL=3;
	/**
	 * Defines the Trust Level GUARD paint mode.
	 */
	public final static int PAINT_TRUST_GUARD=4;
	...
	/**
	 * All regions which can't be assigned to a faction are rendered
	 * with this color.
	 */
	protected Color unknownColor;
	
	/**
	 * Stores the region - color paires. The key is the region type
	 * name	(RegionType.getName()).
	 */
	...
	/**
	 * The default key for settings operations with the faction-color
	 * table.
	 */
	protected String factionKey;
	/**
	 * The default key for settings operations with the region-color
	 * table.
	 */
	protected String regionKey;
	/**
	 * The default key for settings operations with the Politics Mode
	 * variable.
	 */
	 ...
	 * The default settings-operations keys are initialized to:
	 * <ul>
	...
	/**
	 * Constructs a new RegionShapeCellRenderer object using the given geometry and settings.
	 * The given Strings fKey, rKey and pKey are used to initialize the default
	 * settings-operations keys.
	 * @param geo The CellGeometry to be used
	 * @param settings The Properties to be used
	 ...
	 */
	public RegionShapeCellRenderer(CellGeometry geo, java.util.Properties settings,String fKey,String rKey,String pKey) {
		super(geo, settings);
		...
		// initialize the PreferencesAdapter
		/*if (adapter==null)
			adapter=new GeomRendererAdapter();*/
		
		// load politics settings and implicitly update the pref-adapter card
		try{
			setPaintMode(Integer.parseInt(settings.getProperty(paintKey)));
		}catch(Exception exc) {setPaintMode(0);}
	 ...
	 * <ul>
	 * <li>Ebene</li><li>Sumpf</li><li>Wald</li><li>Wüste</li>
	...
	/**
	 * Sets the Paint Mode of this renderer to pm. The new setting
	 * is stored with the paintKey in the global settings.
	 * @param pm The new mode
	 */
	...
	/**
	 * Puts a whole map to the faction-colors Table. Keys of this
	 * map should be factions or strings.
	 * @param map A map containing Faction/String - Color pairs
	 */
	public void setFactionColors(Map map) {
	...
	/**
	 * Puts a whole map to the region-colors Table. Keys should be
	 * region-types or strings.
	...
	/**
	 * Sets the color used for ocean fields in Politics Mode and stores it
	...
	/** Returns the default key for settings operations concerning the faction-colors
	 * table.
	...
	/** Returns the default key for settings operations concerning the region-colors
	 * table.
	...
	/** Returns the default key for settings operations concerning the mode of this
	 * renderer.
	 ...
	 * dest. If the reset flag is set, the hashtable is cleared before parsing.
	...
	protected void load(Map dest,String key,boolean reset, boolean makeID) {
		...
		try{
			while(st.hasMoreTokens()) {
				...
				String red=st.nextToken(),green=st.nextToken(),blue=st.nextToken();
		...
		}catch(Exception exc) {} // maybe not the right count
	}
	...
	/**
	 * Loads the faction colors saved with the given key. If the
	 * reset flag is set, the color-table is cleared before loading.
	...
	/**
	 * Loads the region colors saved with the given key. If the
	 * reset flag is set, the color-table is cleared before loading.
	...
	/**
	 * Fills the cell polygon if in TrustLevel mode. Fallback is
	 * Politics mode. After that left side is filled with lowest
	 * over all trustlevel, right side with lowest trustlevel of
	 * guarding units.
	 */
	...
	/**
	 * Returns the color of a region type. If the type can't be found
	 * in the region-color table, a new random color entry is put.
	...
	protected Color getRegionTypeColor(RegionType type) {
		...
		if (!regionColors.containsKey(type.getID())) {
			Color c=new Color((int)(Math.random()*128)+128,(int)(Math.random()*128)+128,(int)(Math.random()*128)+128);
	 ...
	 * region. The current implementation first looks if the type of this region
	 * is "Ozean" in which case the ocean color is used. Then it checks for an
	 * explicitly set owner unit. Normally the owner of the biggest "Burg" is
	 * returned. If there's no "Burg", the people of the different factions
	 * are counted and the faction with most people "wins". If this algorithm
	 * does not supply a faction, the unknown color is used.
	...
	/**
	 * Returns the color for faction f. If there's no entry in the faction-color
	 * table a new random-color entry is created.
	...
	/**
	 * Returns the color for faction with name f. If there's no entry in the faction-color
	 * table a new random-color entry is created.
	 * @param f the faction for which a color is needed
	 * @return the color for the given faction
	 */
	...
	/**
	 * Returns the color for faction f. If there's no entry in the faction-color
	 * table the entry is created with given colr.
	 * @param f the faction for which a color is needed
	...
	/**
	 * Returns the color for faction with name f. If there's no entry in the faction-color
	 * table the entry is created with given color.
	 * @param f the faction for which a color is needed
	 * @param c the default color
	 * @return the color for the given faction
	 */
	...
	protected void initContextMenu() {
		...
		JMenuItem item = new JMenuItem(getString("cmb.mode.terrain"));
		item.addActionListener(al);
		item.setActionCommand("0");
		contextMenu.add(item);
		
		item = new JMenuItem(getString("cmb.mode.byfaction"));
		item.addActionListener(al);
		item.setActionCommand("1");
		contextMenu.add(item);
		
		item = new JMenuItem(getString("cmb.mode.allfactions"));
		item.addActionListener(al);
		item.setActionCommand("2");
		contextMenu.add(item);
		
		item = new JMenuItem(getString("cmb.mode.trustlevel"));
		item.addActionListener(al);
		item.setActionCommand("3");
		contextMenu.add(item);
		
		item = new JMenuItem(getString("cmb.mode.trustlevelandguard"));
		item.addActionListener(al);
	...
	public void gameDataChanged(GameDataEvent e) {
		...
		while (it.hasNext()) {
			...
			if (f.trustLevel <= Faction.TL_DEFAULT) {
	...
	static {
		...
		defaultTranslations.put("chk.compare.name" , "List by name");
		defaultTranslations.put("chk.compare.trust" , "List by trust");
	...
	/** A PreferencesAdapter for use with RegionShapeCellRenderer objects. It supplies
	 * two cards for both RegionType and Politics Mode. The cards are initialized at
	 * creation time. Updates are performed through calls of addColor(). Changes are
	 * applied directly.
	 */
	protected class GeomRendererAdapter extends JPanel implements PreferencesAdapter,ActionListener {
		...
		private class ModePanel extends JPanel implements ActionListener{
			...
			private class ListElementFactionTrustComparator implements Comparator {
				public int compare(Object o1, Object o2) {
					String name1 = ((ListElement)o1).name, name2=((ListElement)o2).name;
					if (name1 == null) {
						name1 = "null";
					}
					if (name2 == null) {
					...
					if (data != null) {
						...
						if (f1 == null) {
							...
							if (f1.trustLevel == f2.trustLevel) {
								return nameComp.compare(o1, o2);
							}
							return f2.trustLevel - f1.trustLevel;
						}							
					}
					return nameComp.compare(o1, o2);
				}
			...
			/**
			 * Renders the list. Prefers ListElement values were the color is used
			 * for a color icon.
			 */
			...
			private class SortableListModel extends AbstractListModel {
				...
				public void sort(Comparator comp, int offset) {
					...
					try{
						...
						if (offset > 0) {
							front = CollectionFactory.createLinkedList();
			...
			private String oceanLabel, unknownLabel;
			
			private Comparator nameComp, trustComp;
			...
			protected void actionPerformed(int index) {
				...
				if (oceanLabel.equals(element.name)) {
					...
					title = (new java.text.MessageFormat(getString("dialog.color.title"))).format(msgArgs);
				}
				...
				if (c!=null) {
					...
					switch(colMode) {
						...
						case 1: setOceanColor(c);	break;
						case 2: setUnknownColor(c); break;
					}
				}				
			}
		...
		public GeomRendererAdapter() {
			...
			inner.setBorder(new javax.swing.border.TitledBorder(BorderFactory.createEtchedBorder(),getString("border.colortable")));
			...
			items[0]=getString("cmb.mode.terrain");
			items[1]=getString("cmb.mode.byfaction");
			...
			JPanel p=new JPanel();
			p.add(new JLabel(getString("lbl.rendermode.caption")));
			p.add(modeBox);
		...
		/** An empty implementation of the PreferencesAdapter interface. Changes are
		 * applied directly.
		 */
		...
		public void actionPerformed(java.awt.event.ActionEvent p1) {
			...
			switch(s) {
				case 0: card.first(inner);break;
				case 1:
				case 2:
				case 3:
				case 4: card.last(inner);break;
			}
			setPaintMode(s);
		}
