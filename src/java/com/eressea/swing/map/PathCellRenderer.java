...
public class PathCellRenderer extends ImageCellRenderer {
	private final static Logger log = Logger.getInstance(PathCellRenderer.class);

	private final static int ACTIVE      = 0;
	private final static int PASSIVE     = 1;
	private final static int ACTIVEPAST  = 2;
	private final static int PASSIVEPAST = 3;
	...
	public PathCellRenderer(CellGeometry geo, Properties settings) {
		super(geo, settings);
		drawPassivePath = (new Boolean(settings.getProperty("PathCellRenderer.drawPassivePath", "true"))).booleanValue();
	...
	/**
	 * Checks the orders of the specified unit for movement orders and
	 * renders arrows indicating the direction the unit is taking.
	 * Note that the movement orders may not be abbreviated for this
	 * to work.
	 */
	private void render(Unit u) {
		...
		if (activeMovement.size() > 0) {
			...
			if(u.getModifiedShip() != null) {
				...
				if(log.isDebugEnabled()) {
					log.debug("PathCellRenderer.render: "+u+" has "+ carriers.size()+" carriers");
				}
				if(carriers.size() == 1) {
	...
	private final static MessageType transportMessageType = new MessageType(IntegerID.create(891175669));
				...
				if(log.isDebugEnabled()) {
					if(transportMessageType.equals(m.getType())) {
						...
						if(m.attributes != null && m.attributes.get("unit") != null) {
							...
							log.debug("PathCellRenderer("+u+") UnitID "+UnitID.createUnitID((String)m.attributes.get("unit"),10));
						}
					}
				}
			}
	...
	private void renderPath(Unit u, List coordinates, int imageType) {
		if(coordinates != null && coordinates.size()>0) {
			renderPath(u, (Coordinate) coordinates.get(0), Regions.getDirectionObjectsOfCoordinates(coordinates), imageType);
		}
	}
	
	private void renderPath(Unit u, Coordinate start, List directions, int imageType) {
		if(log.isDebugEnabled()) {
			log.debug("renderPath for unit "+u+" from "+start+" with list "+directions+", imageType "+imageType);
		}
	...
	private Image getImage(String name,int imageType) {
		...
		if (name != null) {
			...
			if (ownImages.containsKey(storeName)) {
				...
				switch(imageType) {
				case ACTIVE     : break;
				case PASSIVE    : img = createImage(img, passiveFilter); break;
				case ACTIVEPAST : img = createImage(img, activePastFilter); break;
				case PASSIVEPAST: img = createImage(img, passivePastFilter); break;
				}
				if (img != null) {
	...
	protected class Preferences extends JPanel implements PreferencesAdapter {
		...
		private void init() {
			chkDrawPassivePath = new JCheckBox(getString("drawpassivepath"), source.getDrawPassivePath());
