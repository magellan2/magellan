 ...
 * one computer report.
 * <p>The maps units, regions and so on are declared as abstract
 * methods and the getX and addX provide access to them. This allows
 * for subclasses that implicitely represent only a certain part of
 * the game data by declaring certain maps as <tt>null</tt> and
 * returning <tt>null</tt> on the corresponding getX() methods. This
 * concept has so far not been applied and you usually operate on the
 * <tt>CompleteData</tt> subclass.</p>
 */
abstract public class GameData implements Cloneable {
	 ...
	 * suggestion is accepted by the user (which means, that a
	 * TempUnit with this id was created) curTempID is increased.
	 * A value of -1 indicates, that the variable is uninitialized
	 * and a value of 0 that the old system shall be used
	 * (which means, that the suggested temp id shall
	 * be calculated out of the id of the parent unit of the tempunit).
	 */
	protected int curTempID = -1;

	/** 
	 * This method sets the current temp id with respect to the possible 
	 * max value of the current base. The value also has to be >= -1
	 *
	 * @param new temp id
	 */
	...
	/**
	 * The current file attached to the game data. If it is null, the 
	 * save as dialog shall be opened.
	 */
	public FileType filetype = null;
	/**
	 * The 'round' this game data belongs to. Note that this
	 * imposes a restriction on how fine-grained date information
	 * can be applied to game data or certain parts of it. This
	 * will probably have to be changed the one or the other way.
	 */
	protected Date date = null;

	/**
	 * The 'mail' connection this game data belongs to. This may 
	 * be null
	 */
	public String mailTo=null;
	public String mailSubject=null;

	/** A collection of all units. The keys are <tt>Integer</tt>
	 * objects containg the unit's ids. The values consist of
	 * objects of class <tt>Unit</tt>.
	 * TEMP units are not included, they are only stored in the unit
	 * collection of their parents and their regions.
	 ...
	 * can be referenced by the ownerFaction attribute.
	 ...
	 * @return an instance of class <tt>Building</tt> or
	 * <tt>null</tt> if there is no building with the
	 * specified id or if buildings() is <tt>null</tt>.
	 */
	 ...
	 * @return an instance of class <tt>Ship</tt> or
	 * <tt>null</tt> if there is no ship with the
	 ...
	 * @return an instance of class <tt>Faction</tt> or
	 * <tt>null</tt> if there is no faction with the
	 ...
	 * @return an instance of class <tt>Unit</tt> or
	 * <tt>null</tt> if there is no unit with the
	 ...
	 *
	 * @return an instance of class <tt>Region</tt> or
	 * <tt>null</tt> if there is no region with the
	 ...
	 * @return an instance of class <tt>MessageType</tt> or
	 * <tt>null</tt> if there is no message type with the
	 * specified id or if msgTypes() is <tt>null</tt>.
	 */
	 ...
	 * @return an instance of class <tt>Spell</tt> or
	 * <tt>null</tt> if there is no spell with the
	 ...
	 * @return an instance of class <tt>Potion</tt> or
	 * <tt>null</tt> if there is no potion with the
	 ...
	 * @return an instance of class <tt>Island</tt> or
	 * <tt>null</tt> if there is no island with the
	...
	/**
	 * Add a faction to the specified game data. If factions()
	 * is <tt>null</tt>, this method has no effect.
	 *
	...
	/**
	 * Add a unit to the specified game data. If units()
	 * is <tt>null</tt>, this method has no effect.
	 *
	...
	/**
	 * Add a region to the specified game data. If regions()
	 * is <tt>null</tt>, this method has no effect.
	 *
	...
	/**
	 * Add a ship to the specified game data. If ships()
	 * is <tt>null</tt>, this method has no effect.
	 *
	...
	/**
	 * Add a building to the specified game data. If buildings()
	 * is <tt>null</tt>, this method has no effect.
	 *
	...
	/**
	 * Add a message type to the specified game data. If msgTypes()
	 * is <tt>null</tt>, this method has no effect.
	 *
	...
	/**
	 * Add a spell to the specified game data. If spells()
	 * is <tt>null</tt>, this method has no effect.
	 *
	...
	/**
	 * Add a pption to the specified game data. If potions()
	 * is <tt>null</tt>, this method has no effect.
	 *
	...
	/**
	 * Add an island to the specified game data. If islands()
	 * is <tt>null</tt>, this method has no effect.
	 *
	...
	/**
	 * Add or set a hot spot to the specified game data. If
	 * hotSpots() is <tt>null</tt>, this method has no effect.
	 *
	 ...
	 * @return an instance of class <tt>HotSpot</tt> or
	 * <tt>null</tt> if there is no hot spot with the
	 ...
	 * @return an instance of class <tt>String</tt> or
	 * <tt>null</tt> if there is no value mapping to the specified key
	 * of if translations() is <tt>null</tt>.
	 */
	 ...
	 * Retrieve a translation from translations().
	 *
	 * @return an instance of class <tt>String</tt>. If no
	 * translation could be found, the key is returned.
	 */
	...
	/**
	 * The base (radix) in which ids are interpreted. The default
	 * value is 10. Note that all internal and cr representation is
	 * always decimal.
	 */
	public int base = 10;

	/**
	 * Indicates whether in this report skill points are to be
	 * expected or whether they are meaningful, respecitively.
	 */
			...
			if (f.messages != null) {
				for (Iterator msgIter = f.messages.iterator(); msgIter.hasNext();) {
					...
					if (msg.attributes != null) {
						for (Iterator attrIter = msg.attributes.keySet().iterator(); attrIter.hasNext();) {
	 ...
	 * Currently, this is only used to remember this setting and write
	 * it back into the cr.
	 */
	...
	public static GameData merge(GameData o1, GameData o2) {
		...
		if (!o1.name.equalsIgnoreCase(o2.name)) {
			throw new java.lang.IllegalArgumentException("GameData.merge(): Can't merge different game types. ("+o1.name+" via "+o2.name+")");
		}
	...
	/**
	 * Merges the two game data containers yielding a third one.
	 * By convention, o1 must not be newer than o2.
	 * The resulting game data container inherits the rules and name from
	 * <b>o2</b>.
	 */
	public final static boolean IP_NEW_MERGE=true;
	...
	private static GameData mergeIt(GameData o1, GameData o2) {
		...
		if (newGD.translations() != null) {
			...
			if (o2.translations() != null && o1.getLocale().equals(o2.getLocale())) {
		...
		// FACTIONS
		if (o1.factions() != null) {
			for (Iterator iter = o1.factions().values().iterator(); iter.hasNext();) {
				Faction f = (Faction)iter.next();
				try {
		...
		if (o2.factions() != null) {
			for (Iterator iter = o2.factions().values().iterator(); iter.hasNext();) {
				Faction f = (Faction)iter.next();
		...
		// REGIONS
		if (o1.regions() != null) {
			for (Iterator iter = o1.regions().values().iterator(); iter.hasNext();) {
		...
		if (o2.regions() != null) {
			for (Iterator iter = o2.regions().values().iterator(); iter.hasNext();) {
				Region r = (Region)iter.next();
		...
		// ISLANDS
		if (o1.islands() != null) {
			for (Iterator iter = o1.islands().values().iterator(); iter.hasNext();) {
				Island i = (Island)iter.next();
				try {
		...
		if (o2.islands() != null) {
			for (Iterator iter = o2.islands().values().iterator(); iter.hasNext();) {
				Island i = (Island)iter.next();
		...
		// HOTSPOTS
		if (o1.hotSpots() != null) {
			for (Iterator iter = o1.hotSpots().values().iterator(); iter.hasNext();) {
		...
		if (o2.hotSpots() != null) {
			for (Iterator iter = o2.hotSpots().values().iterator(); iter.hasNext();) {
				HotSpot h = (HotSpot)iter.next();
		...
		if (o2.buildings() != null) {
			for (Iterator iter = o2.buildings().values().iterator(); iter.hasNext();) {
		...
		if (o1.buildings() != null) {
			...
			for (Iterator iter = o1.buildings().values().iterator(); iter.hasNext();) {
				Building b = (Building)iter.next();
		...
		*/
			...
			for (Iterator iter = o1.ships().values().iterator(); iter.hasNext();) {
				...
				if (newShip == null) {
					if(log.isWarnEnabled()) {
						log.warn("unexp ship in o1 " + ((EntityID)curShip.getID()).intValue());
					}
				}
		...
		if (o2.ships() != null) {
			for (Iterator iter = o2.ships().values().iterator(); iter.hasNext();) {
				...
				if (newShip == null) {
					if(log.isWarnEnabled()) {
						log.warn("unexp ship in o2 " + ((EntityID)curShip.getID()).intValue());
					}
				}
		...
		while(it.hasNext()) {
			...
			if (curUnit1 != null) {
				if (dateEqual) { // full merge
					...
					// NOTE: It is wrong to copy skills from units that
					//       have no longer skills and are not from privileged factions
					if (curUnit2.skills!=null || curUnit1.getFaction().trustLevel >= Faction.TL_PRIVILEGED) {
	...
	/**
	 * returns a clone of the game data (using CRWriter/CRParser 
