									   ...
									   CompleterSettingsProvider
{
	...
	protected void offerCompletion(JTextComponent j) {
		...
		if((line.length() == 0) ||
			   ...
			   (j.getText().charAt(j.getCaretPosition() - 1) == '\n')) ||
			   ((j.getCaretPosition() > 0) &&
			   (j.getText().charAt(j.getCaretPosition() - 1) == '\r'))) {
	 ...
	 * TODO: DOCUMENT ME!
	 *
	 * @param completion TODO: DOCUMENT ME!
	 */
	...
	private class DetailAutoCompletionPreferencesAdapter extends JPanel
		implements PreferencesAdapter
	{
		...
		private JPanel getSelfDefinedCompletionPanel() {
			...
			delete.setMnemonic(getString("prefs.SelfDefinedCompletions.deleteButton.mnemonic")
								   .charAt(0));
			...
			newCompletion.setMnemonic(getString("prefs.SelfDefinedCompletions.newCompletionButton.mnemonic")
										  .charAt(0));
		...
		public void applyPreferences() {
			...
			for(Iterator iter = selfDefinedCompletions.keySet().iterator(); iter.hasNext();) {
				...
				settings.setProperty("AutoCompletion.SelfDefinedCompletions.value" +
									 completionCount, value);
				completionCount++;
			}

			settings.setProperty("AutoCompletion.SelfDefinedCompletions.count",
		...
		private class DefineCompletionDialog extends InternationalizedDialog {
			...
			private DefineCompletionDialog(Frame frame) {
				...
				cp.setBorder(new CompoundBorder(BorderFactory.createEtchedBorder(),
												new EmptyBorder(5, 5, 5, 5)));
				...
				name.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(),
												AutoCompletion.getString("DefineCompletionDialog.nameField.title")));
				cp.add(name, c);
				...
				cancel.setMnemonic(AutoCompletion.getString("DefineCompletionDialog.cancelButton.mnemonic")
												 .charAt(0));
