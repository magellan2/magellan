 ...
 * The data with the units to be considered is received solely by SelectionEvents.
 */
public class SkillChartPanel extends InternationalizedDataPanel implements SelectionListener {
	 ...
	 * The data is received by SelectionEvents (where factions and regions are considered).
	 * Despite of that an GameData-reference is necessary to set all regions as data, if no region is selected
	 */
	public SkillChartPanel(EventDispatcher ed, GameData data, Properties settings) {
		...
		try {
		  ...
		  chartPanel = new SkillChartJFreeChartPanel(new JFreeChart("", new Font("Arial", Font.BOLD, 24), dataSource, verticalBarPlot), this);
		}
...
//		chart.setChartBackgroundPaint(new GradientPaint(0, 0, Color.white, 0, 500, new Color(153, 153, 204)));
		...
		c = new GridBagConstraints(1, 1, 1, 1, 0.1, 0.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(6, 6, 6, 6), 2, 2);
		add(persons, c);

		totalSkillPoints = new JComboBox(new String[]{getString("labeltext.totalskilllevel")});
		c = new GridBagConstraints(0, 2, 1, 1, 0.1, 0.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(6, 6, 6, 6), 2, 2);
		...
		c = new GridBagConstraints(1, 2, 1, 1, 0.1, 0.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(6, 6, 6, 6), 2, 2);
	...
	private void setComboBoxes(SkillType skillType) {
		if (skillType != null) {
			...
			vPersons.add(getString("labeltext.totalpersons") + skillStats.getPersonNumber(skillType));
			vTotalSkillLevel.add(getString("labeltext.totalskilllevel") + skillStats.getSkillLevelNumber(skillType));
			vTotalSkillPoints.add(getString("labeltext.totalskillpoints") + skillStats.getSkillPointsNumber(skillType));
			...
			persons.setModel(new DefaultComboBoxModel(new String[]{getString("labeltext.totalpersons")}));
			totalSkillPoints.setModel(new DefaultComboBoxModel(new String[]{getString("labeltext.totalskilllevel")}));
			totalSkillLevel.setModel(new DefaultComboBoxModel(new String[]{getString("labeltext.totalskillpoints")}));
		}
	}

	/**
	 * creates a DefaultCategoryDataSource as basis of a skillchart
	 * out of the SkillStats-Object of this class and the specified skillType.
	 * If skillType is null, an empty datasource is created, containing the single value (0,0)
	 */
	...
	public String getToolTip(int i) {
		...
		if (type == null || i >= skillStats.getKnownSkills(type).size()) {
			...
			String retVal = getString("labeltext.totalpersons") + skillStats.getPersonNumber(skill);
			retVal += ", " + getString("labeltext.totalskilllevel") + skillStats.getSkillLevelNumber(skill);
			retVal += ", " + getString("labeltext.totalskillpoints") + skillStats.getSkillPointsNumber(skill);
			return retVal;
		}
	}
