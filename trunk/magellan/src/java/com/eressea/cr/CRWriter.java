...
/**
 * A class for writing game data, or parts of it, to a stream in the
 * computer report format.
 * <p>The generated output has platform dependent line breaks.</p>
 * <p>Implementation notes:</p>
 * <p>The basic mechanism of this class is to overload the
 * <tt>write()</tt> method for most of the Eressea base classes.
 * Since many blocks and tags come in bunches there are helper
 * functions for each such sequence handling the collections in which
 * the data objects are stored in.</p>
 */
public class CRWriter extends BufferedWriter {
	private final static Logger log = Logger.getInstance(CRWriter.class);
	...
	/**
	 * Remove quotes from text and replace blanks whithin the
	 * quoted section with ~ characters. ("a \"b c\"" -> "a b~c")
	 *
	 * @param text the string to be modified.
	...
	/**
	 * Write the String <tt>str</tt> quoted to the underlying
	 * stream. If a part of <tt>str</tt> is quoted, its quotes
	 * are escaped according to the current quote escape setting.
	 */
	...
	/**
	 * Write the String <tt>str</tt> quoted along with the
	 * specified tag to the underlying stream. If a part of
	 * <tt>str</tt> is quoted, its quotes are escaped
	 * according to the current quote escape setting.
	 * writeQuotedTag("a b", "tag") results in writing
	 * "\"a b\";tag\n" to the
	 *
	 * @param str the string that is to be put in quotes and
	 * written to the
	 * @param tag the tag to be written to the stream,
	 * separated from <tt>str</tt> by a semicolon.
	 */
	...
	/**
	 * Write a sequence of message blocks to the underlying
	 * stream.
	 *
	 * @param list a list containing the <tt>Message</tt> objects
	 * to be written.
	 */
	...
	/**
	 * Write the cr representation of a <tt>Message</tt>
	...
	/**
	 * Write the data as one block named <tt>blockName</tt> to the
	 * underlying stream. The data is written
	 * as simple cr strings. The block name is only written if
	 * there is data to follow.
	 *
	 * @param blockName the name of the block to be written (can
	 * not be a block with an id).
	...
	/**
	 * Write the data as one sequence of
	 * simple cr strings.
	 *
	 * @param data a collection containing <tt>Message</tt> objects.
	 */
	...
	/**
	 * Write the VERSION block for the specified game data to the
	 * underyling
	 */
	public void writeVersion(GameData world) throws IOException {
		...
		if (!serverConformance && world.getCurTempID() != -1) {
			write(world.getCurTempID() + ";curTempID"); /** @see com.eressea.GameData#curTempID */
			newLine();
		}
	 ...
	 *
	 * @param map a map containing the spells to write. The keys
	 * are expected to be <tt>Integer</tt> objects containing the
	 * ids of the spells. The values are expected to be instances
	 * of class <tt>Spell</tt>.
	 */
	...
	/**
	 * Write the cr representation of a <tt>Spell</tt> object to
	...
	/**
	 * Write a sequence of potion (TRANK) blocks to the
	 * underlying stream.
	 *
	 * @param map a map containing the potions to write. The keys
	 * are expected to be <tt>Integer</tt> objects containing the
	 * ids of the potions. The values are expected to be instances
	 * of class <tt>Potion</tt>.
	 */
	...
	/**
	 * Write the cr representation of a <tt>Potion</tt> object to
	 * the underlying stream.
	 */
	...
	/**
	 * Writes the ingredients of a potion as a ZUTATEN block to the
	 * underlying stream.
	 *
	...
	/**
	 * Write a spell components (KOMPONENTEN) block to the
	 * underyling. The block name is only written, if there
	 * are components in <tt>comps</tt>.
	 *
	 * @param map a map containing the components to be written.
	 * The map is expected to contain the names of the components
	 * as keys and the component data as values (both as
	 * <tt>String</tt> objects). Such a map can be found in the
	 * <tt>Spell</tt> class.
	...
	/**
	 * Write the cr representation of a <tt>Option</tt> object to
	 * the underlying stream.
	 */
	...
	/**
	 * Write a sequence of group (GRUPPE) blocks to the
	 * underlying stream.
	 *
	 * @param map a map containing the groups to write. The
	 * keys are expected to be <tt>Integer</tt> objects containing
	 * the ids of the alliances. The values are expected to be
	 * instances of class <tt>Group</tt>.
	 */
	...
	/**
	 * Write the cr representation of a <tt>Group</tt> object
	...
	/**
	 * Write a sequence of alliance (ALLIANZ) blocks to the
	 * underlying stream.
	 *
	 * @param map a map containing the alliances to write. The
	 * keys are expected to be <tt>Integer</tt> objects containing
	 * the ids of the alliances. The values are expected to be
	 * instances of class <tt>Alliance</tt>.
	 */
	...
	/**
	 * Write the cr representation of an <tt>Alliance</tt> object
	 * to the underlying stream.
	 */
	...
	/**
	 * Write a sequence of battle (BATTLE) blocks to the
	 * underlying stream.
	 *
	 * @param list a list containing the <tt>Battle</tt> objects
	 * to be written
	 */
	...
	/**
	 * Write the cr representation of a <tt>Battle</tt> object
	 * to the underlying stream.
	 */
	...
	/**
	 * Write a sequence of faction (PARTEI) blocks to the
	 * underlying stream.
	 *
	 * @param map a map containing the factions to write. The
	 * keys are expected to be <tt>Integer</tt> objects containing
	 * the ids of the factions. The values are expected to be
	 * instances of class <tt>Faction</tt>.
	 */
	...
	/**
	 * Write the cr representation of a <tt>Faction</tt> object to
	 * the underlying stream.
	 */
	...
	/**
	 * Write a sequence of ship (SCHIFF) blocks to the underlying
	 * stream.
	 *
	 * @param it an iterator containing the <tt>Ship</tt> objects
	 *  to be written.
	 */
	...
	/**
	 * Write the cr representation of a <tt>Ship</tt> object to
	 * the underlying stream.
	 */
	...
	/**
	 * Write a sequence of building (BURG) blocks to the
	 * underlying stream.
	 *
	 * @param it an iterator containing the <tt>Building</tt>
	...
	/**
	 * Write the cr representation of a <tt>Building</tt> object
	 * to the underlying stream.
	 */
	public void writeBuilding(Building building) throws IOException {
		...
		if (building.getOwnerUnit() != null) {
			...
			if (building.getOwnerUnit().getFaction() != null) {
				write(((EntityID)building.getOwnerUnit().getFaction().getID()).intValue() + ";Partei");
				newLine();
			}
		}
	 ...
	 * The block is only written, if <tt>skills</tt> contains
	 * at least one <tt>Skill</tt> object.
	 *
	 * @param skills an iterator over the <tt>Skill</tt> objects
	 * to write.
	 * @param persons the number of persons in the unit this skill
	 * belongs to.
	 */
	...
	/**
	 * Write the cr representation of a <tt>Skill</tt> object to
	 * the underlying stream.
	 */
	...
	/**
	 * Write a COMMANDS block to the underlying stream. The block
	 * is only written, if <tt>list</tt> contains at least one
	 * <tt>String</tt> object representing an order.
	 *
	 * @param list a list with the <tt>String</tt> objects to be
	 * written.
	 */
	 ...
	 *  The block is only written, if <tt>list</tt>
	 * contains at least one <tt>Spell</tt> object.
	 *
	 * @param list a list with the <tt>Spell</tt> object names
	 *  to be written.
	 */
	...
	/**
	 * Write a unit's combat spell (KAMPFZAUBER) blocks to the
	 * underlying stream.
	 *
	 * @param map a Map with the <tt>CombatSpell</tt> objects
	 * to be written.
	 */
	...
	/**
	 * Write the cr representation of a <tt>CombatSpell</tt> object to
	 * the underlying stream.
	 */
	...
	/**
	 * Write a unit's items (GEGENSTAENDE) block to the underlying
	 * stream. The block is only written, if <tt>items</tt>
	 * contains at least one <tt>Item</tt> object.
	 *
	 * @param items an iterator over the <tt>Item</tt> objects
	 * to be written.
	 */
	...
	/**
	 * Write the cr representation of a <tt>Item</tt> object to
	 * the underlying stream.
	 */
	...
	/**
	 * Write a sequence of unit (EINHEIT) blocks to the
	 * underlying stream.
	 *
	 * @param it an iterator for the <tt>Unit</tt> objects
	 * to be written.
	 */
	...
	/**
	 * Write the cr representation of a <tt>Unit</tt> object to
	 * the underyling
	 */
	...
	/**
	 * Write a region prices (PREISE) block to the underlying
	 * stream.
	 *
	 * @param list list containing the <tt>LuxuryPrice</tt> objects
	 * to be written.
	 */
	...
	/**
	 * Write region block containing the luxury prices of the last
	 * turn (LETZTEPREISE) to the underlying stream.
	 *
	 * @param map a map containing the <tt>LuxuryPrice</tt> objects
	 * to be written.
	 */
	...
	/**
	 * Write the cr representation of a <tt>LuxuryPrice</tt>
	 * object to the underlying stream.
	 */
	...
	/**
	 * Write a sequence of region border (GRENZE) blocks to the
	 * underlying stream.
	 *
	 * @param c collection containing the <tt>Border</tt> objects
	 * to be written.
	 */
	...
	/**
	 * Write the cr representation of a <tt>Border</tt>
	 * object to the underlying stream.
	 */
	...
	/**
	 * Write a sequence of region blocks to the
	 * underlying stream.
	 *
	 * @param map a map containing the region to write. The keys
	 * are expected to be <tt>Integer</tt> objects containing the
	 * ids of the regions. The values are expected to be instances
	 * of class <tt>Region</tt>.
	 */
	...
	/**
	 * Write a sequence of region (REGION) blocks to the
	 * underlying stream.
	 *
	...
	/**
	 * Write the cr representation of a <tt>Region</tt>
	 * object to the underlying stream.
	 */
	...
	/**
	 * Writes the cr representation of a Scheme object to the
	 * underlying stream.
	 */
	 ...
	 *
	 * @param map a map containing the <tt>MessageType</tt>
	 * objects to be written as values.
	 */
	...
	/**
	 * Write the cr representation of a <tt>MessageType</tt>
	 * object to the underlying stream.
	 */
	public void writeMessageType(MessageType msgType) throws IOException {
		...
		if (msgType.getPattern() == null) {
			log.warn("CRWriter.writeMessageType(): pattern of message type " + msgType.getID() + " is null");
			return;
		}
	...
	/**
	 * Write the complete game data from <tt>world</tt> in the
	 * cr format.
	 *
			...
			if (world.units() != null) {
				if (world.units().size() != unitsWritten) {
					...
					if (world.units().size() - homelessUnitsCounter != unitsWritten) {
						throw new IOException("Although there are " + (world.units().size() - homelessUnitsCounter) + " units, only " + unitsWritten + " were written!");
					}
				}
			}
		}
	...
	/**
	 * Change the quote escape behaviour of this CRWriter. Tilde
	 * escapes look like: 'a "b c"' -> 'a b~c', whereas a
	 * backslash escape works like this: 'a "b c"' -> 'a \"b c\"'
	 *
	 * @param bool if <tt>true</tt>, escape quoted parts of any
	 * string written to the underlying stream with tildes. If
	 * <tt>false</tt>, use backslash character to escape the
	...
	/**
	 * Creates a CR writer with a ouput buffer of the specified
	 * size.
	 *
	 * @param out the stream to write output to.
	...
	/**
	 * Returns whether <tt>write(GameData data)</tt> writes
	 * information about the regions in data to the underlying
	 * stream.
	 */
	...
	/**
	 * Toggles whether <tt>write(GameData data)</tt> writes
	 * information about the regions in data to the underlying
	 * stream.
	 */
	...
	/**
	 * Returns whether <tt>write(GameData data)</tt> writes
	 * information about the buildings in data to the underlying
	 * stream.
	 */
	...
	/**
	 * Returns whether <tt>write(GameData data)</tt> writes
	 * information about the ships in data to the underlying
	 * stream.
	 */
	...
	/**
	 * Returns whether <tt>write(GameData data)</tt> writes
	 * information about the units in data to the underlying
	 * stream.
	 */
	...
	/**
	 * Returns whether <tt>write(GameData data)</tt> writes
	 * detailed information about the regions in data to the
	 * underlying stream.
	 */
	...
	/**
	 * Returns whether <tt>write(GameData data)</tt> writes
	 * information about islands to the underlying stream.
	 */
	...
	/**
	 * Returns whether <tt>write(GameData data)</tt> writes
	 * messages contained in the game data to the underlying
	 * stream.
	 */
	...
	/**
	 * Returns whether <tt>write(GameData data)</tt> writes
	 * a cr that is compatible with cr's generated by the
	...
	/**
	 * Write a sequence of island blocks to the
	 * underlying stream.
	 *
	 * @param map a map containing the islands to write. The keys
	 * are expected to be <tt>Integer</tt> objects containing the
	 * ids of the islands. The values are expected to be instances
	 * of class <tt>Island</tt>.
	 */
	...
	/**
	 * Write the cr representation of an <tt>Island</tt>
	 * object to the underlying stream.
	 */
	...
	/**
	 * Write the cr representation of a hot spot to the underlying
	 * stream.
	 */
	...
	/**
	 * Returns the regions this object writes to the underlying
	 * stream.
	 */
	...
	/**
	 * Supply the writer with a collection of regions it should
	 * write to the underlying stream instead of all regions
	 * contained in the game data. If regions is null or if there
	 * is no element in the supplied collection, the writer returns
	 * to writing all regions defined in the game data.
	 */
