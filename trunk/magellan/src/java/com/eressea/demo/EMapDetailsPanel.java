...
public class EMapDetailsPanel extends InternationalizedDataPanel implements SelectionListener, TreeSelectionListener, ShortcutListener, ActionListener, TreeUpdate, PreferencesFactory, MenuProvider {
	private final static Logger log = Logger.getInstance(EMapDetailsPanel.class);
	...
	/**
	 * A list containing nodewrapper objects, the expansion state
	 * of nodes contained in this list can be automatically stored
	 * and restored
	 */
	...
	private final static NumberFormat weightNumberFormat = NumberFormat.getNumberInstance();
	...
	protected AbstractButton addTag, removeTag;
	protected Container tagContainer, treeContainer;
	...
	private void initGUI(UndoManager _undoMgr) {
		...
		nodeWrapperFactory = new NodeWrapperFactory(settings, "EMapDetailsPanel", getString("wrapperfactory.title"));
		...
		// load settings
		showFactionItemCategories = Boolean.valueOf(settings.getProperty("EMapDetailsPanel.showFactionItemCategories", Boolean.TRUE.toString())).booleanValue();
		// name text area
		name = new JTextArea();
				...
				public void editingStopped(ChangeEvent e) {
					...
					if (selectedNode != null && selectedNode instanceof CommentNode) {
						...
						if (uc != null && uc.comments != null) {
							uc.comments.set(parent.getIndex(cn), tree.getCellEditor().getCellEditorValue());
						}
					}
				}
			});
		...
		pnlRegionInfoTree.add(regionPanel = new BasicRegionPanel(dispatcher, settings), BorderLayout.NORTH);
		...
		add(bottomSplitPane);
		// adjust split panes
		bottomSplitPane.setDividerLocation(Integer.parseInt(settings.getProperty("EMapDetailsPanel.bottomSplitPane", "400")));
		topSplitPane.setDividerLocation(Integer.parseInt(settings.getProperty("EMapDetailsPanel.topSplitPane", "200")));
	...
	private void appendRegionInfo(Region r, DefaultMutableTreeNode parent, Collection expandableNodes) {
		// terrain type
		parent.add(createSimpleNode(getString("node.terrain") + ": " + r.getType().getName(),r.getType().getID()+"-detail"));
		// terrain coordinates
	 ...
	 * - Luxuries: amount
	 *   luxury item 1: price 1
	 *   ...
	 *   luxury item n: price n
	 */
	private void appendRegionLuxuriesInfo(Region r, DefaultMutableTreeNode parent, Collection expandableNodes) {
		if(r == null || r.prices == null || r.prices.values().isEmpty()) {
			return;
		}
		DefaultMutableTreeNode luxuriesNode = new DefaultMutableTreeNode(getString("node.trade") + ": " + getDiffString(r.maxLuxuries(), r.maxOldLuxuries()));
	 ...
	 *   entertain:
	 */
	private void appendRegionPeasantInfo(Region r, DefaultMutableTreeNode parent, Collection expandableNodes) {
		if(r.peasants <= 0) {
			return;
		}
		...
		String peasantsInfo = getDiffString(r.peasants, r.oldPeasants, r.getModifiedPeasants()).toString();
		peasantsInfo += " / " + maxWorkers;
		DefaultMutableTreeNode peasantsNode = createSimpleNode(getString("node.peasants") + ": " + peasantsInfo,"bauern");
		parent.add(peasantsNode);
		expandableNodes.add(new NodeWrapper(peasantsNode, "EMapDetailsPanel.RegionPeasantsExpanded"));

		// recruit
		peasantsNode.add(createSimpleNode(getString("node.recruit") + ": " + getDiffString(r.maxRecruit(),r.maxRecruit(),r.modifiedRecruit())+" "+getString("node.of")+r.maxRecruit(),"rekruten"));
		// silver
		peasantsNode.add(createSimpleNode(getString("node.silver") + ": " + getDiffString(r.silver, r.oldSilver),"items/silber"));
		// surplus
		peasantsNode.add(createSimpleNode(getString("node.surplus") + ": " + getDiffString(surplus, oldSurplus),"items/silber"));
		// wage
		...
		StringBuffer nodeText = new StringBuffer(getString("node.wage")).append(": ").append(getDiffString(getWage(wage, false), getWage(r.oldWage, false)));
		if (data.rules.getRace(EresseaRaceConstants.R_ORKS) != null) {
			nodeText.append(", ").append(data.rules.getRace(EresseaRaceConstants.R_ORKS).getName()).append(": ").append(getDiffString(getWage(wage, true), getWage(r.oldWage, true)));
		}
		peasantsNode.add(createSimpleNode(nodeText.toString(),"lohn"));
		// entertain
		peasantsNode.add(createSimpleNode(getString("node.enterainment") + ": " + getDiffString(r.maxEntertain(), r.maxOldEntertain()),"items/silber"));
	}


	private void appendRegionResourceInfo(Region r, DefaultMutableTreeNode parent, Collection expandableNodes) {
		...
		if (!r.resources().isEmpty()) {
			...
			if (r.trees > 0 || r.oldTrees > 0) {
				...
				if (r.mallorn) {
					treeNode = createSimpleNode(getString("node.mallorntrees") + ": " + getDiffString(r.trees, r.oldTrees),"items/"+mallornID);
				}else {
					treeNode = createSimpleNode(getString("node.trees") + ": " + getDiffString(r.trees, r.oldTrees),"items/"+treesID);
				}
				resourceNode.add(treeNode);
			}
			if (r.iron > 0 || r.oldIron > 0) {
				resourceNode.add(createSimpleNode(data.rules.getItemType(StringID.create("Eisen")).getName() + ": " + getDiffString(r.iron, r.oldIron),"items/Eisen"));
			}
			if (r.laen > 0 || r.oldLaen > 0) {
				resourceNode.add(createSimpleNode(getString("node.laen") + ": " + getDiffString(r.laen, r.oldLaen),"items/"+laenID));
			}
		}

		// horse
		if (r.horses > 0 || r.oldHorses > 0) {
			resourceNode.add(createSimpleNode(getString("node.horses") + ": " + getDiffString(r.horses, r.oldHorses),"items/pferd"));
		}
		// herb
		if (r.herb != null) {
		...
		if (resourceNode.getChildCount() > 0) {
			...
			expandableNodes.add(new NodeWrapper(resourceNode, "EMapDetailsPanel.RegionResourcesExpanded"));
		}
	}
	...
	private void appendMultipleRegionInfo(Collection r, DefaultMutableTreeNode parent, Collection expandableNodes) {
		// collect the data
		...
		for (ListIterator iter = sortedList.listIterator(); iter.hasNext(); ) {
			...
			DefaultMutableTreeNode regionsNode = createSimpleNode(rType.getName() + ": " + list.size(),rType.getID().toString());
			...
			for (Iterator iter2 = list.iterator(); iter2.hasNext(); ) {
				...
				regionsNode.add(new DefaultMutableTreeNode(nodeWrapperFactory.createRegionNodeWrapper(region, persons)));
			}
		}
		...
		for (ListIterator iter = sortedList.listIterator(); iter.hasNext(); ) {
			...
			if (amount > 0) {
				resourcesNode.add(createSimpleNode(resType.getName() + ": " + amount,"items/"+resType.getIconName()));
			}
		}
		if (resourcesNode.getChildCount() > 0) {
			parent.add(resourcesNode);
			expandableNodes.add(new NodeWrapper(resourcesNode, "EMapDetailsPanel.RegionResourcesExpanded"));
		}
		...
		for (ListIterator iter = sortedList.listIterator(); iter.hasNext(); ) {
			...
			DefaultMutableTreeNode regionsNode = new DefaultMutableTreeNode(herbType.getName() + ": " + i, true);
	...
	/**
	 * Return a string showing the signed difference of the two int
	 * values in the form "<tt>current</tt> [&lt;sign&gt;&lt;number&gt;]". If
	 * the two values are equal or if one of them is -1,
	...
	/**
	 * Return a string showing the signed difference of the three int
	 * values in the form "<tt>current</tt> (future) [&lt;sign&gt;&lt;change&gt;]".
	 * e.g. (1,2,1) returns 1 [-1]
	 * e.g. (1,2,2) returns 1 (2) [-1]
	 * e.g. (1,1,2) returns 1 (2)
	 * e.g. (1,1,1) returns 1
	 * If future == -1, then ignore
	 */
	...
	// region guards
	private void  appendRegionGuardInfo( Region r, DefaultMutableTreeNode parent, Collection expandableNodes) {
	...
	/**
	 * Appends a folder with the schemes in the specified region to
	 * the specified parent node.
	 */
	private void appendRegionSchemes(Region region, DefaultMutableTreeNode parent, Collection expandableNodes) {
	 ...
	 */
	private void appendTags(UnitContainer uc, DefaultMutableTreeNode parent, Collection expandableNodes) {
	...
	private void appendTags(Taggable uc, DefaultMutableTreeNode parent, Collection expandableNodes, String nodeInfo) {
		if (!uc.hasTags()) {
			return;
		}
	...
	private void appendFactionsInfo(Collection factions, DefaultMutableTreeNode parent, Collection expandableNodes) {
		if (lastRegion != null) {
			...
			while (units.hasNext() == true) {
				...
				if (u.getFaction() != null && factions.contains(u.getFaction())) {
					...
					for (Iterator i = u.getSkills().iterator(); i.hasNext(); ) {
						...
						SkillStatItem stored = (SkillStatItem)skills.get(skill.getName() + skill.getLevel());
						if (stored != null) {
			...
			if (showFactionItemCategories) {
				Collection catNodes = unitsTools.addCategorizedUnitItems(regionUnits, parent, null, null, true, nodeWrapperFactory);
				for (Iterator catIter = catNodes.iterator(); catIter.hasNext(); ) {
					...
					expandableNodes.add(new NodeWrapper(node, "EMapDetailsPanel." + cat.getID().toString() + "Expanded"));
				}
			} else {
				...
				unitsTools.addUnitItems(regionUnits, itemsNode, null, null, true, nodeWrapperFactory);
			}
...
//						sb.append(",");
						...
						skillNode.add(new DefaultMutableTreeNode(nodeWrapperFactory.createUnitNodeWrapper(u, sb.toString())));
					}
				}
			}
		}
	...
	private void appendGroupInfo(Group g, DefaultMutableTreeNode parent, Collection expandableNodes) {
		if (lastRegion != null) {
			...
			DefaultMutableTreeNode n = new DefaultMutableTreeNode(personCount + " " + getString("node.persons"));
			parent.add(n);
			if (g.allies() != null && g.allies().values().size() > 0) {
				...
				Collections.sort(sortedAllies, new AllianceFactionComparator(new NameComparator(new IDComparator())));
			...
			if (showFactionItemCategories) {
				...
				for (Iterator catIter = catNodes.iterator(); catIter.hasNext(); ) {
					...
					if(log.isDebugEnabled()) {
						log.debug("EmapDetailPanel.appendGroupInfo: found class "+node.getUserObject().getClass()+
	...
	private void appendUnitInfo(Unit u, DefaultMutableTreeNode parent, Collection expandableNodes) {
		...
		if (u.getFaction() == null) {
			fNode = new DefaultMutableTreeNode(nodeWrapperFactory.createSimpleNodeWrapper(getString("node.faction") + ": " + getString("node.unknownfaction"), "faction"));
		} else {
			fNode = new DefaultMutableTreeNode(nodeWrapperFactory.createSimpleNodeWrapper(getString("node.faction") + ": " + u.getFaction().toString(),"faction"));
		}
		parent.add(fNode);
		...
		if (u.getGroup() != null) {
			...
			DefaultMutableTreeNode groupNode = new DefaultMutableTreeNode(getString("node.group") + ": \"" + group.getName() + "\"");
		...
		// Kampfreihenanzeige
		SimpleNodeWrapper cWrapper = nodeWrapperFactory.createSimpleNodeWrapper(getString("node.combatstatus") + ": " + Unit.combatStatusToString(u),"kampfstatus");
		if (u.getFaction() != null && u.getFaction().trustLevel >= Faction.TL_PRIVILEGED) {
		...
		// guard state
		if (u.guard != 0) {
			parent.add(createSimpleNode(getString("node.guards") + ": " + Unit.guardFlagsToString(u.guard),"bewacht"));
		}
		...
		if (u.hideFaction || stealthLevel > 0 ) {
			...
			if (stealthLevel > 0 && u.getFaction()!=null && u.getFaction().trustLevel >= Faction.TL_PRIVILEGED) {
		...
		// magic aura
		if (u.aura != -1) {
			parent.add(createSimpleNode(getString("node.aura") + ": " + u.aura + " / " + u.auraMax,"aura"));
		}
		...
		// }
		if (load != 0 || modLoad != 0) {
			text = getString("node.load") + ": " + weightNumberFormat.format(new Float(load / 100.0F));
			if (load != modLoad) {
		...
		if (maxOnFoot == Unit.CAP_UNSKILLED) {
			parent.add(createSimpleNode(getString("node.capacityonfoot") + ": " + getString("node.toomanyhorses"),"warnung"));
		} else {
			...
			if (maxOnFoot - modLoad < 0) {
				capacityNode = new DefaultMutableTreeNode(nodeWrapperFactory.createSimpleNodeWrapper(getString("node.capacityonfoot") + ": " + getString("node.overloadedby") + weightNumberFormat.format(free) + " " + getString("node.weightunits"),"warnung"));
			} else {
				capacityNode = new DefaultMutableTreeNode(nodeWrapperFactory.createSimpleNodeWrapper(getString("node.capacityonfoot") + ": " + weightNumberFormat.format(free) + " / " + weightNumberFormat.format(max) + " " + getString("node.weightunits"),"fuss"));
				appendUnitCapacityByItems(capacityNode, u, maxOnFoot - modLoad);
				if (capacityNode.getChildCount() > 0) {
					expandableNodes.add(new NodeWrapper(capacityNode, "EMapDetailsPanel.UnitCapacityOnFootExpanded"));
				}
			}
		...
		if (maxOnHorse == Unit.CAP_UNSKILLED) {
			parent.add(createSimpleNode(getString("node.capacityonhorse") + ": " + getString("node.toomanyhorses"),"warnung"));
			...
			if (maxOnHorse - modLoad < 0) {
				capacityNode = new DefaultMutableTreeNode(nodeWrapperFactory.createSimpleNodeWrapper(getString("node.capacityonhorse") + ": " + getString("node.overloadedby") + weightNumberFormat.format(free) + " " + getString("node.weightunits"),"warnung"));
			} else {
				capacityNode = new DefaultMutableTreeNode(nodeWrapperFactory.createSimpleNodeWrapper(getString("node.capacityonhorse") + ": " + weightNumberFormat.format(free) + " / " + weightNumberFormat.format(max) + " " + getString("node.weightunits"),"reiten"));
				appendUnitCapacityByItems(capacityNode, u, maxOnHorse - modLoad);
				if (capacityNode.getChildCount() > 0) {
					expandableNodes.add(new NodeWrapper(capacityNode, "EMapDetailsPanel.UnitCapacityOnHorseExpanded"));
				}
			}
			parent.add(capacityNode);
		}
		...
		if(u.getModifiedItems().size() > 0) {
			...
			for(Iterator items = u.getModifiedItems().iterator(); items.hasNext(); ) {
				...
				DefaultMutableTreeNode itemNode = new DefaultMutableTreeNode(nodeWrapperFactory.createItemNodeWrapper(u, modItem));
				// pavkovic 2002.05.21: a try to handle this node as node with children
				itemsNode.add(itemNode);
				expandableNodes.add(new NodeWrapper(itemNode, "EMapDetailsPanel.UnitItemsRelationsExpanded"));
				for(Iterator iter = u.getItemTransferRelations(modItem).iterator(); iter.hasNext(); ) {
					...
					UnitNodeWrapper unw = nodeWrapperFactory.createUnitNodeWrapper(u2, prefix, u2.getPersons(), u2.getModifiedPersons());
		...
		if(!sortedSkills.isEmpty()) {
			...
			for(Iterator iter = sortedSkills.iterator(); iter.hasNext(); ) {
				...
				skillsNode.add(new DefaultMutableTreeNode(nodeWrapperFactory.createSkillNodeWrapper(u, os, ms)));
			}
		}
		...
		if (teachers.size() > 0) {
			...
			for (Iterator iter = teachers.iterator(); iter.hasNext(); ) {
				...
				UnitNodeWrapper w = nodeWrapperFactory.createUnitNodeWrapper(teacher, teacher.getPersons(), teacher.getModifiedPersons());
				DefaultMutableTreeNode teacherNode = new DefaultMutableTreeNode(w);
				teachersNode.add(teacherNode);
			}
			teachersNode.setUserObject(new UnitListNodeWrapper(getString("node.teacher") + ": " + teacherCounter + " / " + (u.getModifiedPersons() % 10 == 0 ? u.getModifiedPersons() / 10 : u.getModifiedPersons() / 10 + 1), null, teachers));
		}
		if (pupils.size() > 0) {
			...
			for (Iterator iter = pupils.iterator(); iter.hasNext(); ) {
				...
				UnitNodeWrapper w = nodeWrapperFactory.createUnitNodeWrapper(pupil, pupil.getPersons(), pupil.getModifiedPersons());
				DefaultMutableTreeNode pupilNode = new DefaultMutableTreeNode(w);
				pupilsNode.add(pupilNode);
			}
			pupilsNode.setUserObject(new UnitListNodeWrapper(getString("node.pupils") + ": " + pupilCounter + " / " + (u.getModifiedPersons() * 10), null, pupils));
		}

		// transportation relations
		Map carriers = u.getCarriers();
		if (carriers.size() > 0) {
			DefaultMutableTreeNode carriersNode = new DefaultMutableTreeNode(new UnitListNodeWrapper(getString("node.carriers"), null, carriers.values()));
			...
			for (Iterator iter = carriers.values().iterator(); iter.hasNext(); ) {
				...
				UnitNodeWrapper w = nodeWrapperFactory.createUnitNodeWrapper(carrier, carrier.getPersons(), carrier.getModifiedPersons());
				carriersNode.add(new DefaultMutableTreeNode(w));
			}
		}
		if (passengers.size() > 0) {
			DefaultMutableTreeNode passengersNode = new DefaultMutableTreeNode(new UnitListNodeWrapper(getString("node.passengers"), null, passengers.values()));
			parent.add(passengersNode);
			expandableNodes.add(new NodeWrapper(passengersNode, "EMapDetailsPanel.UnitPassengersExpanded"));
			for (Iterator iter = passengers.values().iterator(); iter.hasNext(); ) {
				Unit passenger = (Unit)iter.next();
				String str = passenger.toString() + ": " + weightNumberFormat.format(new Float(passenger.getWeight() / 100.0f)) + " " + getString("node.weightunits");
				if (passenger.getWeight() != passenger.getModifiedWeight()) {
					str += " (" + weightNumberFormat.format(new Float(passenger.getModifiedWeight() / 100.0f)) + " " + getString("node.weightunits") + ")";
				}
...

		{
			...
			if (attacks.size() > 0) {
				DefaultMutableTreeNode attacksNode = new DefaultMutableTreeNode(new UnitListNodeWrapper(getString("node.attacks"), null, carriers.values()));
				...
				for (Iterator iter = attacks.iterator(); iter.hasNext(); ) {
					...
					UnitNodeWrapper w = nodeWrapperFactory.createUnitNodeWrapper(victim, victim.getPersons(), victim.getModifiedPersons());
					attacksNode.add(new DefaultMutableTreeNode(w));
				}
			}
			Collection attackedBy = u.getAttackAggressors();
			if (attackedBy.size() > 0) {
				DefaultMutableTreeNode attackedByNode = new DefaultMutableTreeNode(new UnitListNodeWrapper(getString("node.attackedBy"), null, carriers.values()));
				parent.add(attackedByNode);
				expandableNodes.add(new NodeWrapper(attackedByNode, "EMapDetailsPanel.UnitAttackedByExpanded"));
		...
		// Trank Anzeige
		if (data.potions() != null && u.skills != null && u.skills.containsKey(EresseaSkillConstants.S_ALCHEMIE)) {
			...
			for (Iterator iter = potions.iterator(); iter.hasNext(); ) {
				...
				if (p.getLevel() * 2 <= alchSkill.getLevel()) {
					...
					potionsNode.add(new DefaultMutableTreeNode(nodeWrapperFactory.createPotionNodeWrapper(p, ": " + max)));
				}
			}
	...
	private void appendUnitPersonInfo(Unit u, DefaultMutableTreeNode parent, Collection expandableNodes) {
	...
	private void appendUnitCombatSpells(Map spells, DefaultMutableTreeNode parent, Collection expandableNodes) {
	...
	/**
	 * Determines how many potions potion can be brewed from the herbs
	 * owned by privileged factions in the specified region.
	 */
	...
	private void appendBuildingInfo(Building b, DefaultMutableTreeNode parent, Collection expandableNodes) {
		...
		// Typ
		n = createSimpleNode(getString("node.type") + ": " + b.getType().getName(), b.getType().getID().toString());
		parent.add(n);
		// Besitzer
		if ( b.getOwnerUnit() != null ) {
			UnitNodeWrapper w = nodeWrapperFactory.createUnitNodeWrapper(b.getOwnerUnit(), getString("node.owner") + ": ", b.getOwnerUnit().getPersons(), b.getOwnerUnit().getModifiedPersons());
			n = new DefaultMutableTreeNode(w);
			parent.add(n);
			if (b.getOwnerUnit().getFaction() == null) {
				n = createSimpleNode(getString("node.faction") + ": " + getString("node.unknownfaction"), "faction");
			} else {
				n = createSimpleNode(getString("node.faction") + ": " + b.getOwnerUnit().getFaction().toString(), "faction");
			}
		}
		parent.add(n);
		// Insassen
		...
		if (units.size() > 0 || modUnits.size() > 0) {
			n = new DefaultMutableTreeNode(new UnitListNodeWrapper(getString("node.inmates"), null, allInmates));
			parent.add(n);
		 ...
		 n = new DefaultMutableTreeNode(nodeWrapperFactory.createSimpleNodeWrapper((inmates == modInmates ? getString("node.size") + ": " + inmates : getString("node.size") + ": " + inmates + " (" + modInmates + ")") + " / " + b.getSize(), "build_size"));
		 parent.insert(n, 0);
	...
	private void appendShipInfo(Ship s, DefaultMutableTreeNode parent, Collection expandableNodes) {
		...
		if (s.size != nominalShipSize) {
			...
			parent.add(new DefaultMutableTreeNode(getString("node.completion") + ": " + ratio + "% (" + s.size + "/" + nominalShipSize + ")"));
		} else {
			// Kueste
			if (s.shoreId > -1) {
				parent.add(createSimpleNode(getString("node.shore") + ": " + Direction.toString(s.shoreId), "shore_"+String.valueOf(s.shoreId)));
			}
			...
			if (s.getOwnerUnit() != null && s.getOwnerUnit().race != null && s.getOwnerUnit().race.getID().equals(EresseaRaceConstants.R_MEERMENSCHEN)) {
			...
			parent.add(loadNode);
			if (s.damageRatio > 0) {
				int absolute = new BigDecimal(s.damageRatio * s.size).divide(new BigDecimal(100),BigDecimal.ROUND_UP).intValue();
				parent.add(createSimpleNode(getString("node.damage") + ": " + s.damageRatio + "% / "+String.valueOf(absolute), "damage"));
			}
		}
		...
		if (s.getOwnerUnit() != null) {
			UnitNodeWrapper w = nodeWrapperFactory.createUnitNodeWrapper(s.getOwnerUnit(), getString("node.captain") + ": ", s.getOwnerUnit().getPersons(), s.getOwnerUnit().getModifiedPersons());
			n = new DefaultMutableTreeNode(w);
			parent.add(n);
			Faction fac = s.getOwnerUnit().getFaction();
			if (fac == null) {
				n = new DefaultMutableTreeNode(nodeWrapperFactory.createSimpleNodeWrapper(getString("node.faction") + ": " + getString("node.unknownfaction"), "faction"));
			} else {
				n = new DefaultMutableTreeNode(nodeWrapperFactory.createSimpleNodeWrapper(getString("node.faction") + ": " + fac.toString(),"faction"));
			}
			parent.add(n);
		...
		// Ladung
		if (loadNode != null) {
			String strLoad = s.load != -1 ? Integer.toString(s.load) : weightNumberFormat.format(new Float(load / 100.0F));
	...
	private void appendBorderInfo(Border b, DefaultMutableTreeNode parent, Collection expandableNodes) {
		parent.add(createSimpleNode(getString("node.type") + ": " + b.type, b.type));
		parent.add(createSimpleNode(getString("node.direction") + ": " + Direction.toString(b.direction), "border_"+String.valueOf(b.direction)));
	...
	private void appendSpellInfo(Spell s, DefaultMutableTreeNode parent, Collection expandableNodes, Object backTarget) {
		...
		if (s.getComponents() != null && s.getComponents().size() > 0) {
			...
			expandableNodes.add(new NodeWrapper(componentsNode, "EMapDetailsPanel.SpellComponentsExpanded"));
				...
				if (key.equalsIgnoreCase("Aura")) {
					...
					if (blankPos > 0 && blankPos < val.length()) {
						...
						if (getLevelAtDays.equals("0")) {
							...
							compNode = createSimpleNode(aura + " " + getString("node.aura") + " * " + getString("node.level"),"aura");
						} else {
							compNode = createSimpleNode(aura + " " + getString("node.aura") + " * " + getLevelAtDays + " * " + getString("node.level"),"aura");
						}
					} else {
	...
	private void appendPotionInfo(Potion p, DefaultMutableTreeNode parent, Collection expandableNodes, Object backTarget) {
	...
	/**
	 * sets the island name        editable
	 * sets the island description editable
	 * shows an empty tree
	 */
	...
	/**
	 * sets the name ""            not editable
	 * sets the description ""     not editable
	 * shows an empty tree
	 */
	...
	private void storeExpansionState() {
		for(Iterator iter = myExpandableNodes.iterator(); iter.hasNext(); ) {
			...
			if (nw.getNode().getChildCount() > 0) {
				...
				settings.setProperty(nw.getDesc(), tree.isVisible(new TreePath(firstChild.getPath())) ? "1" : "0");
			}
		}
	}
	...
	/**
	 * Selection event handler, update all elements in this panel
	...
	public void selectionChanged(com.eressea.event.SelectionEvent se) {
		...
		removeTag.setEnabled(false);
		/**
		 * Decide whether to show a single object (activeObject) or
		 * a collection of objects (selectedObjects).
		 * The display for collection is at the moment limited to
		 * collections of regions. Other implementations should be added.
		 */
		...
		/**
		 * No collection of objects could be displayd.
		 * Show the (single) active object.
		 */
	 ...
	 * If no implementation applies, false is
	 * returned, otherwise true.
	 */
	protected boolean showMultiple(Collection c) {
		boolean regions = true;
		boolean factions = true;
		for (Iterator iter = c.iterator(); iter.hasNext() && (regions || factions); ) {
			Object o = iter.next();
	...
	/**
	 * Calculates the wage for the units of a certain faction in the
	 * specified region.
	 */
	...
	/**
	 * TreeSelection event handler, notifies unit-listeners, if a unit
	 * has been selected.
	 */
	...
	private void appendComments(UnitContainer uc, DefaultMutableTreeNode parent, Collection expandableNodes) {
	...
	/**
	 * When showing a faction this controls whether the items of all
	 * units of this faction in the previously selected region are
	 * displayed as a flat collection or below an additional node
	...
	public void showFactionItemCategories(boolean bool) {
		if (showFactionItemCategories != bool) {
			...
			settings.setProperty("EMapDetailsPanel.units.showDetailedSkills", (new Boolean(showFactionItemCategories)).toString());
		}
	}

	/**
	 * Returns whether the items of all units of the selected faction
	 * in the previously selected region are
	 * displayed as a flat collection or below an additional node
	 * level of item categories.
	 */
	...
	public void actionPerformed(java.awt.event.ActionEvent actionEvent) {
		if (actionEvent.getSource() == addTag) {
			...
			if (key!= null && key.length()>0) {
				String value = JOptionPane.showInputDialog(tree, getString("addtag.tagvalue.message"));
...
/*		if (tree.getModel() instanceof DefaultTreeModel) {
	...
	private class SkillStatItemComparator implements Comparator {
		public int compare(Object o1, Object o2) {
			...
			if (retVal == 0) {
				retVal = ((SkillStatItem)o2).skill.getLevel() - ((SkillStatItem)o1).skill.getLevel();
			}
			return retVal;
		}
	...
	class EMapDetailsPreferences extends JPanel implements ExtendedPreferencesAdapter{
		...
		private JPanel getDataViewPanel() {
			...
			help.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("prefs.datadisplay")));
			GridBagConstraints c = new GridBagConstraints(0, 0, 1, 1, 1.0, 0, GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL, new Insets(3, 3, 3, 3), 0, 0);

			chkshowFactionItemCategories = new JCheckBox(getString("prefs.factionItemCategories"), source.isShowingFactionItemCategories());
			...
			chkShowTagButtons = new JCheckBox(getString("prefs.showTagButtons"), source.isShowingTagButtons());
			c.gridy = 1;
	...
	private class StealthContextFactory implements ContextFactory {
		...
		public javax.swing.JPopupMenu createContextMenu(com.eressea.GameData data, Object argument, Collection selectedObjects, javax.swing.tree.DefaultMutableTreeNode node) {
		...
		private class StealthContextMenu extends JPopupMenu implements ActionListener {
			...
			public StealthContextMenu(Unit u) {
				unit = u;
				Skill stealth = unit.getSkill(data.rules.getSkillType(EresseaSkillConstants.S_TARNUNG, true));
	...
	private class CombatStateContextFactory implements ContextFactory {
		...
		public javax.swing.JPopupMenu createContextMenu(GameData data, Object argument, Collection selectedObjects, DefaultMutableTreeNode node) {
			if (argument instanceof Unit) {
	...
	private class CommentContextFactory implements ContextFactory {
		public javax.swing.JPopupMenu createContextMenu(com.eressea.GameData data, Object argument, Collection selectedObjects, DefaultMutableTreeNode node) {
		...
		private class CommentContextMenu extends JPopupMenu implements ActionListener{
			...
			private JMenuItem addComment, removeComment, modifyComment, removeAllComments;

			public CommentContextMenu(UnitContainer container, DefaultMutableTreeNode node, boolean removeAll) {
				uc = container;
			...
			private void addComment() {
				...
				for (Enumeration enum = rootNode.children(); enum.hasMoreElements(); ) {
					...
					if (obj != null && obj instanceof String && obj.equals(getString("node.comments"))) {
						parent = n;
						break;
					}
				}
	...
	/**
	 * We need a special factory for the normal UnitContextMenu since we have to
	 * take care of unit lists.
	 */
	private class DetailsUnitContextFactory implements ContextFactory {
		public javax.swing.JPopupMenu createContextMenu(GameData data, Object argument,Collection selectedObjects, DefaultMutableTreeNode node) {
			if (argument instanceof Unit) {
				...
				return new UnitContextMenu(((UnitNodeWrapper)argument).getUnit(), selectedObjects, dispatcher, data);
	...
	public synchronized static Map getDefaultTranslations() {
		if(defaultTranslations == null) {
			...
			defaultTranslations.put("prefs.factionItemCategories" , "Group a faction's items by category (herbs, weapons, etc.)");
			defaultTranslations.put("prefs.showNextSkillLevelPoints" , "Show the number of points required to reach the next skill level with units");
			defaultTranslations.put("prefs.showNextSkillLevelLearnTurns" , "Show the number of turns to learn in order to reach the next skill level");
			...
			defaultTranslations.put("prefs.showcontainericons" , "Show building and ship icons with units");
