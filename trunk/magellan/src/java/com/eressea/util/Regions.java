...
public class Regions {
	private final static Logger log = Logger.getInstance(Regions.class);
	...
	public static List getPath(Map regions, Coordinate start, Coordinate dest, Map excludedRegionTypes) {
		...
		while (true) {
			...
			if (excludedRegionTypes.containsKey(curRegion.getType().getID())) {
				log.warn("Regions.getPath(): Found an region of type " + curRegion.getType().getName() + " in region list! Removing and ignoring it.");
			...
			if (minDistance < Float.MAX_VALUE) {
				...
				if (!distances.containsKey(curCoord)) {
					...
					log.warn("Regions.getPath(): Found a region with known distance in backlog list: " + curRegion);
				}
			}
		}
			...
			if (dist != null) {
				...
				for (Iterator iter = neighbours.values().iterator(); iter.hasNext();) {
					...
					if (nbDist != null) {
						...
						log.warn("Regions.getPath(): Found neighbouring region without distance: " + curNb + " neighbouring " + curRegion);
					}
				}
	...
	public static List planShipRoute(Ship ship, Coordinate destination, Map allregions, RegionType oceanType, BuildingType harbour, int speedBonus) {
		if (destination != null) {
			...
			for (Iterator iter = startregions.iterator(); iter.hasNext(); ) {
				...
				List path = Regions.getPath(regions, startregion.getCoordinate(), destination, CollectionFactory.createHashtable());
				if (path != null && path.size() > 0) {
					paths.add(path);
				}
				List pathWithHarbours = Regions.getPath(harbourRegions, startregion.getCoordinate(), destination, CollectionFactory.createHashtable());
				if (pathWithHarbours != null && pathWithHarbours.size() > 0 && !pathWithHarbours.equals(path)) {
			...
			if (paths.size() > 1) {
				...
				for (Iterator iter = paths.iterator(); iter.hasNext(); curpos++) {
					...
					if (weeks < minweeks) {
						minweeks = weeks;
						minpos = curpos;
					} else if (weeks == minweeks && ((List)paths.get(minpos)).size() > path.size()) {
						minpos = curpos;
					}
				}
			}
	...
	private static Coordinate getMovement(GameData data, String ID, Coordinate c,List travelledRegions) {
