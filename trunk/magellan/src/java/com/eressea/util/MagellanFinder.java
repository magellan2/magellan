...
/**
 * Small class for start-up help. Two search functions: The Magellan directory and
 * the settings file.
 *
 * @author  Andreas
 * @version 
...
public class MagellanFinder {
	private final static Logger log = Logger.getInstance(MagellanFinder.class);
	...
	/**
	 * Searches for Magellan. This method scans the CLASSPATH and searches
	 * for JARs containing "com.eressea.demo.Client" or correspoding directory
	 * structures.
	 */
	public static File findMagellanDirectory() {
		...
		while (st.hasMoreTokens()) {
			...
			try {
				...
				if (!file.isDirectory())
					file = file.getParentFile();
				File dir = new File(file, "com");
				if (dir.isDirectory()) {
					dir = new File(dir, "eressea");
					if (dir.isDirectory()) {
						dir = new File(dir, "demo");
						if (dir.isDirectory()) {
							String list[] = dir.list();
							if (list.length > 0)
								for (int i = 0; i < list.length; i++)
									if (list[i].equals("Client.class")) {
										log.info("Magellan directory: " + file + "(found com.eressea.demo.Client.class)");
										return file;
									}
						}
					}
	...
	/**
	 * Extracts the directory out of the given file. If any error
	 * occurs, the current directory(".") is returned.
	 */
	...
	/**
	 * Checks if the given file is a zip and contains
	 * a "com/eressea/demo/Client.class". These are the
	 * conditions for the file to be a valid magellan
	 * Java Archive (JAR).
	 */
