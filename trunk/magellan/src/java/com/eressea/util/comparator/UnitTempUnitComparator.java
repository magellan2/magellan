...
/**
 * A comparator imposing an ordering on Unit and TempUnit objects by
 * sorting them according to a parent-child relation.
 ...
 * of equality. I.e. if the two compared units do not have a parent-
 * child relation, instead of 0 the result of either the
 * unit sub-comparator's or the same-parent sub-comparator's
...
public class UnitTempUnitComparator implements Comparator {
	 ...
	 * @param sameParentSubComparator if two units with the same
	 * parent unit are compared, this sub-comparator is applied
	 * if it is not <tt>null</tt>.
	 * @param unitSubComparator if two units do not have the same
	 * parent unit and do not have a parent-child relation, this sub-
	 * comparator is applied if it is not <tt>null</tt>.
	 */
	public UnitTempUnitComparator (Comparator sameParentSubComparator, Comparator unitSubComparator) {
	 ...
	 *
	 * @returns a value less than zero if o1 is.an instance of class
	 ...
	 * returned.
	 * If not both, o1 and o2, are instances of class TempUnit or they
	 * are but have different parents they or their parents
	 * respectively are compared with the unitsubcomparator if it is
	 * not null, else 0 is returned.
	 */
