...
/**
 * A comparator imposing an ordering on Region objects by comparing
 * the islands they belong to.
 ...
 * of equality. I.e. if the two compared regions belong to the same island
 * and they would be regarded as equal by this comparator, instead of 0
 * the result of the sub-comparator's comparison is returned. Similarly,
 * the no-island sub-comparator is applied when the island members of both
 * regions are null.</p>
 */
public class RegionIslandComparator implements Comparator {
	 ...
	 * @param islandComparator determines how the islands are sorted.
	 *  If null is specified the islands are sorted according to their
	 *  natural order.
	 * @param sameIslandSubComparator if two regions belonging to the
	 *  same island are compared, this sub-comparator is applied if
	 *  it is not <tt>null</tt>.
	 * @param noIslandSubComparator if the compared regions both have
	 *  island members that are null, this sub-comparator is applied
	 *  if it is not <tt>null</tt>.
	 */
	public RegionIslandComparator (Comparator islandComparator, Comparator sameIslandSubComparator, Comparator noIslandSubComparator) {
	...
	/**
	 * Compares its two arguments for order according to the islands
	 * they belong to.
	 * @returns the result of comparing <tt>o1</tt>'s and <tt>o2</tt>'s
	 *  islands with the specified island comparator.
	 *  If both belong to the same island and a same-island
	 *  sub-comparator was specified, the result of that
	 *  sub-comparator's comparison is returned.
	 *  If one of the islands is null, it is considered larger than
	 *  the other one.
	 *  If both islands are null the no-island sub-comparator is
	 *  applied.
	 */
