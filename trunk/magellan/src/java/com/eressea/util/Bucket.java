...
/**
 * A LRU-like organized collection, i.e. calls to the add() method
 * always insert elements at the front of the collection. If an
 * already exisiting element is added to the bucket it is moved to
 * the front of the bucket. If a maximum size is set for the bucket
 * it does not grow beyond this limit by dropping elements at the end
 * of the bucket.
 */
public class Bucket extends AbstractCollection {
	...
	/**
	 * Creates a Bucket object with unlimited maximum size. I.e. this
	 * bucket will never drop elements by itself.
	 */
	public Bucket() {
		this(-1);
	}
	...
	/**
	 * Add an object to the front of the bucket. If the number of
	 * elements in the bucket exceeds the bucket's maximum size an
	 * element at the end of the bucket is dropped.
	 * If object o is already contained in the bucket, it is moved
	...
	public boolean equals(Object o) {
		...
		if (o instanceof Bucket) {
			...
			if (this.getMaxSize() == b.getMaxSize() && this.size() == b.size()) {
				Iterator i1 = this.iterator(), i2 = b.iterator();
	...
	/**
	 * Returns the maximum size of this bucket that it is not
	 * allowed to outgrow.
	 * 
	...
	/**
	 * Sets the specified maximum size for this bucket. If there are
	 * more elements in the bucket than maxSize, elements at the end
	 * of the bucket are dropped until the size of the bucket equals
	 * maxSize.
	 * 
