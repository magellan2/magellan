...
/**
 * This is a self cleaning properties implementation. The cleaning is done
 * after  loading the properties.
 *
...
public class SelfCleaningProperties extends OrderedOutputProperties {
	...
	/**
	 * This operation iterates other a copy of all keys to check if they can be
	 * cleaned
	 */
	private void doClean() {
		// operate on a copy of the keys as they can be deleted/changed while runtime
		for(Iterator iter = CollectionFactory.createHashSet(keySet()).iterator();
				iter.hasNext();) {
	...
	private void doClean(String name) {
		...
		name = renameProperty(name, "DirectoryHistory",
	...
	private String doExpandFactionColors(String oldName, String key) {
		if(oldName.equals(key)) {
			...
			for(StringTokenizer st = new StringTokenizer(getProperty(oldName),
														 delim);
					st.hasMoreTokens(); i++) {
			...
			log.error("SelfCleaningProperties.doClean: Expanded property " +
					  oldName + " to " + newName);
	...
	/**
	 * rename property e.g. Ausdauer.compareValue to
	 * ClientPreferences.compareValue.Ausdauer
	 *
	 * @param name TODO: DOCUMENT ME!
	 *
	...
	private String renameProperty(String oldName, String newName) {
		...
		log.error("SelfCleaningProperties.doClean: Renamed property " +
				  oldName + " to " + newName);
