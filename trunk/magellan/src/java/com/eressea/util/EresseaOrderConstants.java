// ===
// Copyright (C) 2000, 2001 Roger Butenuth, Andreas Gampe, Stefan Götz, Sebastian Pappert, Klaas Prause, Enno Rehling, Sebastian Tusk
// ---
// This file is part of the Eressea Java Code Base, see the file LICENSING for the licensing information applying to this file
// ---
// $Id$
// ===

package com.eressea.util;


/**
 * Contains the String keys of all Eressea orders
 */
public interface EresseaOrderConstants {
	public static final String O_ADDRESSES      = "ADDRESSES";
	public static final String O_AGGRESSIVE     = "AGGRESSIVE";
	public static final String O_AFTER          = "AFTER";
	public static final String O_ALL            = "ALL";
	public static final String O_ATTACK         = "ATTACK";
	public static final String O_AURA           = "AURA";
	public static final String O_BANNER         = "BANNER";
	public static final String O_BEFORE         = "BEFORE";
	public static final String O_BUY            = "BUY";
	public static final String O_BZIP2          = "BZIP2";
	public static final String O_CARRY          = "CARRY";
	public static final String O_CAST           = "CAST";
	public static final String O_CASTLE         = "CASTLE";
	public static final String O_COMBAT         = "COMBAT";
	public static final String O_COMBATSPELL    = "COMBATSPELL";
	public static final String O_CONTACT        = "CONTACT";
	public static final String O_CONTROL        = "CONTROL";
	public static final String O_COMPUTER       = "COMPUTER";
	public static final String O_DEFAULT        = "DEFAULT";
	public static final String O_DEFENSIVE      = "DEFENSIVE";
	public static final String O_DESCRIBE       = "DESCRIBE";
	public static final String O_DESTROY        = "DESTROY";
	public static final String O_EMAIL          = "EMAIL";
	public static final String O_END            = "END";
	public static final String O_ENTER          = "ENTER";
	public static final String O_ENTERTAIN      = "ENTERTAIN";
	public static final String O_ERESSEA        = "ERESSEA";
	public static final String O_FACTION        = "FACTION";
	public static final String O_FACTIONSTEALTH = "FACTIONSTEALTH";
	public static final String O_FLEE           = "FLEE";
	public static final String O_FOLLOW         = "FOLLOW";
	public static final String O_FOREIGN        = "FOREIGN";
	public static final String O_FORGET         = "FORGET";
	public static final String O_FRONT          = "FRONT";
	public static final String O_GIVE           = "GIVE";
	public static final String O_GROUP          = "GROUP";
	public static final String O_GROW           = "GROW";
	public static final String O_GUARD          = "GUARD";
	public static final String O_HELP           = "HELP";
	// FIXME(pavkovic) 2003.04.16: this is only used for german to distinguish between "HELFEN" and "HELFE"
	public static final String O_HELP_COMBAT    = "HELP_COMBAT";
	public static final String O_HERBS          = "HERBS";
	public static final String O_HIDE           = "HIDE";
	public static final String O_HORSES         = "HORSES";
	public static final String O_ITEMPOOL       = "ITEMPOOL";
	public static final String O_LEARN          = "LEARN";
	public static final String O_LEAVE          = "LEAVE";
	public static final String O_LEVEL          = "LEVEL";
	public static final String O_LOCALE         = "LOCALE";
	public static final String O_MAKE           = "MAKE";
	public static final String O_MEN            = "MEN";
	public static final String O_MESSAGE        = "MESSAGE";
	public static final String O_MOVE           = "MOVE";
	public static final String O_NAME           = "NAME";
	public static final String O_NEXT           = "NEXT";
	public static final String O_NOT            = "NOT";
	public static final String O_NUMBER         = "NUMBER";
	public static final String O_OPTION         = "OPTION";
	public static final String O_ORIGIN         = "ORIGIN";
	public static final String O_PASSWORD       = "PASSWORD";
	public static final String O_PAUSE          = "PAUSE";
	public static final String O_PIRACY         = "PIRACY";
	public static final String O_PLAINTEXT      = "PLAINTEXT";
	public static final String O_PLANT          = "PLANT";
	public static final String O_POTIONS        = "POTIONS";
	public static final String O_PREFIX         = "PREFIX";
	public static final String O_PRIVATE        = "PRIVATE";
	public static final String O_QUIT           = "QUIT";
	public static final String O_REAR           = "REAR";
	public static final String O_RECRUIT        = "RECRUIT";
	public static final String O_REGION         = "REGION";
	public static final String O_REPORT         = "REPORT";
	public static final String O_RESEARCH       = "RESEARCH";
	public static final String O_RESERVE        = "RESERVE";
	public static final String O_RESTART        = "RESTART";
	public static final String O_RIDE           = "RIDE";
	public static final String O_ROAD           = "ROAD";
	public static final String O_ROUTE          = "ROUTE";
	public static final String O_SEED           = "SEED";
	public static final String O_SELL           = "SELL";
	public static final String O_SABOTAGE       = "SABOTAGE";
	public static final String O_SHIP           = "SHIP";
	public static final String O_SHOW           = "SHOW";
	public static final String O_SIEGE          = "SIEGE";
	public static final String O_SILVER         = "SILVER";
	public static final String O_SILVERPOOL     = "SILVERPOOL";
	public static final String O_SORT           = "SORT";
	public static final String O_SPELLBOOK      = "SPELLBOOK";
	public static final String O_SPELLS         = "SPELLS";
	public static final String O_SPY            = "SPY";
	public static final String O_STATISTICS     = "STATISTICS";
	public static final String O_STEAL          = "STEAL";
	public static final String O_SUPPLY         = "SUPPLY";
	public static final String O_TAX            = "TAX";
	public static final String O_TEACH          = "TEACH";
	public static final String O_TEMP           = "TEMP";
	public static final String O_TEMPLATE       = "TEMPLATE";
	public static final String O_UNIT           = "UNIT";
	public static final String O_USE            = "USE";
	public static final String O_WORK           = "WORK";
	public static final String O_ZIPPED         = "ZIPPED";

	// directions:
	public static final String O_NORTHWEST      = "NORTHWEST";
	public static final String O_NORTHEAST      = "NORTHEAST";
	public static final String O_EAST           = "EAST";
	public static final String O_SOUTHEAST      = "SOUTHEAST";
	public static final String O_SOUTHWEST      = "SOUTHWEST";
	public static final String O_WEST           = "WEST";

	public static final String O_NW             = "NW";
	public static final String O_NE             = "NE";
	public static final String O_E              = "E";
	public static final String O_SE             = "SE";
	public static final String O_SW             = "SW";
	public static final String O_W              = "W";

	/** @deprecated */
	public static final String O_ADRESSEN = O_ADDRESSES;
	/** @deprecated */
	public static final String O_AGGRESSIV = O_AGGRESSIVE;
	/** @deprecated */
	public static final String O_ALLES = O_ALL;
	/** @deprecated */
	public static final String O_ARBEITEN = O_WORK;
	/** @deprecated */
	public static final String O_ATTACKIEREN = O_ATTACK;
	/** @deprecated ? */
	public static final String O_AUSWERTUNG = O_REPORT;
	/** @deprecated */
	public static final String O_BEKLAUEN = O_STEAL;
	/** @deprecated */
	public static final String O_BELAGERE = O_SIEGE;
	/** @deprecated */
	public static final String O_BENENNEN = O_NAME;
	/** @deprecated */
	public static final String O_BENUTZEN = O_USE;
	/** @deprecated */
	public static final String O_BESCHREIBEN = O_DESCRIBE;
	/** @deprecated */
	public static final String O_BETRETEN = O_ENTER;
	/** @deprecated */
	public static final String O_BEWACHEN = O_GUARD;
	/** @deprecated */
	public static final String O_BOTSCHAFT = O_MESSAGE;
	/** @deprecated */
	public static final String O_BURG = O_CASTLE;
	/** @deprecated */
	public static final String O_DEFENSIV = O_DEFENSIVE;
	/** @deprecated */
	public static final String O_EINHEIT = O_UNIT;
	/** @deprecated */
	public static final String O_ENDE = O_END;
	/** @deprecated */
	public static final String O_FAHREN = O_RIDE;
	/** @deprecated */
	public static final String O_FLIEHE = O_FLEE;
	/** @deprecated */
	public static final String O_FOLGEN = O_FOLLOW;
	/** @deprecated */
	public static final String O_FORSCHEN = O_RESEARCH;
	/** @deprecated */
	public static final String O_FREMDES = O_FOREIGN;
	/** @deprecated */
	public static final String O_GIB = O_GIVE;
	/** @deprecated */
	public static final String O_GRUPPE = O_GROUP;
	/** @deprecated */
	public static final String O_HELFE = "HELFE";
	/** @deprecated */
	public static final String O_HELFEN = O_HELP;
	/** @deprecated */
	public static final String O_HINTEN = O_REAR;
	/** @deprecated */
	public static final String O_HINTER = O_AFTER;
	/** @deprecated */
	public static final String O_KAMPFZAUBER = O_COMBATSPELL;
	/** @deprecated */
	public static final String O_KAUFEN = O_BUY;
	/** @deprecated */
	public static final String O_KOMMANDO = O_CONTROL;
	/** @deprecated */
	public static final String O_KONTAKTIEREN = O_CONTACT;
	/** @deprecated */
	public static final String O_KRAEUTER = O_HERBS;
	/** @deprecated */
	public static final String O_KAEMPFE = O_COMBAT;
	/** @deprecated */
	public static final String O_LEHREN = O_TEACH;
	/** @deprecated */
	public static final String O_LERNEN = O_LEARN;
	/** @deprecated */
	public static final String O_LIEFERE = O_SUPPLY;
	/** @deprecated */
	public static final String O_MACHEN = O_MAKE;
	/** @deprecated */
	public static final String O_MATERIALPOOL = O_ITEMPOOL;
	/** @deprecated */
	public static final String O_NACH = O_MOVE;
	/** @deprecated */
	public static final String O_NEUSTART = O_RESTART;
	/** @deprecated */
	public static final String O_NICHT = O_NOT;
	/** @deprecated */
	public static final String O_NUMMER = O_NUMBER;
	/** @deprecated */
	public static final String O_NAECHSTER = O_NEXT;
	/** @deprecated */
	public static final String O_PARTEI = O_FACTION;
	/** @deprecated */
	public static final String O_PARTEITARNUNG = O_FACTIONSTEALTH;
	/** @deprecated */
	public static final String O_PASSWORT = O_PASSWORD;
	/** @deprecated */
	public static final String O_PERSONEN = O_MEN;
	/** @deprecated */
	public static final String O_PFERDE = O_HORSES;
	/** @deprecated */
	public static final String O_PFLANZEN = O_PLANT;
	/** @deprecated */
	public static final String O_PIRATERIE = O_PIRACY;
	/** @deprecated */
	public static final String O_PRAEFIX = O_PREFIX;
	/** @deprecated */
	public static final String O_PRIVAT = O_PRIVATE;
	/** @deprecated */
	public static final String O_REKRUTIEREN = O_RECRUIT;
	/** @deprecated */
	public static final String O_RESERVIEREN = O_RESERVE;
	/** @deprecated */
	public static final String O_SAMEN = O_SEED;
	/** @deprecated */
	public static final String O_SABOTIEREN = O_SABOTAGE;
	/** @deprecated */
	public static final String O_SCHIFF = O_SHIP;
	/** @deprecated */
	public static final String O_SILBER = O_SILVER;
	/** @deprecated */
	public static final String O_SILBERPOOL = O_SILVERPOOL;
	/** @deprecated */
	public static final String O_SORTIEREN = O_SORT;
	/** @deprecated */
	public static final String O_SPIONIEREN = O_SPY;
	/** @deprecated */
	public static final String O_STATISTIK = O_STATISTICS;
	/** @deprecated */
	public static final String O_STIRB = O_QUIT;
	/** @deprecated */
	public static final String O_STRASSE = O_ROAD;
	/** @deprecated */
	public static final String O_STUFE = O_LEVEL;
	/** @deprecated */
	public static final String O_TARNEN = O_HIDE;
	/** @deprecated */
	public static final String O_TRANSPORTIEREN = O_CARRY;
	/** @deprecated */
	public static final String O_TREIBEN = O_TAX;
	/** @deprecated */
	public static final String O_TRAENKE = O_POTIONS;
	/** @deprecated */
	public static final String O_UNTERHALTEN = O_ENTERTAIN;
	/** @deprecated */
	public static final String O_URSPRUNG = O_ORIGIN;
	/** @deprecated */
	public static final String O_VERGESSEN = O_FORGET;
	/** @deprecated */
	public static final String O_VERKAUFEN = O_SELL;
	/** @deprecated */
	public static final String O_VERLASSEN = O_LEAVE;
	/** @deprecated */
	public static final String O_VOR = O_BEFORE;
	/** @deprecated */
	public static final String O_VORNE = O_FRONT;
	/** @deprecated */
	public static final String O_ZAUBER = O_SPELLS;
	/** @deprecated */
	public static final String O_ZAUBERBUCH = O_SPELLBOOK;
	/** @deprecated */
	public static final String O_ZAUBERE = O_CAST;
	/** @deprecated */
	public static final String O_ZEIGEN = O_SHOW;
	/** @deprecated */
	public static final String O_ZERSTOEREN = O_DESTROY;
	/** @deprecated */
	public static final String O_ZUGVORLAGE = O_TEMPLATE;
	/** @deprecated */
	public static final String O_ZUECHTEN = O_GROW;
}
