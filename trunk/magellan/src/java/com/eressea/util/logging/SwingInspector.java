...
public class SwingInspector {
	...
	public static String printInputMaps(JComponent comp) {
		return printMap(comp.getInputMap(JComponent.WHEN_FOCUSED), "WHEN_FOCUSED",comp.getActionMap())+
			printMap(comp.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT), "WHEN_ANCESTOR_OF_FOCUSED_COMPONENT",comp.getActionMap())+
			printMap(comp.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW), "WHEN_IN_FOCUSED_WINDOW",comp.getActionMap())+
	...
	private static StringBuffer printKeyStrokes(KeyStroke[] keystrokes, ActionProvider actionprovider) {
		...
		for (int i = 0; i < keystrokes.length; i++) {
			...
			String keyText = ppString(keystroke.getKeyChar()!= 0 ? String.valueOf(keystroke.getKeyChar()) : KeyEvent.getKeyText(keystroke.getKeyCode()),15);
			sb.append(keyText+" ");
	...
	private static class ActionProvider{
		...
		public String getAction(KeyStroke keystroke) {
			...
			if(inputmap != null) {
				...
				if(action == null || actionmap.get(action) == null) return null;
