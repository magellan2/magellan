 ...
 * quoted strings and comments and marks the generated tokens as such. Note that
 * the tokenizer is not intended to detect keywords, identifiers and numbers, since
 * such a classification can only be made with syntactical context.
 * For proper handling of escaped new lines this class wraps the
 * underlying stream in a MergeLineReader. This implies that the
 * start and end attributes of the produced <tt>OrderToken</tt>
 * objects reflect the actual position and length of the token on the
...
public class OrderTokenizer {
	...
	/**
	 * Creates a new <tt>OrderTokenizer</tt> object which will
	 * perform its read operations on the specified stream.
	 * @param r the stream this <tt>OrderTokenizer</tt> reads
	 * from.
	 */
	 ...
	 * The token types are only recognized and set partially by
	 * this tokenizer. Apart from comments and quoted strings
	 * this has to be done in a semantical context.
	 */
	...
	/**
	 * Reads from the underlying stream up to the next quotation
	 * mark or line break.
	 * @return a <tt>OrderToken</tt> object of type TT_STRING
	 * containing the quoted string.
	 */
	private OrderToken readQuote() throws IOException {
		StringBuffer sb = new StringBuffer("\"");
		int c = 0, start = in.getPos() - 1;
		while ((c = in.read()) != -1) {
			if (c == '"') {
				sb.append((char)c);
				break;
	...
	/**
	 * Reads a one line comment beginning with a semicolon up to
	 * the next line break.
	 * @return a <tt>OrderToken</tt> object of type TT_COMMENT
	...
	/**
	 * Reads a one line comment beginning with a double slash up
	...
	private OrderToken readSSComment() throws IOException {
		...
		if (c == '/') {
			...
			retVal = new OrderToken(sb.toString(), start, in.getPos() - 1, OrderToken.TT_COMMENT, true);
		} else {
			in.unread(c);
			in.unread('/');
			retVal = readWord();
	 ...
	 * @return a <tt>OrderToken</tt> object of type TT_UNDEF
	 * containing the word.
	 */
	private OrderToken readWord() throws IOException {
		StringBuffer sb = new StringBuffer();
		OrderToken retVal = new OrderToken("", -1, -1, OrderToken.TT_EOC);
		int c = 0, start = in.getPos();
		while ((c = in.read()) != -1) {
			if (c == '\r' || c == '\n' || c == ' ' || c == '\t' || c == '"' || c == ';' || c == '/') {
				in.unread(c);
				break;
