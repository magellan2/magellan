...
public class ReportMerger extends java.lang.Object {
	private final static Logger log = Logger.getInstance(ReportMerger.class);
		...
		private class Confirm implements Runnable {
			...
			public void run() {
				if (JOptionPane.showConfirmDialog(dlg,
					strMessage, getString("msg.confirmmerge.title"), JOptionPane.YES_NO_OPTION,
	...
	private class Worker implements Runnable {
		...
		Worker(ReportMerger _merger) { merger = _merger; }

		public void run() { merger.mergeThread(); }
	}
			...
			if (iFailedConnectivity > 0) {
				...
				for( int i = 0; i < reports.length; i++) {
					if (!reports[i].merged) {
						...
						if (i+1 < reports.length) strMessage += ", ";
					}
				}
				...
				if (ui.confirm( strMessage, getString("msg.confirmmerge.title"))) {
					for( int i = 0; i < reports.length; i++) {
						if (!reports[i].merged) {
							iProgress += 2;
							ui.setProgress( reports[i].file.getName() + " - " + getString("status.merging"),
								iProgress );
	...
	private boolean mergeReport( Report report ) {
		...
		if ( data.getDate() != null
				...
				// to be added report is newer
				&& data.getDate().getDate() < report.data.getDate().getDate()
				// and contains userset trust levels
		...
		/**
		 * Prepare curTempID-Value for merging.
		 * If reports are of the same age, keep existing
		 * by setting the new one to default value.
		 * Otherwise set the existing to default value.
		 */
		if ( data.getDate() != null
				&& report.data.getDate() != null
				// to be added report is newer
		...
		int maxTerrainMismatches = (int)(Math.max(data.regions().size(), report.data.regions().size()) * 0.02);
					...
					if (reportDataRegion != null && reportDataRegion.getType() != null) {
						if (!r.getType().equals(reportDataRegion.getType())) {
							/* now we have a mismatch. If the reports
							   ...
							   can be transformed. */
							if (data.getDate() != null && report.data.getDate() != null && data.getDate().equals(report.data.getDate())) {
								mismatches++;
							} else {
								if (! (forestTerrain != null && plainTerrain != null && (forestTerrain.equals(r.getType()) && plainTerrain.equals(reportDataRegion.getType()) ||
										plainTerrain.equals(r.getType()) && forestTerrain.equals(reportDataRegion.getType())) ||
										oceanTerrain != null && glacierTerrain != null && (oceanTerrain.equals(r.getType()) && glacierTerrain.equals(reportDataRegion.getType()) ||
										glacierTerrain.equals(r.getType()) && oceanTerrain.equals(reportDataRegion.getType())) ||
										activeVolcanoTerrain != null && volcanoTerrain != null && (activeVolcanoTerrain.equals(r.getType()) && volcanoTerrain.equals(reportDataRegion.getType()) ||
	...
	public synchronized static Map getDefaultTranslations() {
		if(defaultTranslations == null) {
			...
			defaultTranslations.put("msg.noconnection.text.1" , "These reports seem to be non-overlapping and therefore not connectable:");
