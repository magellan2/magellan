 ...
 * This class can be used as a stand-alone application or can be
 * integrated as dialog into another application.
 */
public class OrderWriterDialog extends InternationalizedDataDialog {
	private final static Logger log = Logger.getInstance(OrderWriterDialog.class);
	...
	/**
	 * Create a new OrderWriterDialog object as a dialog with a parent
	 * window.
	 */
	...
	/**
	 * Create a new OrderWriterDialog object as a dialog with a parent
	 * window and a set of selected regions.
	 */
	public OrderWriterDialog(Frame owner, boolean modal, GameData initData, Properties p,
	...
	private Container getControlsPanel() {
		...
		chkECheckComments = new JCheckBox(getString("chk.addecheckcomments.caption"), (new Boolean(settings.getProperty("OrderWriter.addECheckComments", "true"))).booleanValue());
		chkRemoveSCComments = new JCheckBox(getString("chk.removesemicoloncomments.caption"), (new Boolean(settings.getProperty("OrderWriter.removeSCComments", "false"))).booleanValue());
		chkRemoveSSComments = new JCheckBox(getString("chk.removedoubleslashcomments.caption"), (new Boolean(settings.getProperty("OrderWriter.removeSSComments", "false"))).booleanValue());
		chkConfirmedOnly = new JCheckBox(getString("chk.skipunconfirmedorders.caption"), (new Boolean(settings.getProperty("OrderWriter.confirmedOnly", "false"))).booleanValue());
		chkSelRegionsOnly = new JCheckBox(getString("chk.selectedregions.caption"), (new Boolean(settings.getProperty("OrderWriter.includeSelRegionsOnly", "false"))).booleanValue());
		...
		pnlCmdSave.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("border.outputoptions")));
	...
	private Container getFactionPanel() {
		...
		pnlCmdSave.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("border.faction")));
	...
	private void setGroups(Faction f) {
		...
		if(f == null || f.groups == null || f.groups.isEmpty()) return;
		cmbGroup.addItem("");
	...
	private Container getGroupPanel() {
		...
		JPanel pnlCmdSave = new JPanel(new GridLayout(1, 1));
		pnlCmdSave.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("border.group")));
	...
	private Container getFilePanel() {
		...
		pnlFile.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("border.outputfile")));
	...
	private Container getMailPanel() {
		...
		GridBagConstraints c = new GridBagConstraints();
		pnlMail.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(), getString("border.mailoptions")));

		chkUseSettingsFromCR = new JCheckBox(getString("chk.usesettingsfromcr.caption"), (new Boolean(settings.getProperty("OrderWriter.useSettingsFromCr", "true"))).booleanValue());
		...
		chkCCToSender = new JCheckBox(getString("chk.cctosender.caption"), (new Boolean(settings.getProperty("OrderWriter.CCToSender", "true"))).booleanValue());
	...
	private void storeSettings() {
		...
		settings.setProperty("OrderWriter.addECheckComments", (new Boolean(chkECheckComments.isSelected())).toString());
		settings.setProperty("OrderWriter.removeSCComments", (new Boolean(chkRemoveSCComments.isSelected())).toString());
		settings.setProperty("OrderWriter.removeSSComments", (new Boolean(chkRemoveSSComments.isSelected())).toString());
		settings.setProperty("OrderWriter.confirmedOnly", (new Boolean(chkConfirmedOnly.isSelected())).toString());
		if(chkUseSettingsFromCR.isEnabled()) {
			settings.setProperty("OrderWriter.useSettingsFromCr", new Boolean(chkUseSettingsFromCR.isSelected()).toString());
		}
		settings.setProperty("OrderWriter.CCToSender", new Boolean(chkCCToSender.isSelected()).toString());
		if (chkSelRegionsOnly.isEnabled()) {
			settings.setProperty("OrderWriter.includeSelRegionsOnly", (new Boolean(chkSelRegionsOnly.isSelected())).toString());
		}
		...
		try {
			...
			if (chkFixedWidth.isSelected()) {
				...
				try {
					...
					JOptionPane.showMessageDialog(this, getString("msg.invalidfixedwidth.text"), getString("msg.invalidfixedwidth.title"), JOptionPane.WARNING_MESSAGE);
					return false;
				}
			...
			(new java.text.MessageFormat(getString("msg.writtenunits.text"))).format(new Object[] {String.valueOf(writtenUnits),String.valueOf(allUnits),faction}),
			getString("msg.writtenunits.title"),
			JOptionPane.INFORMATION_MESSAGE);
		} catch (IOException ioe) {
			setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			JOptionPane.showMessageDialog(this, getString("msg.erroronsave.text") + ioe.toString(), getString("msg.erroronsave.title"), JOptionPane.WARNING_MESSAGE);
			return false;
		}
	...
	private boolean checkPassword() {
		...
		if (f.password == null) {
			...
			JOptionPane.showMessageDialog(getRootPane(), (new java.text.MessageFormat(getString("msg.nopassword.text"))).format(msgArgs));
			return false;
		} else {
			return true;
		}
	...
	protected void sendMail() {
		...
		if (sender.equals("")) {
			...
			JOptionPane.showMessageDialog(ae, getString("msg.invalidfromaddress.text"), getString("msg.mailerror.title"), JOptionPane.WARNING_MESSAGE);
			return;
		}
		if (recipient.equals("")) {
			ae.getTopLevelAncestor().setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			JOptionPane.showMessageDialog(ae, getString("msg.invalidrecipient.text"), getString("msg.mailerror.title"), JOptionPane.WARNING_MESSAGE);
			return;
		}
		if (mailHost.equals("")) {
			ae.getTopLevelAncestor().setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			JOptionPane.showMessageDialog(ae, getString("msg.invalidsmtpserver.text"), getString("msg.mailerror.title"), JOptionPane.WARNING_MESSAGE);
			return;
		} 
		...
		try {
			...
			mailMessage.setHeader("Content-Type", "text/plain; charset="+FileType.DEFAULT_ENCODING);
			...
			JOptionPane.showMessageDialog(ae, (new java.text.MessageFormat(getString("msg.smtpserverunreachable.text"))).format(msgArgs), getString("msg.mailerror.title"), JOptionPane.WARNING_MESSAGE);
			log.debug(e);
			return;
		}
		try {
			mailWriter = new OutputStreamWriter(mailMessage.getPrintStream(),FileType.DEFAULT_ENCODING);
			...
			JOptionPane.showMessageDialog(ae, (new java.text.MessageFormat(getString("msg.transfererror.text"))).format(msgArgs), getString("msg.mailerror.title"), JOptionPane.WARNING_MESSAGE);
			log.debug(e);
			return;
		}
	...
	protected void saveToFile() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, (new java.text.MessageFormat(getString("msg.writeerror.text"))).format(msgArgs), getString("msg.writeerror.title"), JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	public synchronized static Map getDefaultTranslations() {
		if(defaultTranslations == null) {
			...
			defaultTranslations.put("chk.removesemicoloncomments.caption" , "Remove semicolon comments");
			defaultTranslations.put("chk.removedoubleslashcomments.caption" , "Remove double-slash comments");
			defaultTranslations.put("chk.skipunconfirmedorders.caption" , "Skip units with unconfirmed orders");
			...
			defaultTranslations.put("msg.writeerror.text" , "Unable to write to file \"{0}\". Please choose a different target file.");
			defaultTranslations.put("msg.writeerror.title" , "Error on save");
			defaultTranslations.put("msg.erroronsave.text" , "An error occurred while saving the orders:\n\n");
			defaultTranslations.put("msg.erroronsave.title" , "Error on save");
			defaultTranslations.put("msg.nopassword.text" , "Faction \"{0}\" does not have a password set, therefore Magellan cannot export the orders of that faction.\nYou can set a password for your faction in the faction statistics dialog (in the Extras menu) - passwords are necessary\nfor exporting orders. For safety reasons, passwords are stored only locally on your computer.");
			defaultTranslations.put("msg.mailerror.title" , "Error on mailing");
			defaultTranslations.put("msg.invalidfromaddress.text" , "The specified from address is invalid.");
			defaultTranslations.put("msg.invalidrecipient.text" , "The specified recipient address is invalid.");
			defaultTranslations.put("msg.invalidsmtpserver.text" , "The specified SMTP server is invalid.");
			defaultTranslations.put("msg.smtpserverunreachable.text" , "Unable to connect to the specified SMTP server \"{0}\". Maybe the server name is invalid or the server is temporarily not available.\n\n({1})");
			defaultTranslations.put("msg.transfererror.text" , "Unable to transfer the orders to the SMTP server. Maybe the server is temporarily unavailable.\n\n({0})");
			defaultTranslations.put("msg.invalidfixedwidth.text" , "The specified maximum line width is invalid.");
			...
			defaultTranslations.put("msg.writtenunits.text" , "Wrote {0} of {1} units from faction {2}.");
		}
