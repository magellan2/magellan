...
public class PathPreferencesAdapter extends JPanel implements PreferencesAdapter {
	...
	protected List textFields,keys;
	...
	public void applyPreferences() {
		if (textFields.size()>0)
			for(int i=0;i<textFields.size();i++)
				settings.setProperty((String)keys.get(i),((JTextField)textFields.get(i)).getText());
	}
	...
	protected class DirButton extends JButton implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			}catch(Exception exc) {fchooser.setCurrentDirectory(file);}
