...
public abstract class DetailedPreferencesAdapter extends JPanel implements PreferencesAdapter {
	...
	protected int rows = 0, count = 0, subcount[];
	
	/** Creates new DetailedPreferencesAdapter */
	public DetailedPreferencesAdapter(int count, int subcount[], Properties p, String prefix, String sK[][], String lK[], int rows) {
		this(count, subcount, p, prefix, sK, lK, rows, false);
	}
	
	public DetailedPreferencesAdapter(int count, int subcount[], Properties p, String prefix, String sK[][], String lK[], int rows, boolean waitWithInit) {
		settings = p;
		this.prefix = prefix;
		settKeys = sK;
		langKeys = lK;
	...
	protected void init() {
		...
		for(int i=0; i<count; i++) {
			...
			if (subcount != null && subcount[i] != 0) { // create detail
				...
				}catch(Exception mexc) {} // maybe no help available
	...
	protected void initProperties() {
		for(int i=0; i<properties.length; i++) {
			properties[i] = settings.getProperty(prefix+"."+settKeys[i][0], settKeys[i][1]).equals(TRUE);
		}
	}
	...
	protected void initBoxes() {
		for(int i=0; i<boxes.length; i++) {
			...
			}catch(Exception mexc) {} // maybe no tooltip
		}
	}
