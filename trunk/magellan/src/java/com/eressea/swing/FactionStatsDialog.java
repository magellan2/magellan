...
public class FactionStatsDialog extends InternationalizedDataDialog {
					...
					for(ListIterator iter = (ListIterator) victims.iterator(); iter.hasNext();) {
						...
						if(f.units().size() > 0) {
							...
							JOptionPane.showMessageDialog(d,
														  (new java.text.MessageFormat(getString("msg.factioncontainsunits.text"))).format(msgArgs));
							veto = true;
						}
						...
						if(!veto) {
							...
							if(JOptionPane.showConfirmDialog(d,
																 (new java.text.MessageFormat(getString("msg.confirmdeletefaction.text"))).format(msgArgs),
																 getString("msg.confirmdeletefaction.title"),
	...
	public static synchronized Map getDefaultTranslations() {
		if(defaultTranslations == null) {
			...
			defaultTranslations.put("msg.factioncontainsunits.text",
									"The faction \"{0}\" cannot be deleted because it contains units.");
			defaultTranslations.put("msg.factionisallied.text",
									"The faction \"{0}\" cannot be deleted because faction \"{1}\" has an alliance with it.");
			defaultTranslations.put("msg.factionownsbuilding.text",
									"The faction \"{0}\" cannot be deleted because it is the owner of building \"{1}\".");
			defaultTranslations.put("msg.factionownsship.text",
