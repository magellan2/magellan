...
public class ContextManager extends MouseAdapter {
			...
			if (path != null) {
				...
				if (last instanceof DefaultMutableTreeNode) {
					...
					if (user != null && (user instanceof Changeable)) {
						...
						if ((change.getChangeModes() & Changeable.CONTEXT_MENU) != 0 && change.getContextFactory() != null) {
							JPopupMenu menu = change.getContextFactory().createContextMenu(data, change.getArgument(), selection, node);
							if (menu != null) {
			...
			if (!found) {
				if (failFactory != null) {
					...
					if (failArgument != null) {
						...
						if (node != null) {
							menu = failFactory.createContextMenu(data, node.getUserObject(), selection, node);
						}
						if (menu == null) {
	 ...
	 * If there is not enough space on the screen, the menu not shown
	 * down and right from (x,y) but left or up or both of that point.
	 */
