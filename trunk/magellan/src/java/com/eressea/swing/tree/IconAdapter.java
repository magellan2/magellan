 ...
 * @version
 */
public class IconAdapter extends InternationalizedPanel implements ExtendedPreferencesAdapter, ItemListener {
	private final static Logger log = Logger.getInstance(IconAdapter.class);
	...
	protected class EmphasizeStyle extends JPanel implements ActionListener{
		protected JCheckBox bold, italic, actColor;
		...
		public EmphasizeStyle() {
			...
			style.add(bold=new JCheckBox(getString("emphasize.bold"),(CellRenderer.emphasizeStyleChange & Font.BOLD) != 0));
			style.add(italic=new JCheckBox(getString("emphasize.italic"),(CellRenderer.emphasizeStyleChange & Font.ITALIC) != 0));
			this.add(style);
			this.add(actColor=new JCheckBox(getString("emphasize.color.text"),CellRenderer.emphasizeColor != null));
		...
		public void actionPerformed(ActionEvent e) {
			...
			Color col=JColorChooser.showDialog(this,getString("colorchooser.title"),source.getBackground());
			if (col!=null) {
	...
	protected class ColorMapping extends JPanel implements ActionListener,PreferencesAdapter, ListSelectionListener, KeyListener {
		...
		protected class ColorMappingListCellRenderer extends JLabel implements ListCellRenderer {
			...
			protected Border border1,border2;
			...
			public Component getListCellRendererComponent(JList l, Object value,int index,boolean sel,boolean foc) {
		...
		public ColorMapping() {
			...
			dialog=new HelpDialog((Frame)this.getTopLevelAncestor(),getString("colormap.help.text"),getString("colormap.help.button"));
		}
		...
		protected void createPair() {
			...
			Color col=JColorChooser.showDialog(this,getString("colormap.create.color"),CellRenderer.getTypeset(3).getForeground());
			if (col!=null)
				addPair(value,col);
		}
		...
		protected void changePair() {
			...
			if (newValue.equals(""))
				newValue=elem.value;
			Color col=JColorChooser.showDialog(this,getString("colormap.change.color"),elem.color);
			if (col==null)
				return;
			elem.color=col;
			elem.value=newValue;
	...
	protected class Stylesets extends JPanel  implements ActionListener, TreeSelectionListener, PreferencesAdapter{
		
		protected JPanel content;
		protected CardLayout contentLayout;
		protected Map subPanels, nodeMap;
		...
		protected class StylesetPanel extends JPanel{
			...
			class DirectionPanel extends JPanel implements ActionListener {
				protected AbstractButton buttons[],last;
				protected int horiz,vertic;
				public DirectionPanel() {
					...
					for(int j=0;j<3;j++) {
						int jv=0;
						switch(j) {
							case 0: jv=SwingConstants.TOP;break;
							case 1: jv=SwingConstants.CENTER;break;
							case 2: jv=SwingConstants.BOTTOM;break;
						}
						for(int i=0;i<3;i++) {
							int iv=0;
							switch(i) {
								case 0: iv=SwingConstants.LEFT;break;
								case 1: iv=SwingConstants.CENTER;break;
								case 2: iv=SwingConstants.RIGHT;break;
							}
				...
				public void apply(GraphicsStyleset set) {
					...
					int i=vertic;
					if (last==buttons[0] || last==buttons[2] || last==buttons[6] || last==buttons[8]) {
			...
			protected class ColorPanel extends JPanel implements ActionListener{
				...
				public ColorPanel() {
					...
					for(int i=0;i<4;i++) {
						con.gridx=0;
						con.gridy=i;
						this.add(boxes[i]=new JCheckBox(getString("styles.color."+String.valueOf(i))),con);
						con.gridx=1;
		...
		public Stylesets() {
			...
			for(int i = 0; i<3; i++) {
				String key = null;
				switch(i) {
					case 0: key = "SIMPLE";break;
					case 1: key = "MAIN";break;
					case 2: key = "ADDITIONAL";break;
				}
		...
		public void updatePreferences() {
			...
			DefaultMutableTreeNode node = new DefaultMutableTreeNode(new TreeObject("SIMPLE", getString("styles.simple")));
			...
			node = new DefaultMutableTreeNode(new TreeObject("ADDITIONAL", getString("styles.additional")));
			newRoot.add(node);
			nodeMap.put("MAIN", node);
		...
		protected void addStyleset() {
			...
			if (!stylesets.isSelectionEmpty()) {
				TreeObject obj = (TreeObject)((DefaultMutableTreeNode)stylesets.getSelectionPath().getLastPathComponent()).getUserObject();
				name = obj.name+"."+name;
			}
		...
		protected void removeStyleset() {
			if (!stylesets.isSelectionEmpty()) {
				DefaultMutableTreeNode node = (DefaultMutableTreeNode)stylesets.getSelectionPath().getLastPathComponent();				
		...
		public void valueChanged(TreeSelectionEvent e) {
			if (stylesets.isSelectionEmpty()) {
				...
				TreeObject obj = (TreeObject)((DefaultMutableTreeNode)e.getPath().getLastPathComponent()).getUserObject();
				if (obj != null) {
	...
	static {
		defaultTranslations.put("icontext.position.text" , "Choose the position of the text in the left field. The middle place marks the place of the icon.");
		...
		defaultTranslations.put("emphasize.tooltip" , "Visual style of strong information, e.g. units with non-confirmed orders.");
		...
		defaultTranslations.put("colormap.change.value" , "Enter new value. Empty field indicates that you want to use your old value.");
