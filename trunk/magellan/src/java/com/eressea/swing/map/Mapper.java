...
/**
 * A component displaying a map based on a <tt>GameData</tt> object.
 * The appearance of the map is made configurable by using
 * combinations of classes implementing the <tt>CellRenderers</tt>
 * interface.
 * <p><b>Note:</b></p>
 * <p>This class avoids Java2D methods so it can be used with JDK
 * version earlier than 1.2.</p>
 */
public class Mapper extends InternationalizedDataPanel implements SelectionListener, Scrollable, UnitOrdersListener, GameDataListener, Initializable {
	private final static Logger log = Logger.getInstance(Mapper.class);
	public final static int PLANE_REGION = 0;
	public final static int PLANE_BORDER = 1;
	public final static int PLANE_BUILDING = 2;
	public final static int PLANE_SHIP = 3;
	public final static int PLANE_TEXT = 4;
	
	public final static int PLANE_PATH = 5;
	public final static int PLANE_HIGHLIGHT = 6;
	public final static int PLANE_MARKINGS = 7;
	
	private final static int PLANES = 8;
	...
	public Mapper(EventDispatcher ed, Properties p, Collection customRenderers,CellGeometry geom) {
		...
		addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent me) {
				...
				Coordinate c = cellGeometry.getCoordinate(me.getPoint().x + mapToScreenBounds.x, me.getPoint().y + mapToScreenBounds.y, showLevel);
				...
				if (r != null) {
					if ((me.getModifiers() & MouseEvent.BUTTON1_MASK) != 0) {
						if ((me.getModifiers() & KeyEvent.CTRL_MASK) != 0) {
							...
							dispatcher.fire(new com.eressea.event.SelectionEvent(mapper, selectedRegions.values(), null, SelectionEvent.ST_REGIONS));
							repaint();
							prevDragRegion = r;
						} else {
							activeRegion = r;
							activeObject = r;
							dispatcher.fire(new com.eressea.event.SelectionEvent(mapper, null, activeRegion, SelectionEvent.ST_DEFAULT));
							repaint();
						}
	...
	public String getToolTipText(MouseEvent e) {
		if (tooltipDefinition != null) {
			try{
				Coordinate c = cellGeometry.getCoordinate(e.getPoint().x + mapToScreenBounds.x, e.getPoint().y + mapToScreenBounds.y, showLevel);
				Region r = data.getRegion(c);
				if (r!=null) {
	...
	/**
	 * Add a cell renderer object to the mapper. Each cell renderer
	 * has a rendering plane associated with it, so if there is
	 * already a renderer in the rendering plane of the added renderer
	 * the old renderer is removed.
	 *
	 ...
	 * This function can be used to override the renderes default
	 * rendering plane.
	 *
	 * @param renderer the object responsible for rendering a
	 *	graphical representation of regions.
	 * @param plane the plane the renderer will draw to. Lower
	 *	planes are painted over by higher planes. See
	 * 	the constants in com.eressea.swing.map.Mapper for
	 * 	possible values or choose a value between 0 and
	 *  getRenderPlainCount() - 1.
	 */
	...
	/**
	 * Get the cell renderer objects that are available for a certain
	 * rendering plane. It is suggested that these objects are used
	 * for calling one of the setRenderer() methods.
	 *
	 * @param plane the plane the renderer will draw to. Lower
	 *	planes are painted over by higher planes. See
	 * 	the constants in com.eressea.swing.map.Mapper for
	 * 	possible values.
	 * @return the renderer object associated with the specified
	 *  rendering plane or null if no such association exists.
	 */
	...
	/**
	 * Returns a list of object containing the rendering planes
	 * existing in this Mapper object. The planes are sorted
	 * with ascending plane indices.
	 */
	...
	/**
	 * Set a path - a list of consecutive regions - to be rendered
	 * by the renderer registered to the path rendering plane.
	 *
	 * @param path a list of <tt>Region</tt> objects to be rendered
	 *  as a path on the map.
	 * @param isPersistent if <tt>true</tt>, always render the path,
	 *  else render the path only until a different region is
	 *  selected.
	 */
	...
	/**
	 * Creates a sublist of regions to render according to the state
	 * of the given int. Values are interpreted as those of RenderingPlane.
	 */
	protected List createSubList(int condition, Coordinate upperLeft, Coordinate lowerRight, List regionList, int duration, int paintNumber) {
	...
	public void paint(Graphics g) {
		...
		Coordinate lowerRightCorner = cellGeometry.getCoordinate(offset.x + clipBounds.width, offset.y + clipBounds.height, showLevel);		
		...
		if (!clipBounds.equals(currentBounds) || isRenderContextChanged()) {
			...
			if (buffer == null || !clipBounds.equals(currentBounds)) {
				...
				buffer = new java.awt.image.BufferedImage(clipBounds.width, clipBounds.height,java.awt.image.BufferedImage.TYPE_INT_ARGB);
			}
			...
			for (int planeIndex = 0; planeIndex<5 && planeIndex < planes.length ; planeIndex++) {
				if (planes[planeIndex]==null) {
					continue;
				}
		...
		if (planes.length>5) {		
			...
			for (int planeIndex = 5; planeIndex < planes.length ; planeIndex++) {
				if (planes[planeIndex]==null) {
					continue;
				}
	...
	/**
	 * Returns the current scale or zoom factor. This value is a real
	 * factor, i.e. 1.0 means that the components are painted
	 * according to the values supplied by the underlying CellGeometry
	 * object.
	 */
	...
	/**
	 * Sets the scale or zoom factor. This value is a real
	 * factor, i.e. 1.0 means that the components are painted
	 * according to the values supplied by the underlying CellGeometry
	 * object.
	 */
	...
	/**
	 * Returns a list containing all the different levels
	 * ('Eressea-Ebenen') this Mapper knows of. The list
	 * contains Integer objects stating the level number.
	 */
	...
	/**
	 * Sets the level ('Eressea-Ebene') this Mapper knows of. The list
	 * contains Integer objects stating the level number.
	 */
	...
	/**
	 * Returns the bounds of the specified region on this
	 * component.
	 *
	 * @param cell the coordinate of the region to be evaluated.
	 * @return the bounds (the upper left corner and the size) of the
	 * region cell in component coordinates.
	 */
	...
	/**
	 * Returns the coordinate of the region that is at the center of
	 * the currently displayed area.
	 *
	 * @param clipBounds the bounds indicating which part of the
	 * mappers drawing area is actually visible.
	 */
	...
	/**
	 * Returns the location (upper left corner) of the drawing area
	 * so that a certain region is at the center of the view port.
	 *
	 * @param viewSize the size of the mappers viewport, i.e. the
	 * size of the part of the mappers drawing area that is actually
	 * visible.
	 * @param center the coordinate to center on.
	 * @return a Point with x and y so that a view port of size
	 * viewSize is centered over the specified region center.
	 */
	...
	/**
	 * Creates a preferences panel allowing to configure this
	 * component.
	 */
	...
	/**
	 * Returns whether deferred painting after loading and scaling
	 * images is used or not. Together with the ImageCellRenderer
	 * class this option tells the mapper/renderer whether to scale
	 * images synchronously and introduce delays on painting or to
	 * scale images asynchronously and trigger a redraw after a short
	 * amount of time.
	 */
	...
	/**
	 * Activates or de-activates deferred painting after loading and
	 * scaling images. Together with the ImageCellRenderer class this
	 * option tells the mapper/renderer whether to scale images
	 * synchronously and introduce delays on painting or to scale
	 * images asynchronously and trigger a redraw after a short
	 * amount of time.
	 */
	...
	/**
	 * Returns a rectangle that indicates the offset and the size of
	 * the whole map that is formed by data.regions(). The
	 * values returned are given in pixels as returned by
	 * CellGeometry, i.e. if there is only region 0, 0 then the
	 * returned rectangle would be : x=0, y=0, width=cellwidth,
	 * height=cellheight.
	 */
	...
	protected RenderingPlane[] initRenderingPlanes() {
		...
		p[PLANE_REGION].setRenderer(getRenderer(settings.getProperty("Mapper.Planes." + PLANE_REGION, "com.eressea.swing.map.RegionImageCellRenderer")));
		
		p[PLANE_BORDER] = new RenderingPlane(PLANE_BORDER, getString("plane.border.name"));
		p[PLANE_BORDER].setRenderer(getRenderer(settings.getProperty("Mapper.Planes." + PLANE_BORDER, "com.eressea.swing.map.BorderCellRenderer")));
		
		p[PLANE_BUILDING] = new RenderingPlane(PLANE_BUILDING, getString("plane.building.name"));
		p[PLANE_BUILDING].setRenderer(getRenderer(settings.getProperty("Mapper.Planes." + PLANE_BUILDING, "com.eressea.swing.map.BuildingCellRenderer")));
		
		p[PLANE_SHIP] = new RenderingPlane(PLANE_SHIP, getString("plane.ship.name"));
		p[PLANE_SHIP].setRenderer(getRenderer(settings.getProperty("Mapper.Planes." + PLANE_SHIP, "com.eressea.swing.map.ShipCellRenderer")));
		
		p[PLANE_TEXT] = new RenderingPlane(PLANE_TEXT, getString("plane.text.name"));
		p[PLANE_TEXT].setRenderer(getRenderer(settings.getProperty("Mapper.Planes." + PLANE_TEXT, "com.eressea.swing.map.TextCellRenderer")));
		
		p[PLANE_PATH] = new RenderingPlane(PLANE_PATH, getString("plane.path.name"), RenderingPlane.ACTIVE_OBJECT);
		p[PLANE_PATH].setRenderer(getRenderer(settings.getProperty("Mapper.Planes." + PLANE_PATH, "com.eressea.swing.map.PathCellRenderer")));
		
		p[PLANE_HIGHLIGHT] = new RenderingPlane(PLANE_HIGHLIGHT, getString("plane.highlight.name"), RenderingPlane.VISIBLE_REGIONS | RenderingPlane.ACTIVE_OR_SELECTED);
		p[PLANE_HIGHLIGHT].setRenderer(getRenderer(settings.getProperty("Mapper.Planes." + PLANE_HIGHLIGHT, "com.eressea.swing.map.HighlightImageCellRenderer")));
		
		p[PLANE_MARKINGS] = new RenderingPlane(PLANE_MARKINGS, getString("plane.markings.name"), RenderingPlane.VISIBLE_REGIONS | RenderingPlane.TAGGED_REGIONS);
		p[PLANE_MARKINGS].setRenderer(getRenderer(settings.getProperty("Mapper.Planes." + PLANE_MARKINGS, "com.eressea.swing.map.MarkingsImageCellRenderer")));
		
		return p;
	}
	
	private Collection initAvailableRenderers(CellGeometry geo, Properties settings,Collection cRenderers) {
	 ...
	 * @param renderContextChanged New value of property renderContextChanged.
	 */
