...
/**
 * A class holding the information necessary to describe the 
 * geometry of a map cell. The information should be sufficient
 * to calculate the size of a cell, an optional offset of the
 * opaque part of an image representing the cell and the position
 * of a cell with arbitrary coordinates on the map in pixels.
 * Some effort has been put into the routines calculating the
 * pixel position of a cell and for retrieving the coordinate of a
 * cell containing a certain pixel position with regard to good
 * accuracy.
 */
public class CellGeometry {
	private final static Logger log = Logger.getInstance(CellGeometry.class);
	...
	/**
	 * Creates a new CellGeometry object initializing itself with the
	 * data available in the specified resource file. The file is
	 * supposed to have the following layout:
	 * <p><tt>x0=32<br>x1=63<br>x2=63<br>x3=32<br>x4=0<br>x5=0<br>
	 * y0=0<br>y1=16<br>y2=47<br>y3=63<br>y4=47<br>y5=16<br>
	 * imgOffsetx=8<br>imgOffsety=8<br>imgSizex=80<br>imgSizey=80
	 * </tt></p>
	 * These values are default values if they cannot be retrieved
	 * from the file. The coordinates are interpreted to describe a
	 * symmetric hexagon that sits at the coordinate axes with
	 * positive x values increasing to the right and positive y
	 * values increasing downwards. The x0 through x5 values describe
	 * the x values of the points that make up the hexagonal cell,
	 * y0 through y5 are the respective y values. x0, y0 represent
	 * the top node of the hexagon, the other points are numbered up
	 * clockwise. Therefore x4, x5 and y0 are expected to always be
	 * 0, else no guarantee can be made as to the correctness of the
	 * position calculations.
	 * The imgOffset and imgSize values give additional information
	 * about how the hexagon is located within the graphics files
	 * used together with this cellgeometry. This information is
	 * necessary to calculate the images' pixel positions for 
	 * painting. imgOffset is the offset of the opaque part of the
	 * image representing a cell within the graphics file, imgSize
	 * denotes the size of the graphics files in pixels.
	 * The created CellGeometry is fully initialized with this data,
	 * no further information has to be given.
	 *
	 * @param fileName the name of the file to read the geometry data
	 * from. fileName is retrieved from the current resource bundle.
	 */
	public CellGeometry(String fileName) {
		Properties p = new Properties();
		try {
			java.net.URL url = com.eressea.resource.ResourcePathClassLoader.getResourceStatically("images/map/" + fileName);
			InputStream is = url.openStream();
			p.load(is);
			is.close();
		}
		catch (Exception e) {
			log.error("CellGeometry(): unable to load file images/map/" + fileName + " from resource path. Using default values",e);
		}
		...
		setImageOffset(Integer.parseInt(p.getProperty("imgOffsetx", "8")), Integer.parseInt(p.getProperty("imgOffsety", "8")));
		setImageSize(Integer.parseInt(p.getProperty("imgSizex", "80")), Integer.parseInt(p.getProperty("imgSizey", "80")));
	}

	/**
	 * Sets the hexagon coordinates of the cell geometry, the
	 * supplied arrays are expected to be of size 6 and the values
	 * are expected to confirm with the restrictions given in the
	...
	/**
	 * Sets the offset of the opaque part of an image representing
	 * a cell in the graphics file in pixels. The x and y values
	 * are expected to be non-negative and smaller than the size
	 * of the cell hexagon.
	 */
	...
	/**
	 * Sets the size of graphics files used together with this cell
	 * geometry.
	 */
	...
	/**
	 * Set a scale factor to be used for all calculations of cell
	 * positions and sizes.
	 */
	...
	/**
	 * Returns the scaled pixel position where to draw an image to
	 * exactly match the position of the cell with the map
	...
	/**
	 * Returns the scaled size of images used together with this cell
	 * geometry.
	 */
	...
	/**
	 * Returns the scaled pixel position and size of an image to
	 * exactly match the position of the cell with the map coordinate
	 * mapX and mapY.
	 */
	...
	/**
	 * Returns the unscaled pixel position on the x-axis of a cell with
	 * the map coordinates mapX and mapY.
	 */
	...
	/**
	 * Returns the unscaled pixel position on the y-axis of a cell with
	 * the map coordinates mapX and mapY.
	 */
	...
	/**
	 * Returns the scaled pixel position on the x-axis of a cell with
	 * the map coordinates mapX and mapY.
	 */
	...
	/**
	 * Returns the scaled pixel position on the y-axis of a cell with
	 * the map coordinates mapX and mapY.
	 */
	...
	/**
	 * Returns the scaled pixel position of a cell with
	 * the map coordinates mapX and mapY.
	 */
	...
	/**
	 * Returns the scaled pixel position and size of a cell with
	 * the map coordinates mapX and mapY.
	 */
	...
	/**
	 * Returns the map coordinate of a cell containing the pixels
	 * sx and sy. The z-value of the returned coordinate is z.
	 */
	...
	/**
	 * Returns a new Polygon object that is a scaled instance of
	 * the unscaled polygon.
	 */
