...
public class TextCellRenderer extends AbstractTextCellRenderer {
	private final static Logger log = Logger.getInstance(TextCellRenderer.class);

	protected Color fontColor = Color.black;
	protected Font unscaledFont = null, font = null;
	protected FontMetrics fontMetrics = null;
	protected int minimumFontSize = 10, fontHeight = 0;
	...
	public TextCellRenderer(CellGeometry geo, java.util.Properties settings) {
		...
		if (settings != null) {
			...
			setScalingFont((new Boolean(settings.getProperty("TextCellRenderer.isScalingFont", "false"))).booleanValue());
		}
		setFont(new Font(settings.getProperty("TextCellRenderer.fontName", "SansSerif"), Integer.parseInt(settings.getProperty("TextCellRenderer.fontStyle", Font.PLAIN + "")), Integer.parseInt(settings.getProperty("TextCellRenderer.fontSize", 11 + ""))));
		setMinimumFontSize(Integer.parseInt(settings.getProperty("TextCellRenderer.minimumFontSize", "10")));
	...
	protected void setScalingFont(boolean bool) {
		if (bool != isScalingFont()) {
			...
			settings.setProperty("TextCellRenderer.isScalingFont", (new Boolean(isScalingFont())).toString());
		}
	}
		...
		private void init() {
			...
			pnlFontColor.addMouseListener(new MouseAdapter() {
				public void mousePressed(MouseEvent e) {
					Color newColor = JColorChooser.showDialog(pnlFontColor.getTopLevelAncestor(), getString("textcolor"), pnlFontColor.getBackground());
					if (newColor != null) {
						pnlFontColor.setBackground(newColor);
					}
				}
			});
			JLabel lblFontColor = new JLabel(getString("fontcolor"));
			...
			JLabel lblFontName = new JLabel(getString("fonttype"));
			lblFontName.setLabelFor(cmbFontName);
			
			chkFontBold = new JCheckBox(getString("usebold"), source.getFont().getStyle() != Font.PLAIN);
			...
			JLabel lblFontSize = new JLabel(getString("fontsize"));
