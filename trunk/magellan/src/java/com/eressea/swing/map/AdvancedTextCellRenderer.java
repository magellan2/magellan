 ...
 * to create region dependent output.
 *
 * <i>Caches a lot to improve performance, so let the interpreter allocate more
 * memory!</i>
 *
 * @author  Andreas
 * @version 1.1
 */
public class AdvancedTextCellRenderer extends TextCellRenderer implements ExtendedShortcutListener,GameDataListener, ContextChangeable, ActionListener {
	...
	public AdvancedTextCellRenderer(CellGeometry geo,Properties settings) {
		...
		}catch(Exception exc2) {breakLines = false;}		
		try{
			setHAlign(Integer.parseInt(settings.getProperty("ATR.horizontalAlign","0")));
		}catch(Exception exc) {setHAlign(CENTER);}

		// create shortcut structure
		DesktopEnvironment.registerShortcutListener(KeyStroke.getKeyStroke(KeyEvent.VK_T,KeyEvent.CTRL_MASK | KeyEvent.ALT_MASK), this);
	...
	public void saveSet(Properties settings) {
		...
		if (!exists(set.getName(), settings)) {
			settings.setProperty("ATR.Sets", settings.getProperty("ATR.Sets", "")+";"+set.getName());
		}
	}
	...
	public JMenuItem getContextAdapter() {
		if (contextMenu == null) {
			contextMenu = new JMenu(getString("context.title"));
			fillDefItems();
		}		
		return contextMenu;
	}
	...
	public synchronized static Map getDefaultTranslations() {
		if(defaultTranslations == null) {
			...
			defaultTranslations.put("prefs.fonthelp" , "Please change font properties at the text renderer");
		}
		return defaultTranslations;
	}
	
	/**
	 * Encapsulates one setting containing a name, a definition string and a cache
	 * object.
	 */
	protected class ATRSet {
		protected String name, def, unknown = "-?-";
		...
		class DefListener implements ShortcutListener {
			...
			public DefListener() {
				keys=CollectionFactory.createArrayList(10);
				for(int i=1;i<10;i++)
	...
	protected class ATRPreferences extends JPanel implements ActionListener, PreferencesAdapter, ListSelectionListener {
		...
		protected AbstractButton add, remove, rename, importB, export;
		
		public ATRPreferences(AdvancedTextCellRenderer s) {
			this.s=s;
			this.setLayout(new GridBagLayout());
			
			GridBagConstraints c=new GridBagConstraints(0, 0, 2, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(1,1,1,1), 0, 0);
		...
		protected Component createNavPanel() {
			add = new JButton(getString("prefs.add"));
			add.addActionListener(this);
			remove = new JButton(getString("prefs.remove"));
			remove.addActionListener(this);
			rename = new JButton(getString("prefs.rename"));
			rename.addActionListener(this);
			importB = new JButton(getString("prefs.import"));
			importB.addActionListener(this);
			export = new JButton(getString("prefs.export"));
			...
			GridBagConstraints c = new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(0, 1, 1, 1), 0, 0);
		...
		protected Component createSetPanel() {
			...
			help.add(new JLabel(getString("prefs.replace")));
			help.add(replace = new JTextField(set.getUnknown(),5));
			sPanel.add(help, BorderLayout.NORTH);
			
			help = new JPanel();
			help.add(new JLabel(getString("prefs.def")));//getString("prefs.planes")),c);
		...
		protected Component createAlignPanel() {
			...
			box.add(new JLabel(getString("prefs.aligntext")));
			box.add(Box.createHorizontalStrut(5));
			ButtonGroup group=new ButtonGroup();
			align = new JRadioButton[3];
			for(int i=0;i<3;i++) {
				align[i] = new JRadioButton(getString("prefs.align"+String.valueOf(i)),i==s.getHAlign());
				group.add(align[i]);
				box.add(align[i]);
			}
			return box;
