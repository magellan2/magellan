 ...
 */
public class MessagePanel extends InternationalizedDataPanel implements SelectionListener, TreeSelectionListener, PreferencesFactory, MenuProvider {
	// tree elements
	...
	/**
	 * Handles the selection of a new object to display messages
	 * for.
	 */
	...
	private void show(Region r, DefaultMutableTreeNode parent) {
		...
		if ((r.travelThru != null && r.travelThru.size() > 0) || (r.travelThruShips != null && r.travelThruShips.size() > 0)) {
	...
	/**
	 * Adds the given messages to the parent node, categorized by the section of the
	 * messagetypes and then sorted by messagetypes.
	 */
	...
	protected class Pref extends JPanel implements PreferencesAdapter {
		...
		public Pref(MessagePanel src) {
			...
			GridBagConstraints c = new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(1, 1, 1, 1), 0, 0);
