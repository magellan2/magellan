...
/**
 * A dialog to fetch a destination region (usually for a ship). Out of the
 * knowledge of this destination region magellan can create respective orders
 * that move the ship to that region. It is also possible, to determine, if
 * ship range shall be considered, whether Vorlage-meta-orders shall be
 * created if the destination region can not be reached in one week and if the
 * captain's orders shall be replaced.
 *
...
public class RoutingDialog extends InternationalizedDialog {
		...
		if(destRegions != null) {
			...
			regionName.getDocument().addDocumentListener(new DocumentListener() {
					public void insertUpdate(DocumentEvent e) {
						int i = Collections.binarySearch(regionList,
														 regionName.getText(),
	...
	public class RetValue {
		 ...
		 */
		public RetValue(Coordinate d, boolean b1, boolean b2, boolean b3,
						boolean b4) {
			dest		  = d;
			makeRoute     = b1;
			useRange	  = b2;
	...
	public static synchronized Map getDefaultTranslations() {
		if(defaultTranslations == null) {
			...
			defaultTranslations.put("chkbox.considerrange.title",
									"Consider range");
			defaultTranslations.put("chkbox.createvorlageorders.title",
									"Create Vorlage meta orders");
			defaultTranslations.put("chkbox.replaceorders.title",
									"Replace unit's orders");
			...
			defaultTranslations.put("radiobtn.createsingletrip.title",
									"Plan single trip");
			defaultTranslations.put("radiobtn.createroute.title",
									"Plan permanent route");
		}
