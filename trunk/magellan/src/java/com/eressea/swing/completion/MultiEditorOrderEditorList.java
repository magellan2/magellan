...
public class MultiEditorOrderEditorList extends InternationalizedDataPanel implements OrderEditorList, KeyListener, SelectionListener, TempUnitListener, FocusListener, CacheHandler {
	private final static Logger log = Logger.getInstance(MultiEditorOrderEditorList.class);
	...
	private final static javax.swing.border.Border standardBorder = new LineBorder(Color.lightGray, 2);
	private final static javax.swing.border.Border activeBorder = new LineBorder(Color.darkGray, 2);
	...
	private final static int LIST_UNIT = 0;
	private final static int LIST_FACTION = 1;
	private final static int LIST_REGION = 2;
	private final static int LIST_ISLAND = 3;

	public MultiEditorOrderEditorList(EventDispatcher d, GameData initData, Properties settings, javax.swing.undo.UndoManager _undoMgr) {
	...
	private void readSettings() {
		multiEditorLayout = Boolean.valueOf(settings.getProperty("OrderEditor.multiEditorLayout", Boolean.TRUE.toString())).booleanValue();
		hideButtons =       Boolean.valueOf(settings.getProperty("OrderEditor.hideButtons", Boolean.FALSE.toString())).booleanValue();
		activeBgColor = Colors.decode(settings.getProperty("OrderEditor.activeBackgroundColor", "255,255,255"));
		standardBgColor = Colors.decode(settings.getProperty("OrderEditor.standardBackgroundColor", "228,228,228"));
		activeBgColorConfirmed = Colors.decode(settings.getProperty("OrderEditor.activeBackgroundColorConfirmed", "255,255,102"));
		standardBgColorConfirmed = Colors.decode(settings.getProperty("OrderEditor.standardBackgroundColorConfirmed", "255,204,0"));
	}
	...
	public void selectionChanged(SelectionEvent se) {
		...
		boolean restoreFocus = (currentUnit != null && currentUnit.cache != null && currentUnit.cache.orderEditor != null && currentUnit.cache.orderEditor.hasFocus());
		...
		if (multiEditorLayout) {
			...
			if (currentUnit != null && currentUnit.cache != null && currentUnit.cache.orderEditor != null) {
				currentUnit.cache.orderEditor.setBorder(new TitledBorder(standardBorder, currentUnit.toString() + ": " + currentUnit.persons));
			...
			if (se.getActiveObject() != null) {
				...
				((Unit) se.getActiveObject()).isSpy() == false) {
					...
					if (currentUnit.cache != null && currentUnit.cache.orderEditor != null) {
						currentUnit.cache.orderEditor.setBorder(new TitledBorder(activeBorder, currentUnit.toString() + ": " + currentUnit.persons));
						if(currentUnit.ordersConfirmed) {
					...
					currentUnit = null;
					currentUnitIndex = -1;
					if (((Faction)se.getActiveObject()).trustLevel >= Faction.TL_PRIVILEGED && currentRegion != null) {
			...
			if (activeObject instanceof Unit && ((Unit) se.getActiveObject()).getFaction().trustLevel >= Faction.TL_PRIVILEGED && ((Unit) activeObject).isSpy() == false) {
		...
		// restore focus
		if ((DesktopEnvironment.getMode() == DesktopEnvironment.FRAME || restoreFocus) && currentUnit != null && currentUnit.cache != null && currentUnit.cache.orderEditor != null) {
	...
	public void tempUnitCreated(TempUnitEvent e) {
		...
		if (currentUnit != null && multiEditorLayout) {
			...
			log.debug("MultiEditorOrderEditorList.tempUnitCreated: "+e.getTempUnit().cache.orderEditor);
	...
	/**
	 * Fires an SelectionChanged event if a different editor than the
	 * current one is selected.
	 */
	public void focusGained(FocusEvent e) {
		if (multiEditorLayout && e.getSource() instanceof OrderEditor) {
			if (currentUnit == null || (currentUnit.cache != null && currentUnit.cache.orderEditor != e.getSource())) {
				dispatcher.fire(new SelectionEvent(this, null, ((OrderEditor) e.getSource()).getUnit()));
			}
		}
	}
	...
	/**
	 * Return whether syntax highlighting in the editor is
	 * enabled or disabled.
	 */
	...
	/**
	 * Return the color of the specified token style used for
	...
	/**
	 * Set the color of the specified token style used for
	 * syntax highlighting in the editor.
	 */
	...
	/**
	 * Removes the Adapters for Key-, Caret- & Focusevents from
	 * the given JTextComponents.
	 */
	...
	/**
	 * Adds editors for all units in privileged factions that are in
	 * the specified list to this component. OrderEditors are
	 * created if necessary, else the cached version are used.
	 */
	private void loadEditors(List unitsToAdd) {
		...
		for (Iterator unitIter = allUnits.listIterator(); unitIter.hasNext(); ) {
			...
			if (regionUnit.getFaction().trustLevel >= Faction.TL_PRIVILEGED && regionUnit.isSpy() == false) {
				addUnit(regionUnit);
	...
	/**
	 * Adds the specified unit to this component. This includes
	 * creating and adding a new order editor and updating the
	...
	/**
	 * Performs the clean-up necessary to put the editor list into
	 * a state without units and editors
	 */
	...
	public synchronized static Map getDefaultTranslations() {
		if(defaultTranslations == null) {
			...
			defaultTranslations.put("msg.newtempid.text" , "Please enter an id for the new temporary unit:");
			defaultTranslations.put("msg.newtempid.title" , "New temp id");
			defaultTranslations.put("msg.duplicatetempid.text" , "The specified temp id is already assigned to a unit. Please enter a different id.");
			defaultTranslations.put("msg.duplicatetempid.title" , "Temp id used");
			defaultTranslations.put("msg.invalidtempid.text" , "The temp id you specified is invalid.");
			defaultTranslations.put("prefs.listMode.text" , "This settings affects the units to be shown while selecting a game object. Only useful in Multieditor-Mode.");
			...
			defaultTranslations.put("prefs.listMode.2.text" , "Shows all privileged units of the current Faction");
			defaultTranslations.put("prefs.listMode.1.text" , "Shows all privileged units of the current region");
			defaultTranslations.put("prefs.listMode.0.text" , "Shows all privileged units of the current island");
			...
			defaultTranslations.put("prefs.syntaxhighlighting.caption" , "Activate Syntax highlighting");
			...
			defaultTranslations.put("prefs.inactivebackground.confirmed" , "Color of inactive and confirmed editors");
	...
	class OrderEditorListPreferences extends JPanel implements PreferencesAdapter {
		...
		protected Container getLayoutPanel() {
			...
			chkMultiEditorLayout = new JCheckBox(getString("prefs.multieditorlayout"), source.isMultiEditorLayout());
		...
		protected Container getColorPanel() {
			...
			GridBagConstraints c = new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 2, 1, 1), 0, 0);
			...
			pnlStandardColor.addMouseListener(new MouseAdapter() {
				public void mousePressed(MouseEvent e) {
					Color newColor = JColorChooser.showDialog(((JComponent) e.getSource()).getTopLevelAncestor(), getString("prefs.backgroundcolor"), ((Component) e.getSource()).getBackground());
		...
		protected Container getHighlightPanel() {
			...
			chkSyntaxHighlighting = new JCheckBox(getString("prefs.syntaxhighlighting.caption"), source.getUseSyntaxHighlighting());
		...
		protected Container getListModePanel() {
			...
			GridBagConstraints con = new GridBagConstraints(0, 0, 1, 1, 0.5, 0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 0, 0);
		...
		protected Container createStylesContainer() {
			...
			pnlStylesColor.addMouseListener(new MouseAdapter() {
				public void mousePressed(MouseEvent me) {
					...
					Color newColor = JColorChooser.showDialog(((JComponent) me.getSource()).getTopLevelAncestor(), sc.description + " Farbe", sc.color);
					if (newColor != null) {
						sc.color = newColor;
		...
		private Object[] getStyles() {
			...
			styles[0] = new StyleContainer(getString("prefs.colors.standard"), OrderEditor.S_REGULAR, source.getTokenColor(OrderEditor.S_REGULAR));
			styles[1] = new StyleContainer(getString("prefs.colors.keywords"), OrderEditor.S_KEYWORD, source.getTokenColor(OrderEditor.S_KEYWORD));
			styles[2] = new StyleContainer(getString("prefs.colors.strings"), OrderEditor.S_STRING, source.getTokenColor(OrderEditor.S_STRING));
			styles[3] = new StyleContainer(getString("prefs.colors.numbers"), OrderEditor.S_NUMBER, source.getTokenColor(OrderEditor.S_NUMBER));
			styles[4] = new StyleContainer(getString("prefs.colors.ids"), OrderEditor.S_ID, source.getTokenColor(OrderEditor.S_ID));
			styles[5] = new StyleContainer(getString("prefs.colors.comments"), OrderEditor.S_COMMENT, source.getTokenColor(OrderEditor.S_COMMENT));
	...
	private class SwingGlitchThread implements Runnable {
		...
		public void run() {
			...
			log.debug("MultiEditorOrderEditorList.selectionChanged.runnable: viewRect:"+viewRect);
			...
			if (currentUnit != null &&
				...
				while (!viewRect.contains(viewRect.x,
										  bounds.y,
										  viewRect.width,
					...
					log.debug("MultiEditorOrderEditorList.selectionChanged.runnable: newPos : "+newPos);
				...
				log.debug("MultiEditorOrderEditorList.selectionChanged.runnable: viewRect after:"+viewRect);
				log.debug("MultiEditorOrderEditorList.selectionChanged.runnable: Bounds after:"+bounds);
			}
			content.validate();
	...
	protected class ButtonPanel extends JPanel implements ActionListener {
			...
			DesktopEnvironment.registerShortcutListener(new com.eressea.demo.desktop.ShortcutListener() {
				...
				public Iterator getShortCuts() {
					if (shortcuts == null) {
						...
						shortcuts.add(KeyStroke.getKeyStroke(KeyEvent.VK_T, KeyEvent.CTRL_MASK | KeyEvent.SHIFT_MASK));
					}
		...
		protected void createTempUnit() {
			if (currentUnit != null && currentUnit.getFaction().trustLevel >= Faction.TL_PRIVILEGED) {
				...
				if (data.getCurTempID() == 0) {
					...
					boolean ascending = settings.getProperty("ClientPreferences.ascendingOrder", "true").equalsIgnoreCase("true");
		...
		/**
		 * @return the next int, that is bigger than the given one but
		 *  consists only out of decimal digits (interpreted in the current base)
		 *  if the given int did so also.
		 */
		...
		private void createTempImpl(Unit parentUnit, UnitID id, Region parentRegion) {
			...
			if (!settings.getProperty("MultiEditorOrderEditorList.ButtonPanel.ShowTempUnitDialog", "true").equalsIgnoreCase("true")) {
					...
					if (dialog.isApproved()) {
						...
						try {
							...
							if (parentRegion.getUnit(checkID) == null) {
								...
								JOptionPane.showMessageDialog(this, getString("msg.duplicatetempid.text"), getString("msg.duplicatetempid.title"), JOptionPane.ERROR_MESSAGE);
							}
						}
						catch (NumberFormatException nfe) {
							JOptionPane.showMessageDialog(this, getString("msg.invalidtempid.text"), getString("msg.invalidtempid.title"), JOptionPane.ERROR_MESSAGE);
						}
					} else {
						// dialog canceled
						return;
		...
		protected void deleteTempUnit() {
			if (currentUnit != null && currentUnit instanceof TempUnit) {
				...
				dispatcher.fire(new TempUnitEvent(this, (TempUnit) currentUnit, TempUnitEvent.DELETED), true);
		...
		public void currentUnitChanged() {
			boolean enable = (currentUnit != null && currentUnit.getFaction().trustLevel >= Faction.TL_PRIVILEGED);
	...
	/**
	 * A simple JPanel that implements the Scrollable interface and is
	 * used to hold the order editors.
	 */
	private class ScrollPanel extends JPanel implements Scrollable {
		...
		/**
		 * If the parent of this component is an instance of JViewport
		 * this method returns the maximum of the original preferred size
		...
		public int getScrollableUnitIncrement(java.awt.Rectangle visibleRect, int orientation, int direction) {
			if (orientation == SwingConstants.HORIZONTAL) {
				...
				if (direction < 0) { // up
					...
					lastVisibleComponent = this.getComponentAt(visibleRect.x, visibleRect.y + visibleRect.height);
					if (lastVisibleComponent != null) {
						if (visibleRect.y + visibleRect.height < lastVisibleComponent.getY() + lastVisibleComponent.getHeight()) {
							// component is not fully visible
							return lastVisibleComponent.getY() + lastVisibleComponent.getHeight() - visibleRect.y - visibleRect.height;
						} else {
							// component is fully visible, get next component
							List components = Arrays.asList(this.getComponents());
							int count = components.indexOf(lastVisibleComponent) + 1;
