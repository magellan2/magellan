...
public class OrderEditor extends JTextPane implements DocumentListener, KeyListener, SelectionListener, FocusListener, GameDataListener {
	...
	/**
	 * Indicates whether the contents of this editor have been
	 * modified.
	 */
	...
	/**
	 * If the active object in the selection event is a unit it
	...
	/**
	 * Sets the unit for this editor component. If the orders have
	 * been modified and there is a previous unit registered with this
	...
	/**
	 * Returns a list of the orders this text pane is currently
	 * containing. A order extending over more than one line is
	 * stripped of the trailing backslashes, concatenated and
	 * returned as one order.
	 */
	...
	/**
	 * Puts the list elements in <tt>c</tt> into this text pane,
	 * one at a line.
	 */
	...
	/**
	 * Allows to change the modified state of this text pane.
	 * Setting it to <tt>true</tt> currently does not result in
	...
	public Color getTokenColor(String styleName) {
		return Colors.decode(settings.getProperty("OrderEditor.styles." + styleName + ".color", getDefaultColor(styleName)));
	}
	...
	/**
	 * Adds styles to this component with one style for each
	 * token type.
	 */
	private void initStyles() {
		...
		StyleConstants.setForeground(s, Colors.decode(settings.getProperty("OrderEditor.styles." + S_KEYWORD + ".color", getDefaultColor(S_KEYWORD))));
		s = addStyle(S_STRING, regular);
		StyleConstants.setForeground(s, Colors.decode(settings.getProperty("OrderEditor.styles." + S_STRING + ".color", getDefaultColor(S_STRING))));
		s = addStyle(S_NUMBER, regular);
		StyleConstants.setForeground(s, Colors.decode(settings.getProperty("OrderEditor.styles." + S_NUMBER + ".color", getDefaultColor(S_NUMBER))));
		s = addStyle(S_ID, regular);
		StyleConstants.setForeground(s, Colors.decode(settings.getProperty("OrderEditor.styles." + S_ID + ".color", getDefaultColor(S_ID))));
		s = addStyle(S_COMMENT, regular);
		StyleConstants.setForeground(s, Colors.decode(settings.getProperty("OrderEditor.styles." + S_COMMENT + ".color", getDefaultColor(S_COMMENT))));
	}
	...
	private String getDefaultColor(String styleName) {
		...
		if (styleName.equals(S_STRING)) {
			retVal = "192,0,0";
		} else
		if (styleName.equals(S_NUMBER)) {
			retVal = "0,0,0";
		} else
		if (styleName.equals(S_ID)) {
			retVal = "168,103,0";
		} else
		if (styleName.equals(S_COMMENT)) {
			retVal = "0,128,0";
		}
		return retVal;
	}
	 ...
	 * space. This is a so-called "minor incompatibility" introduced by jdk 1.4.
	 *
	 */
	...
	/**
	 * Format the tokens in the line starting in the
	 * document at insertPos, with regard to the token type colors.
	 */
	private void formatTokens(int startPos) {
		...
		newStartPos = pos[0];
		if(log.isDebugEnabled()) {
			log.debug("OrderEditor.formatTokens("+startPos+"): ("+text+","+pos[0]+","+pos[1]+")");
		}
	...
	private int[] getLineBorders(String text, int offset) {
		...
		try {
			...
			log.error("OrderEditor.getLineBorders(), text: " + text + ", offset: " + offset + ", ret: (" + ret[0] + ", " + ret[1] + ")",e);
		}
		log.debug("OrderEditor.getLineBorders:("+text+","+offset+") "+ret[0]+","+ret[1]);
		if(ret[0] >=0) {
			if(ret[1] > ret[0]) {
				log.debug("OrderEditor.getLineBorders would be \""+text.substring(ret[0],ret[1])+"\"");
			}
		}
		return ret;
	}
	...
	private class OrderEditorCaret extends DefaultCaret {
		...
		protected void adjustVisibility (Rectangle nloc){
			...
			if (keepVisible()) super.adjustVisibility(nloc);
		}
