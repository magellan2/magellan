...
public class Faction extends UnitContainer {
	private final static Logger log = Logger.getInstance(Faction.class);
	...
	/** This trust level indicates that units of this faction may
	 * receive new orders and similar privileges. */
	public static final int TL_PRIVILEGED = 100;

	/**
	 * The password of this faction required for authentication of
	 * orders sent to the Eressea server.
	 */
	...
	/**
	 * Indicates to what amount this faction can be trusted. It also
	 * influences the privileges of this faction (e.g. being able to
	 * edit its units' orders).
	 */
	 ...
	 * false: indicates that this is either a default level or was calculated by Magellan
	 *        based on the alliances of the privileged factions.
	 */
	...
	/**
	 * The allies of this faction are stored in this map with the
	 * faction ID of the ally as key and an <tt>Alliance</tt> object
	 * as value.
	 */
	public Map allies = null;

	/**
	 * The different groups in this faction. The map contains
	 * <tt>ID</tt> objects with the group id as keys and
	 * <tt>Group</tt> objects as values.
	 */
	...
	/**
	 * Creates a new Faction object with the specified id on top of
	 * the specified game data object.
	 */
	...
	/**
	 * Assigns this faction a locale indicating the language of its
	 * report and the orders.
	 */
	...
	/**
	 * Returns the locale of this faction indicating the language of
	 * its report and orders.
	 */
	...
	/**
	 * A faction dependent prefix to be prepended to this faction's
	 * race name.
	 */
	...
	/**
	 * Returns the race of this faction. This method is an alias for
	 * the getType() method.
	 */
	public Race getRace() {
	...
	public static void merge(GameData curGD, Faction curFaction, GameData newGD, Faction newFaction) {
		...
		// ReportMerger.mergeReport() :
			/**
			 * prepare faction trustlevel for merging:
			 * - to be added CR is older or of same age
			 *      -> hold existing trust levels
			 * - to be added CR is newer and contains trust level
			 *   that were set by the user explicitly (or read from CR
			 *   what means the same)
			 *      -> take the trust levels out of the new CR
			 *   otherwise
			 *      -> hold existing trust levels
			 *
			 * This means: set those trust levels, that will not be retained to
			 * default values
			 */
		...
		if (curGD.getDate().equals(newGD.getDate())) {
			...
			if (curFaction.battles != null && curFaction.battles.size() > 0) {
				...
				for (Iterator iter = curFaction.battles.iterator(); iter.hasNext();) {
					...
					try {
						Battle newBattle = new Battle((ID)curBattle.getID().clone(), curBattle.isBattleSpec());
	...
	/**
	 * Compares this faction to another object. Returns true only if
	 * o is not null and an instance of class Faction and their id's
	 * are equal.
	 */
	...
	/**
	 * Imposes a natural ordering on faction objects based on the
